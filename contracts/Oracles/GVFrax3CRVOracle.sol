...
contract GVFrax3CrvOracle is IOracle {
	IGrizzlyVault public constant GVFrax3Crv =
		IGrizzlyVault(0xF437C8cEa5Bb0d8C10Bb9c012fb4a765663942f1);
	...
	constructor() {
		DECIMAL_ADJUSTMENT = int256(
			10**(GVFrax3Crv.decimals() + Frax3Crv.decimals() + FRAX.decimals() - 18)
		); // 10 ** 18
	}
	...
	function _getLatestLPTokenPrice() internal view returns (int256 LPPrice) {
		...
		int256 price = int256(GVFrax3Crv.pricePerShare() * Frax3Crv.get_virtual_price()) *
			minStablePrice;
