...
contract TroveManager is VestaBase, CheckContract, ITroveManager {
	...
	TroveManagerHelpers public troveManagerHelpers;
	...
	modifier troveIsActive(address _asset, address _borrower) {
		require(
			troveManagerHelpers.isTroveActive(_asset, _borrower),
			"IT"
		);
		_;
	}
	...
	function _liquidateNormalMode(
		...
		troveManagerHelpers._closeTrove(_asset, _borrower, ITroveManagerHelpers.Status.closedByLiquidation);
	...
	function _getTotalsFromLiquidateTrovesSequence_RecoveryMode(
		...
		for (vars.i = 0; vars.i < _n && vars.user != firstUser; vars.i++) {
			...
			vars.ICR = troveManagerHelpers.getCurrentICR(assetVars._asset, vars.user, assetVars._price);
	...
	function _redeemCollateralFromTrove(
		...
		if (newDebt == vestaParams.VST_GAS_COMPENSATION(_asset)) {
			...
			troveManagerHelpers._closeTrove(vars._asset, vars._borrower, ITroveManagerHelpers.Status.closedByRedemption);
			_redeemCloseTrove(
				vars._asset,
				_contractsCache,
				vars._borrower,
	...
	function _isValidFirstRedemptionHint(
		...
		if (
			...
			troveManagerHelpers.getCurrentICR(_asset, _firstRedemptionHint, _price) < vestaParams.MCR(_asset)
		) {
			return false;
		}
	...
	function redeemCollateral(
		...
		troveManagerHelpers._requireAmountGreaterThanZero(_VSTamount);
		troveManagerHelpers._requireVSTBalanceCoversRedemption(contractsCache.vstToken, msg.sender, _VSTamount);
		...
		if (
			...
			while (
				...
				troveManagerHelpers.getCurrentICR(_asset, currentBorrower, totals.price) < vestaParams.MCR(_asset)
			) {
