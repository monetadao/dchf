...
contract PriceFeed is Ownable, CheckContract, BaseMath, IPriceFeed {
    ...
    */
    function fetchPrice()
        ...
        returns (uint256)
    {
        address _base = 0x9326BFA02ADD2366b30bacB125260Af641031331;
        address _quote = 0xed0616BeF04D374969f302a34AE4A63882490A8C;
        uint8 _decimals = 18;

        require(_decimals > uint8(0) && _decimals <= uint8(18), "Invalid _decimals");
        uint256 decimals = uint256(10 ** uint256(_decimals));
        ( , uint256 basePrice, , , ) = AggregatorV3Interface(_base).latestRoundData();
        uint8 baseDecimals = AggregatorV3Interface(_base).decimals();
        basePrice = scalePrice(basePrice, baseDecimals, _decimals);

        ( , uint256 quotePrice, , , ) = AggregatorV3Interface(_quote).latestRoundData();
        uint8 quoteDecimals = AggregatorV3Interface(_quote).decimals();
        quotePrice = scalePrice(quotePrice, quoteDecimals, _decimals);

        lastGoodPrice = basePrice * decimals / quotePrice;

        return lastGoodPrice;
    }

    // Adapts the numbers of decimals (Dchf is Using 18 decimals)
