...
contract('DfrancParameters', async (accounts) => {
  ...
  const DCHF_GAS_COMPENSATION_SAFETY_MAX = toBN(dec(200, 18))
  const DCHF_GAS_COMPENSATION_SAFETY_MIN = toBN(dec(1, 18))
  ...
  describe('Dfranc Parameters', async () => {
    ...
    it('Formula Checks: Call every function with default value, Should match default values', async () => {
      ...
      await dfrancParameters.setDCHFGasCompensation(ZERO_ADDRESS, dec(200, 18))
    ...
    it('Try to edit Parameters has User, Revert Transactions', async () => {
      ...
      await assertRevert(
        dfrancParameters.setDCHFGasCompensation(ZERO_ADDRESS, GAS_COMPENSATION, { from: user })
      )
    ...
    it('sanitizeParameters: User call sanitizeParameters on Configured Collateral - Ignore it', async () => {
      ...
      const newGasComp = DCHF_GAS_COMPENSATION_SAFETY_MAX
    ...
    it('setDCHFGasCompensation: Owner change parameter - Failing SafeCheck', async () => {
      ...
      await assertRevert(
        dfrancParameters.setDCHFGasCompensation(ZERO_ADDRESS, DCHF_GAS_COMPENSATION_SAFETY_MIN.sub(toBN(1)))
      )
      await assertRevert(
        dfrancParameters.setDCHFGasCompensation(ZERO_ADDRESS, DCHF_GAS_COMPENSATION_SAFETY_MAX.add(toBN(1)))
      )
    })
    ...
    it('setDCHFGasCompensation: Owner change parameter - Valid SafeCheck', async () => {
      ...
      await dfrancParameters.setDCHFGasCompensation(ZERO_ADDRESS, DCHF_GAS_COMPENSATION_SAFETY_MIN)
      assert.equal(
        DCHF_GAS_COMPENSATION_SAFETY_MIN.toString(),
        await dfrancParameters.DCHF_GAS_COMPENSATION(ZERO_ADDRESS)
      )

      await dfrancParameters.setDCHFGasCompensation(ZERO_ADDRESS, DCHF_GAS_COMPENSATION_SAFETY_MAX)
      assert.equal(
        DCHF_GAS_COMPENSATION_SAFETY_MAX.toString(),
        await dfrancParameters.DCHF_GAS_COMPENSATION(ZERO_ADDRESS)
      )
    })
    ...
    it('setCollateralParameters: Owner change parameter - Failing SafeCheck', async () => {
      ...
      await assertRevert(
        dfrancParameters.setCollateralParameters(
          ...
          MCR,
          CCR,
          DCHF_GAS_COMPENSATION_SAFETY_MAX.add(toBN(1)),
          MIN_NET_DEBT,
          PERCENT_DIVISOR,
          BORROWING_FEE_FLOOR,
          MAX_BORROWING_FEE,
