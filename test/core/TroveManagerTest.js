...
contract('TroveManager', async (accounts) => {
  ...
  const withdrawDCHF = async (params) => th.withdrawDCHF(contracts, params)
  ...
  beforeEach(async () => {
    ...
    await monToken.unprotectedMint(multisig, dec(1, 24))
  ...
  it('liquidate(): closes a Trove that has ICR < MCR', async () => {
    ...
    const ICR_Before = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, alice, price)
    assert.equal(ICR_Before, dec(4, 18))

    const ICR_Before_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)
    ...
    await withdrawDCHF({ ICR: targetICR, extraParams: { from: alice } })
    await withdrawDCHF({ asset: erc20.address, ICR: targetICR, extraParams: { from: alice } })

    const ICR_AfterWithdrawal = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, alice, price)
    const ICR_AfterWithdrawal_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)
    ...
    // price drops to 1ETH:100DCHF, reducing Alice's ICR below MCR
    ...
    const status = (await troveManagerHelpers.Troves(alice, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX]
    const status_Asset = (await troveManagerHelpers.Troves(alice, erc20.address))[th.TROVE_STATUS_INDEX]
  ...
  it('liquidate(): decreases ActivePool ETH and DCHFDebt by correct amounts', async () => {
    // --- SETUP ---
    ...
    const { collateral: A_collateral_Asset, totalDebt: A_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(4, 18)),
      extraParams: { from: alice },
    })
    const { collateral: B_collateral_Asset, totalDebt: B_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(21, 17)),
      extraParams: { from: bob },
    })
    ...
    // check ActivePool ETH and DCHF debt before
    ...
    const activePooL_DCHFDebt_Before = (await activePool.getDCHFDebt(ZERO_ADDRESS)).toString()
    ...
    const activePooL_DCHFDebt_Before_Asset = (await activePool.getDCHFDebt(erc20.address)).toString()
    ...
    th.assertIsApproximatelyEqual(activePooL_DCHFDebt_Before, A_totalDebt.add(B_totalDebt))

    assert.equal(activePool_ETH_Before_Asset, A_collateral_Asset.add(B_collateral_Asset))
    assert.equal(
      activePool_RawEther_Before_Asset,
      A_collateral_Asset.add(B_collateral_Asset).div(toBN(10 ** 10))
    )
    th.assertIsApproximatelyEqual(activePooL_DCHFDebt_Before_Asset, A_totalDebt_Asset.add(B_totalDebt_Asset))

    // price drops to 1ETH:100DCHF, reducing Bob's ICR below MCR
    await priceFeed.setPrice('100000000000000000000')
    ...
    /* close Bob's Trove. Should liquidate his ether and DCHF,
    leaving Alice’s ether and DCHF debt in the ActivePool. */
    await troveManager.liquidate(ZERO_ADDRESS, bob, { from: owner })
    await troveManager.liquidate(erc20.address, bob, { from: owner })

    // check ActivePool ETH and DCHF debt
    ...
    const activePooL_DCHFDebt_After = (await activePool.getDCHFDebt(ZERO_ADDRESS)).toString()
    ...
    const activePooL_DCHFDebt_After_Asset = (await activePool.getDCHFDebt(erc20.address)).toString()

    assert.equal(activePool_ETH_After, A_collateral)
    assert.equal(activePool_RawEther_After, A_collateral)
    th.assertIsApproximatelyEqual(activePooL_DCHFDebt_After, A_totalDebt)
    ...
    th.assertIsApproximatelyEqual(activePooL_DCHFDebt_After_Asset, A_totalDebt_Asset)
  })

  it('liquidate(): increases DefaultPool ETH and DCHF debt by correct amounts', async () => {
    // --- SETUP ---
    await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: alice } })
    const { collateral: B_collateral, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(21, 17)),
    ...
    // check DefaultPool ETH and DCHF debt before
    ...
    const defaultPooL_DCHFDebt_Before = (await defaultPool.getDCHFDebt(ZERO_ADDRESS)).toString()
    ...
    const defaultPooL_DCHFDebt_Before_Asset = (await defaultPool.getDCHFDebt(erc20.address)).toString()

    assert.equal(defaultPool_ETH_Before, '0')
    assert.equal(defaultPool_RawEther_Before, '0')
    assert.equal(defaultPooL_DCHFDebt_Before, '0')

    assert.equal(defaultPool_ETH_Before_Asset, '0')
    assert.equal(defaultPool_RawEther_Before_Asset, '0')
    assert.equal(defaultPooL_DCHFDebt_Before_Asset, '0')
    ...
    const defaultPooL_DCHFDebt_After = (await defaultPool.getDCHFDebt(ZERO_ADDRESS)).toString()
    ...
    const defaultPooL_DCHFDebt_After_Asset = (await defaultPool.getDCHFDebt(erc20.address)).toString()
    ...
    th.assertIsApproximatelyEqual(defaultPooL_DCHFDebt_After, B_totalDebt)
    ...
    th.assertIsApproximatelyEqual(defaultPooL_DCHFDebt_After_Asset, B_totalDebt_Asset)
  })

  it("liquidate(): removes the Trove's stake from the total stakes", async () => {
    // --- SETUP ---
    const { collateral: A_collateral } = await openTrove({
      ICR: toBN(dec(4, 18)),
      extraParams: { from: alice },
    })
    const { collateral: B_collateral } = await openTrove({
      ICR: toBN(dec(21, 17)),
      extraParams: { from: bob },
    })

    const { collateral: A_collateral_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(4, 18)),
      extraParams: { from: alice },
    })
    const { collateral: B_collateral_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(21, 17)),
      extraParams: { from: bob },
    })
    ...
    const totalStakes_Before = (await troveManagerHelpers.totalStakes(ZERO_ADDRESS)).toString()
    const totalStakes_Before_Asset = (await troveManagerHelpers.totalStakes(erc20.address)).toString()
    ...
    const totalStakes_After = (await troveManagerHelpers.totalStakes(ZERO_ADDRESS)).toString()
    const totalStakes_After_Asset = (await troveManagerHelpers.totalStakes(erc20.address)).toString()
  ...
  it('liquidate(): Removes the correct trove from the TroveOwners array, and moves the last array element to the new empty slot', async () => {
    ...
    const arrayLength_Before = await troveManagerHelpers.getTroveOwnersCount(ZERO_ADDRESS)
    const arrayLength_Before_Asset = await troveManagerHelpers.getTroveOwnersCount(erc20.address)
    ...
    const arrayLength_After = await troveManagerHelpers.getTroveOwnersCount(ZERO_ADDRESS)
    const arrayLength_After_Asset = await troveManagerHelpers.getTroveOwnersCount(erc20.address)
    ...
    const trove_0 = await troveManagerHelpers.TroveOwners(ZERO_ADDRESS, 0)
    const trove_1 = await troveManagerHelpers.TroveOwners(ZERO_ADDRESS, 1)
    const trove_2 = await troveManagerHelpers.TroveOwners(ZERO_ADDRESS, 2)
    const trove_3 = await troveManagerHelpers.TroveOwners(ZERO_ADDRESS, 3)
    const trove_4 = await troveManagerHelpers.TroveOwners(ZERO_ADDRESS, 4)

    const trove_0_Asset = await troveManagerHelpers.TroveOwners(erc20.address, 0)
    const trove_1_Asset = await troveManagerHelpers.TroveOwners(erc20.address, 1)
    const trove_2_Asset = await troveManagerHelpers.TroveOwners(erc20.address, 2)
    const trove_3_Asset = await troveManagerHelpers.TroveOwners(erc20.address, 3)
    const trove_4_Asset = await troveManagerHelpers.TroveOwners(erc20.address, 4)
    ...
    const whale_arrayIndex = (await troveManagerHelpers.Troves(whale, ZERO_ADDRESS))[th.TROVE_ARRAY_INDEX]
    const alice_arrayIndex = (await troveManagerHelpers.Troves(alice, ZERO_ADDRESS))[th.TROVE_ARRAY_INDEX]
    const bob_arrayIndex = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_ARRAY_INDEX]
    const dennis_arrayIndex = (await troveManagerHelpers.Troves(dennis, ZERO_ADDRESS))[th.TROVE_ARRAY_INDEX]
    const erin_arrayIndex = (await troveManagerHelpers.Troves(erin, ZERO_ADDRESS))[th.TROVE_ARRAY_INDEX]

    const whale_arrayIndex_Asset = (await troveManagerHelpers.Troves(whale, erc20.address))[
      th.TROVE_ARRAY_INDEX
    ]
    const alice_arrayIndex_Asset = (await troveManagerHelpers.Troves(alice, erc20.address))[
      th.TROVE_ARRAY_INDEX
    ]
    const bob_arrayIndex_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_ARRAY_INDEX]
    const dennis_arrayIndex_Asset = (await troveManagerHelpers.Troves(dennis, erc20.address))[
      th.TROVE_ARRAY_INDEX
    ]
    const erin_arrayIndex_Asset = (await troveManagerHelpers.Troves(erin, erc20.address))[
      th.TROVE_ARRAY_INDEX
    ]
  ...
  it('liquidate(): updates the snapshots of total stakes and total collateral', async () => {
    ...
    const totalStakesSnapshot_Before = (
      await troveManagerHelpers.totalStakesSnapshot(ZERO_ADDRESS)
    ).toString()
    const totalCollateralSnapshot_Before = (
      await troveManagerHelpers.totalCollateralSnapshot(ZERO_ADDRESS)
    ).toString()
    const totalStakesSnapshot_Before_Asset = (
      await troveManagerHelpers.totalStakesSnapshot(erc20.address)
    ).toString()
    const totalCollateralSnapshot_Before_Asset = (
      await troveManagerHelpers.totalCollateralSnapshot(erc20.address)
    ).toString()
    ...
    // close Bob's Trove.  His ether*0.995 and DCHF should be added to the DefaultPool.
    await troveManager.liquidate(ZERO_ADDRESS, bob, { from: owner })
    await troveManager.liquidate(erc20.address, bob, { from: owner })
    ...
    const totalStakesSnapshot_After = (await troveManagerHelpers.totalStakesSnapshot(ZERO_ADDRESS)).toString()
    const totalCollateralSnapshot_After = (
      await troveManagerHelpers.totalCollateralSnapshot(ZERO_ADDRESS)
    ).toString()
    const totalStakesSnapshot_After_Asset = (
      await troveManagerHelpers.totalStakesSnapshot(erc20.address)
    ).toString()
  ...
  it('liquidate(): updates the L_ETH and L_DCHFDebt reward-per-unit-staked totals', async () => {
    // --- SETUP ---
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(8, 18)),
      extraParams: { from: alice },
    ...
    const { collateral: C_collateral_Asset, totalDebt: C_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(111, 16)),
      extraParams: { from: carol },
    })
    ...
    // price drops to 1ETH:100DCHF, reducing Carols's ICR below MCR
    await priceFeed.setPrice('100000000000000000000')
    ...
    // Carol's ether*0.995 and DCHF should be added to the DefaultPool.
    const L_ETH_AfterCarolLiquidated = await troveManagerHelpers.L_ASSETS(ZERO_ADDRESS)
    const L_DCHFDebt_AfterCarolLiquidated = await troveManagerHelpers.L_DCHFDebts(ZERO_ADDRESS)
    const L_ETH_AfterCarolLiquidated_Asset = await troveManagerHelpers.L_ASSETS(erc20.address)
    const L_DCHFDebt_AfterCarolLiquidated_Asset = await troveManagerHelpers.L_DCHFDebts(erc20.address)

    const L_ETH_expected_1 = th
      .applyLiquidationFee(C_collateral)
      .mul(mv._1e18BN)
      .div(A_collateral.add(B_collateral))
    const L_DCHFDebt_expected_1 = C_totalDebt.mul(mv._1e18BN).div(A_collateral.add(B_collateral))
    const L_ETH_expected_1_Asset = th
      .applyLiquidationFee(C_collateral_Asset)
      .mul(mv._1e18BN)
      .div(A_collateral_Asset.add(B_collateral_Asset))
    const L_DCHFDebt_expected_1_Asset = C_totalDebt_Asset.mul(mv._1e18BN).div(
    ...
    assert.isAtMost(th.getDifference(L_DCHFDebt_AfterCarolLiquidated, L_DCHFDebt_expected_1), 100)
    assert.isAtMost(th.getDifference(L_ETH_AfterCarolLiquidated_Asset, L_ETH_expected_1_Asset), 100)
    assert.isAtMost(th.getDifference(L_DCHFDebt_AfterCarolLiquidated_Asset, L_DCHFDebt_expected_1_Asset), 100)

    // Bob now withdraws DCHF, bringing his ICR to 1.11
    const { increasedTotalDebt: B_increasedTotalDebt } = await withdrawDCHF({
      ICR: toBN(dec(111, 16)),
      extraParams: { from: bob },
    })
    const { increasedTotalDebt: B_increasedTotalDebt_Asset } = await withdrawDCHF({
      asset: erc20.address,
      ICR: toBN(dec(111, 16)),
      extraParams: { from: bob },
    })
    ...
    // price drops to 1ETH:50DCHF, reducing Bob's ICR below MCR
   ...
   L_DCHFDebt = (180 / 20) + (890 / 10) = 98 DCHF */
    const L_ETH_AfterBobLiquidated = await troveManagerHelpers.L_ASSETS(ZERO_ADDRESS)
    const L_DCHFDebt_AfterBobLiquidated = await troveManagerHelpers.L_DCHFDebts(ZERO_ADDRESS)
    ...
    const L_DCHFDebt_expected_2 = L_DCHFDebt_expected_1.add(
      B_totalDebt.add(B_increasedTotalDebt)
        .add(B_collateral.mul(L_DCHFDebt_expected_1).div(mv._1e18BN))
        .mul(mv._1e18BN)
        .div(A_collateral)
    )
    assert.isAtMost(th.getDifference(L_ETH_AfterBobLiquidated, L_ETH_expected_2), 100)
    assert.isAtMost(th.getDifference(L_DCHFDebt_AfterBobLiquidated, L_DCHFDebt_expected_2), 100)
  })
  ...
  it('liquidate(): Liquidates undercollateralized trove if there are two troves in the system', async () => {
    ...
    // Alice creates a single trove with 0.7 ETH and a debt of 70 DCHF, and provides 10 DCHF to SP
    await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
    await openTrove({ asset: erc20.address, ICR: toBN(dec(2, 18)), extraParams: { from: alice } })

    // Alice proves 10 DCHF to SP
    ...
    const alice_ICR = (await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, alice, price)).toString()
    const alice_ICR_Asset = (await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)).toString()
    assert.equal(alice_ICR, '1050000000000000000')
    assert.equal(alice_ICR_Asset, '1050000000000000000')

    const activeTrovesCount_Before = await troveManagerHelpers.getTroveOwnersCount(ZERO_ADDRESS)
    const activeTrovesCount_Before_Asset = await troveManagerHelpers.getTroveOwnersCount(erc20.address)
    ...
    const activeTrovesCount_After = await troveManagerHelpers.getTroveOwnersCount(ZERO_ADDRESS)
    const activeTrovesCount_After_Asset = await troveManagerHelpers.getTroveOwnersCount(erc20.address)
  ...
  it('liquidate(): reverts if trove is non-existent', async () => {
    ...
    assert.equal(await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, carol), 0) // check trove non-existent
    assert.equal(await troveManagerHelpers.getTroveStatus(erc20.address, carol), 0) // check trove non-existent
  ...
  it('liquidate(): reverts if trove has been closed', async () => {
    ...
    assert.equal(await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, carol), 3) // check trove closed by liquidation
    assert.equal(await troveManagerHelpers.getTroveStatus(erc20.address, carol), 3) // check trove closed by liquidation
  ...
  it('liquidate(): does nothing if trove has >= 110% ICR', async () => {
    ...
    // Check Bob's ICR > 110%
    const bob_ICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price)
    const bob_ICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)
    ...
    await assertRevert(troveManager.liquidate(ZERO_ADDRESS, bob), '0L')
    await assertRevert(troveManager.liquidate(erc20.address, bob), '0L')
  ...
  it('liquidate(): Given the same price and no other trove changes, complete Pool offsets restore the TCR to its value prior to the defaulters opening troves', async () => {
    // Whale provides DCHF to SP
    const spDeposit = toBN(dec(100, 24))
    await openTrove({ ICR: toBN(dec(4, 18)), extraDCHFAmount: spDeposit, extraParams: { from: whale } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(4, 18)),
      extraDCHFAmount: spDeposit,
  ...
  it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
    ...
    await openTrove({ ICR: toBN(dec(3, 18)), extraDCHFAmount: spDeposit, extraParams: { from: bob } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(3, 18)),
      extraDCHFAmount: spDeposit,
    ...
    await DCHFToken.transfer(dennis, spDeposit.mul(toBN(2)), { from: bob })

    //Dennis provides DCHF to SP
    ...
    const dennis_Deposit_Before = (await stabilityPool.getCompoundedDCHFDeposit(dennis)).toString()
    const dennis_ETHGain_Before = (await stabilityPool.getDepositorAssetGain(dennis)).toString()
    const dennis_Deposit_Before_Asset = (await stabilityPoolERC20.getCompoundedDCHFDeposit(dennis)).toString()
    ...
    assert.isAtMost(
      th.getDifference(dennis_Deposit_Before_Asset, spDeposit.sub(liquidatedDebt_Asset)),
      1000000
    )
    assert.isAtMost(
      th.getDifference(dennis_ETHGain_Before_Asset, liquidatedColl_Asset.div(toBN(10 ** 10))),
      1000
    )
    ...
    const dennis_Deposit_After = (await stabilityPool.getCompoundedDCHFDeposit(dennis)).toString()
    const dennis_ETHGain_After = (await stabilityPool.getDepositorAssetGain(dennis)).toString()

    const dennis_Deposit_After_Asset = (await stabilityPoolERC20.getCompoundedDCHFDeposit(dennis)).toString()
  ...
  it("liquidate(): does not liquidate a SP depositor's trove with ICR > 110%, and does not affect their SP deposit or ETH gain", async () => {
    ...
    await priceFeed.setPrice(dec(200, 18))
    const price = await priceFeed.getPrice()
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price)).gt(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)).gt(mv._MCR))
    ...
    const bob_Deposit_Before_Asset = (await stabilityPoolERC20.getCompoundedDCHFDeposit(bob)).toString()
    ...
    assert.isAtMost(
      th.getDifference(bob_ETHGain_Before_Asset, liquidatedColl_Asset.div(toBN(10 ** 10))),
      1000
    )
    ...
    const bob_Deposit_After_Asset = (await stabilityPoolERC20.getCompoundedDCHFDeposit(bob)).toString()
  ...
  it("liquidate(): liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and ETH gain", async () => {
    ...
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    await openTrove({ ICR: toBN(dec(8, 18)), extraDCHFAmount: A_spDeposit, extraParams: { from: alice } })

    await openTrove({ asset: erc20.address, ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(8, 18)),
      extraDCHFAmount: A_spDeposit,
      extraParams: { from: alice },
    })

    const { collateral: B_collateral, totalDebt: B_debt } = await openTrove({
      ICR: toBN(dec(218, 16)),
      extraDCHFAmount: B_spDeposit,
      extraParams: { from: bob },
    })
    ...
    const { collateral: B_collateral_Asset, totalDebt: B_debt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(218, 16)),
      extraDCHFAmount: B_spDeposit,
      extraParams: { from: bob },
    })
    const { collateral: C_collateral_Asset, totalDebt: C_debt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(210, 16)),
      extraDCHFAmount: toBN(dec(100, 18)),
      extraParams: { from: carol },
    ...
    const bob_Deposit_Before_Asset = await stabilityPoolERC20.getCompoundedDCHFDeposit(bob)
    ...
    assert.isAtMost(
      th.getDifference(
        bob_ETHGain_Before_Asset,
        th.applyLiquidationFee(C_collateral_Asset).div(toBN(10 ** 10))
      ),
      1000
    )

    // Alice provides DCHF to SP
    ...
    assert.isFalse(await sortedTroves.contains(ZERO_ADDRESS, bob))
    assert.isFalse(await sortedTroves.contains(erc20.address, bob))
    const bob_Trove_Status = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[
      th.TROVE_STATUS_INDEX
    ].toString()
    const bob_Trove_Status_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[
      th.TROVE_STATUS_INDEX
    ].toString()
    assert.equal(bob_Trove_Status, 3)
    assert.equal(bob_Trove_Status_Asset, 3)

    /* Alice's DCHF Loss = (300 / 400) * 200 = 150 DCHF
       Alice's ETH gain = (300 / 400) * 2*0.995 = 1.4925 ETH

       Bob's DCHFLoss = (100 / 400) * 200 = 50 DCHF
       Bob's ETH gain = (100 / 400) * 2*0.995 = 0.4975 ETH

     Check Bob' SP deposit has been reduced to 50 DCHF, and his ETH gain has increased to 1.5 ETH. */
    const alice_Deposit_After = (await stabilityPool.getCompoundedDCHFDeposit(alice)).toString()
    const alice_ETHGain_After = (await stabilityPool.getDepositorAssetGain(alice)).toString()

    const alice_Deposit_After_Asset = (await stabilityPoolERC20.getCompoundedDCHFDeposit(alice)).toString()
    ...
    assert.isAtMost(
      th.getDifference(
        ...
        th
          .applyLiquidationFee(B_collateral_Asset)
          .mul(A_spDeposit)
          .div(totalDeposits_Asset)
          .div(toBN(10 ** 10))
      ),
      1000000
    )

    const bob_Deposit_After = await stabilityPool.getCompoundedDCHFDeposit(bob)
    const bob_ETHGain_After = await stabilityPool.getDepositorAssetGain(bob)

    const bob_Deposit_After_Asset = await stabilityPoolERC20.getCompoundedDCHFDeposit(bob)
    ...
    assert.isAtMost(
      th.getDifference(
        ...
        bob_Deposit_Before_Asset.sub(B_debt.mul(bob_Deposit_Before_Asset).div(totalDeposits_Asset))
      ),
      1000000
    )
    assert.isAtMost(
      th.getDifference(
        ...
        bob_ETHGain_Before_Asset.add(
          th
            .applyLiquidationFee(B_collateral_Asset.div(toBN(10 ** 10)))
            .mul(bob_Deposit_Before_Asset)
            .div(totalDeposits_Asset)
        )
      ),
      1000000
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    const { DCHFAmount: A_DCHFAmount } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraDCHFAmount: toBN(dec(300, 18)),
      extraParams: { from: alice },
    })
    const { DCHFAmount: B_DCHFAmount } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraDCHFAmount: toBN(dec(200, 18)),
      extraParams: { from: bob },
    })
    const { DCHFAmount: C_DCHFAmount } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraDCHFAmount: toBN(dec(100, 18)),
      extraParams: { from: carol },
    })
    ...
    await openTrove({ asset: erc20.address, ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    const { DCHFAmount: A_DCHFAmount_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(2, 18)),
      extraDCHFAmount: toBN(dec(300, 18)),
      extraParams: { from: alice },
    })
    const { DCHFAmount: B_DCHFAmount_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(2, 18)),
      extraDCHFAmount: toBN(dec(200, 18)),
      extraParams: { from: bob },
    })
    const { DCHFAmount: C_DCHFAmount_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(2, 18)),
      extraDCHFAmount: toBN(dec(100, 18)),
      extraParams: { from: carol },
    ...
    // Liquidate A, B and C
    await activePool.getDCHFDebt(ZERO_ADDRESS)
    await defaultPool.getDCHFDebt(ZERO_ADDRESS)
    await activePool.getDCHFDebt(erc20.address)
    await defaultPool.getDCHFDebt(erc20.address)
  ...
  it('liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt', async () => {
    ...
    // Defaulter opens with 60 DCHF, 0.6 ETH
    ...
    const alice_ICR_Before = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, alice, price)
    const bob_ICR_Before = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price)
    const carol_ICR_Before = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)

    const alice_ICR_Before_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)
    const bob_ICR_Before_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)
    const carol_ICR_Before_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)
    ...
    /* Liquidate defaulter. 30 DCHF and 0.3 ETH is distributed between A, B and C.

    A receives (30 * 2/4) = 15 DCHF, and (0.3*2/4) = 0.15 ETH
    B receives (30 * 1/4) = 7.5 DCHF, and (0.3*1/4) = 0.075 ETH
    C receives (30 * 1/4) = 7.5 DCHF, and (0.3*1/4) = 0.075 ETH
    */
    await troveManager.liquidate(ZERO_ADDRESS, defaulter_1)
    await troveManager.liquidate(erc20.address, defaulter_1)

    const alice_ICR_After = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, alice, price)
    const bob_ICR_After = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price)
    const carol_ICR_After = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)

    const alice_ICR_After_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)
    const bob_ICR_After_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)
    const carol_ICR_After_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)
    ...
    const bob_Coll = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
    const bob_Debt = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]
    const bob_Coll_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_COLL_INDEX]
    const bob_Debt_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_DEBT_INDEX]
   ...
   (because his pending rewards bring his ICR < MCR) */
    ...
    assert.equal(
      (await troveManagerHelpers.Troves(alice, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX].toString(),
      '1'
    )
    assert.equal((await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX].toString(), '3')
    assert.equal(
      (await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX].toString(),
      '3'
    )

    assert.equal(
      (await troveManagerHelpers.Troves(alice, erc20.address))[th.TROVE_STATUS_INDEX].toString(),
      '1'
    )
    assert.equal(
      (await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_STATUS_INDEX].toString(),
      '3'
    )
    assert.equal(
      (await troveManagerHelpers.Troves(carol, erc20.address))[th.TROVE_STATUS_INDEX].toString(),
      '3'
    )
  })

  it('liquidate(): when SP > 0, triggers MON reward event - increases the sum G', async () => {
    ...
    assert.equal(await stabilityPool.getTotalDCHFDeposits(), dec(100, 18))

    await stabilityPoolERC20.provideToSP(dec(100, 18), { from: B })
    assert.equal(await stabilityPoolERC20.getTotalDCHFDeposits(), dec(100, 18))
    ...
    // Price drops to 1ETH:100DCHF, reducing defaulters to below MCR
    await priceFeed.setPrice(dec(100, 18))
    await priceFeed.getPrice()
    assert.isFalse(await th.checkRecoveryMode(contracts))
    assert.isFalse(await th.checkRecoveryMode(contracts, erc20.address))
  ...
  it('liquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards', async () => {
    ...
    // A adds 10 DCHF to the SP, but less than C's debt
    ...
    const TCR = await troveManagerHelpers.getTCR(ZERO_ADDRESS, price)
    const ICR_C = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, C, price)
    const TCR_Asset = await troveManagerHelpers.getTCR(erc20.address, price)
    const ICR_C_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, C, price)
    ...
    await borrowerOperations.repayDCHF(ZERO_ADDRESS, dec(1, 18), D, D, { from: D })
    await borrowerOperations.repayDCHF(ZERO_ADDRESS, dec(1, 18), E, E, { from: E })
    await borrowerOperations.repayDCHF(erc20.address, dec(1, 18), D, D, { from: D })
    await borrowerOperations.repayDCHF(erc20.address, dec(1, 18), E, E, { from: E })

    // Check C is the only trove that has pending rewards
    assert.isTrue(await troveManagerHelpers.hasPendingRewards(ZERO_ADDRESS, C))
    assert.isFalse(await troveManagerHelpers.hasPendingRewards(ZERO_ADDRESS, D))
    assert.isFalse(await troveManagerHelpers.hasPendingRewards(ZERO_ADDRESS, E))

    assert.isTrue(await troveManagerHelpers.hasPendingRewards(erc20.address, C))
    assert.isFalse(await troveManagerHelpers.hasPendingRewards(erc20.address, D))
    assert.isFalse(await troveManagerHelpers.hasPendingRewards(erc20.address, E))

    // Check C's pending coll and debt rewards are <= the coll and debt in the DefaultPool
    const pendingETH_C = await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, C)
    const pendingDCHFDebt_C = await troveManagerHelpers.getPendingDCHFDebtReward(ZERO_ADDRESS, C)
    const defaultPoolETH = await defaultPool.getAssetBalance(ZERO_ADDRESS)
    const defaultPoolDCHFDebt = await defaultPool.getDCHFDebt(ZERO_ADDRESS)

    const pendingETH_C_Asset = await troveManagerHelpers.getPendingAssetReward(erc20.address, C)
    const pendingDCHFDebt_C_Asset = await troveManagerHelpers.getPendingDCHFDebtReward(erc20.address, C)
    const defaultPoolETH_Asset = await defaultPool.getAssetBalance(erc20.address)
    const defaultPoolDCHFDebt_Asset = await defaultPool.getDCHFDebt(erc20.address)

    assert.isTrue(pendingETH_C.lte(defaultPoolETH))
    assert.isTrue(pendingDCHFDebt_C.lte(defaultPoolDCHFDebt))

    assert.isTrue(pendingETH_C_Asset.lte(defaultPoolETH_Asset))
    assert.isTrue(pendingDCHFDebt_C_Asset.lte(defaultPoolDCHFDebt_Asset))
    //Check only difference is dust
    assert.isAtMost(th.getDifference(pendingETH_C, defaultPoolETH), 1000)
    assert.isAtMost(th.getDifference(pendingDCHFDebt_C, defaultPoolDCHFDebt), 1000)

    assert.isAtMost(th.getDifference(pendingETH_C_Asset, defaultPoolETH_Asset), 1000)
    assert.isAtMost(th.getDifference(pendingDCHFDebt_C_Asset, defaultPoolDCHFDebt_Asset), 1000)
  ...
  it('liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves', async () => {
    ...
    // Price drops to 1ETH:100DCHF, reducing Bob and Carol's ICR below MCR
    await priceFeed.setPrice(dec(100, 18))
    const price = await priceFeed.getPrice()
    ...
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, alice, price)).lte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price)).lte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)).lte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, erin, price)).lte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, flyn, price)).lte(mv._MCR))

    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)).lte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)).lte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)).lte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, erin, price)).lte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, flyn, price)).lte(mv._MCR))

    // Confirm troves G, H, I are ICR > 110%
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, graham, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, harriet, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, ida, price)).gte(mv._MCR))

    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, graham, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, harriet, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, ida, price)).gte(mv._MCR))

    // Confirm Whale is ICR > 110%
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, whale, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, whale, price)).gte(mv._MCR))
    ...
    assert.equal(
      (await troveManagerHelpers.Troves(erin, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX].toString(),
      '3'
    )
    assert.equal(
      (await troveManagerHelpers.Troves(flyn, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX].toString(),
      '3'
    )
    ...
    assert.equal(
      (await troveManagerHelpers.Troves(erin, erc20.address))[th.TROVE_STATUS_INDEX].toString(),
      '3'
    )
    assert.equal(
      (await troveManagerHelpers.Troves(flyn, erc20.address))[th.TROVE_STATUS_INDEX].toString(),
      '3'
    )
  ...
  it('liquidateTroves(): liquidates  up to the requested number of undercollateralized troves', async () => {
    ...
    const TroveOwnersArrayLength = await troveManagerHelpers.getTroveOwnersCount(ZERO_ADDRESS)
    const TroveOwnersArrayLength_Asset = await troveManagerHelpers.getTroveOwnersCount(erc20.address)
    ...
    const aliceTroveStatus = (await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, alice)).toString()
    const bobTroveStatus = (await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, bob)).toString()
    const carolTroveStatus = (await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, carol)).toString()

    const aliceTroveStatus_Asset = (await troveManagerHelpers.getTroveStatus(erc20.address, alice)).toString()
    const bobTroveStatus_Asset = (await troveManagerHelpers.getTroveStatus(erc20.address, bob)).toString()
    const carolTroveStatus_Asset = (await troveManagerHelpers.getTroveStatus(erc20.address, carol)).toString()
    ...
    const dennisTroveStatus = (await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, dennis)).toString()
    const erinTroveStatus = (await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, erin)).toString()

    const dennisTroveStatus_Asset = (
      await troveManagerHelpers.getTroveStatus(erc20.address, dennis)
    ).toString()
    const erinTroveStatus_Asset = (await troveManagerHelpers.getTroveStatus(erc20.address, erin)).toString()
  ...
  it('liquidateTroves(): does nothing if all troves have ICR > 110%', async () => {
    ...
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, whale, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, alice, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)).gte(mv._MCR))

    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, whale, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)).gte(mv._MCR))
  ...
  it('liquidateTroves(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt', async () => {
    ...
    // Liquidate defaulter. 30 DCHF and 0.3 ETH is distributed uniformly between A, B and C. Each receive 10 DCHF, 0.1 ETH
    await troveManager.liquidate(ZERO_ADDRESS, defaulter_1)
    await troveManager.liquidate(erc20.address, defaulter_1)
    ...
    // Whale enters system, pulling it into Normal Mode
    await openTrove({ ICR: toBN(dec(10, 18)), extraDCHFAmount: dec(1, 24), extraParams: { from: whale } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(10, 18)),
      extraDCHFAmount: dec(1, 24),
      extraParams: { from: whale },
    })
  ...
  it('liquidateTroves(): reverts if n = 0', async () => {
    ...
    const alice_ICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, alice, price)
    const bob_ICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price)
    const carol_ICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)

    const alice_ICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)
    const bob_ICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)
    const carol_ICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)
  ...
  it('liquidateTroves():  liquidates troves with ICR < MCR', async () => {
    ...
    const bob_ICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price)
    const carol_ICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)
    const dennis_ICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, dennis, price)
    const erin_ICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, erin, price)
    const flyn_ICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, flyn, price)

    const alice_ICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)
    const bob_ICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)
    const carol_ICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)
    const dennis_ICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, dennis, price)
    const erin_ICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, erin, price)
    const flyn_ICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, flyn, price)
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const D_balanceBefore = await DCHFToken.balanceOf(dennis)
    const E_balanceBefore = await DCHFToken.balanceOf(erin)
    const F_balanceBefore = await DCHFToken.balanceOf(flyn)
    ...
    assert.equal((await DCHFToken.balanceOf(dennis)).toString(), D_balanceBefore)
    assert.equal((await DCHFToken.balanceOf(erin)).toString(), E_balanceBefore)
    assert.equal((await DCHFToken.balanceOf(flyn)).toString(), F_balanceBefore)
  })
  ...
  it('liquidateTroves(): A liquidation sequence containing Pool offsets increases the TCR', async () => {
    ...
    await openTrove({
      ICR: toBN(dec(100, 18)),
      extraDCHFAmount: toBN(dec(500, 18)),
      extraParams: { from: whale },
    })
    await openTrove({
      asset: erc20.address,
    ...
    // Check pool has 500 DCHF
    assert.equal((await stabilityPool.getTotalDCHFDeposits()).toString(), dec(500, 18))
    assert.equal((await stabilityPoolERC20.getTotalDCHFDeposits()).toString(), dec(500, 18))
    ...
    assert.equal((await stabilityPool.getTotalDCHFDeposits()).toString(), '0')
    assert.equal((await stabilityPoolERC20.getTotalDCHFDeposits()).toString(), '0')
  ...
  it('liquidateTroves(): A liquidation sequence of pure redistributions decreases the TCR, due to gas compensation, but up to 0.5%', async () => {
    ...
    const { collateral: W_coll_Asset, totalDebt: W_debt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(100, 18)),
      extraParams: { from: whale },
    })
    const { collateral: A_coll_Asset, totalDebt: A_debt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(4, 18)),
      extraParams: { from: alice },
    })
    const { collateral: B_coll_Asset, totalDebt: B_debt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(28, 18)),
      extraParams: { from: bob },
    })
    const { collateral: C_coll_Asset, totalDebt: C_debt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(8, 18)),
      extraParams: { from: carol },
    })
    const { collateral: D_coll_Asset, totalDebt: D_debt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(80, 18)),
      extraParams: { from: dennis },
    })

    const { collateral: d1_coll_Asset, totalDebt: d1_debt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(199, 16)),
      extraParams: { from: defaulter_1 },
    })
    const { collateral: d2_coll_Asset, totalDebt: d2_debt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(156, 16)),
      extraParams: { from: defaulter_2 },
    })
    const { collateral: d3_coll_Asset, totalDebt: d3_debt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(183, 16)),
      extraParams: { from: defaulter_3 },
    })
    const { collateral: d4_coll_Asset, totalDebt: d4_debt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(166, 16)),
      extraParams: { from: defaulter_4 },
    })
  ...
  it('liquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and ETH gain', async () => {
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraDCHFAmount: whaleDeposit, extraParams: { from: whale } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(100, 18)),
      extraDCHFAmount: whaleDeposit,
      extraParams: { from: whale },
    })
    ...
    const { collateral: A_coll, totalDebt: A_debt } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraDCHFAmount: A_deposit,
      extraParams: { from: alice },
    })
    const { collateral: B_coll, totalDebt: B_debt } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraDCHFAmount: B_deposit,
      extraParams: { from: bob },
    })
    const { collateral: C_coll, totalDebt: C_debt } = await openTrove({
      ICR: toBN(dec(2, 18)),
    ...
    assert.equal((await stabilityPool.getTotalDCHFDeposits()).toString(), totalDeposits)
    assert.equal((await stabilityPoolERC20.getTotalDCHFDeposits()).toString(), totalDeposits)
    ...
    Total DCHF in Pool: 800 DCHF

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 150 + 350 + 150 = 650 DCHF
    Total liquidated ETH = 1.1 + 3.1 + 1.1 = 5.3 ETH

    whale DCHF loss: 650 * (400/800) = 325 DCHF
    alice DCHF loss:  650 *(100/800) = 81.25 DCHF
    bob DCHF loss: 650 * (300/800) = 243.75 DCHF

    whale remaining deposit: (400 - 325) = 75 DCHF
    alice remaining deposit: (100 - 81.25) = 18.75 DCHF
    bob remaining deposit: (300 - 243.75) = 56.25 DCHF
    ...
    Total remaining deposits: 150 DCHF
    Total ETH gain: 4.975 ETH */

    // Check remaining DCHF Deposits and ETH gain, for whale and depositors whose troves were liquidated
    const whale_Deposit_After = await stabilityPool.getCompoundedDCHFDeposit(whale)
    const alice_Deposit_After = await stabilityPool.getCompoundedDCHFDeposit(alice)
    const bob_Deposit_After = await stabilityPool.getCompoundedDCHFDeposit(bob)
    ...
    const whale_Deposit_After_Asset = await stabilityPoolERC20.getCompoundedDCHFDeposit(whale)
    const alice_Deposit_After_Asset = await stabilityPoolERC20.getCompoundedDCHFDeposit(alice)
    const bob_Deposit_After_Asset = await stabilityPoolERC20.getCompoundedDCHFDeposit(bob)
    ...
    assert.isAtMost(
      th.getDifference(
        whale_ETHGain_Asset,
        th
          .applyLiquidationFee(liquidatedColl_Asset)
          .mul(whaleDeposit)
          .div(totalDeposits_Asset)
          .div(toBN(10 ** 10))
      ),
    ...
    const total_DCHFinSP_Asset = (await stabilityPoolERC20.getTotalDCHFDeposits()).toString()
    const total_ETHinSP_Asset = (await stabilityPoolERC20.getAssetBalance()).toString()

    assert.isAtMost(th.getDifference(total_DCHFinSP, totalDeposits.sub(liquidatedDebt)), 1000)
    assert.isAtMost(th.getDifference(total_ETHinSP, th.applyLiquidationFee(liquidatedColl)), 1000)
    assert.isAtMost(
      th.getDifference(total_DCHFinSP_Asset, totalDeposits_Asset.sub(liquidatedDebt_Asset)),
      1000
    )
    assert.isAtMost(th.getDifference(total_ETHinSP_Asset, th.applyLiquidationFee(liquidatedColl_Asset)), 1000)
  })

  it('liquidateTroves(): when SP > 0, triggers MON reward event - increases the sum G', async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } })
    await openTrove({ asset: erc20.address, ICR: toBN(dec(100, 18)), extraParams: { from: whale } })

    // A, B, C open troves
    await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraDCHFAmount: toBN(dec(100, 18)), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraParams: { from: C } })
  ...
  it('batchLiquidateTroves(): closes every trove with ICR < MCR in the given array', async () => {
    ...
    // Price drops to 1ETH:100DCHF, reducing A, B, C ICR below MCR
    await priceFeed.setPrice(dec(100, 18))
    const price = await priceFeed.getPrice()
    ...
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, alice, price)).lt(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price)).lt(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)).lt(mv._MCR))

    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)).lt(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)).lt(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)).lt(mv._MCR))

    // Confirm D-E are ICR > 110%
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, dennis, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, erin, price)).gte(mv._MCR))

    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, dennis, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, erin, price)).gte(mv._MCR))
  ...
  it('batchLiquidateTroves(): does not liquidate troves that are not in the given array', async () => {
    ...
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price)).lt(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)).lt(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, dennis, price)).lt(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, erin, price)).lt(mv._MCR))

    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)).lt(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)).lt(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)).lt(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, dennis, price)).lt(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, erin, price)).lt(mv._MCR))
    ...
    assert.equal(
      (await troveManagerHelpers.Troves(dennis, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX].toString(),
      '1'
    )
    assert.equal(
      (await troveManagerHelpers.Troves(erin, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX].toString(),
    ...
    assert.equal(
      (await troveManagerHelpers.Troves(dennis, erc20.address))[th.TROVE_STATUS_INDEX].toString(),
      '1'
    )
    assert.equal(
      (await troveManagerHelpers.Troves(erin, erc20.address))[th.TROVE_STATUS_INDEX].toString(),
  ...
  it('batchLiquidateTroves(): reverts if array is empty', async () => {
    ...
    try {
      ...
      assert.include(error.message, 'CA')
    }
    try {
  ...
  it('batchLiquidateTroves(): skips if trove is non-existent', async () => {
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraDCHFAmount: spDeposit, extraParams: { from: whale } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(100, 18)),
      extraDCHFAmount: spDeposit,
    ...
    const { totalDebt: A_debt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(190, 16)),
      extraParams: { from: alice },
    })
    const { totalDebt: B_debt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(120, 16)),
      extraParams: { from: bob },
    })
    ...
    th.assertIsApproximatelyEqual(
      (await stabilityPool.getTotalDCHFDeposits()).toString(),
      spDeposit.sub(A_debt).sub(B_debt)
    )
    th.assertIsApproximatelyEqual(
      (await stabilityPoolERC20.getTotalDCHFDeposits()).toString(),
  ...
  it('batchLiquidateTroves(): skips if a trove has been closed', async () => {
    ...
    await DCHFToken.transfer(carol, dec(200, 18), { from: whale })
    ...
    assert.equal(await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, carol), 2) // check trove closed
    assert.equal(await troveManagerHelpers.getTroveStatus(erc20.address, carol), 2) // check trove closed
  ...
  it('batchLiquidateTroves: when SP > 0, triggers MON reward event - increases the sum G', async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } })
    await openTrove({ asset: erc20.address, ICR: toBN(dec(100, 18)), extraParams: { from: whale } })
  ...
  it('getRedemptionHints(): gets the address of the first Trove and the final ICR of the last Trove involved in a redemption', async () => {
    ...
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(310, 16)),
      extraDCHFAmount: partialRedemptionAmount,
      extraParams: { from: alice },
    })
    ...
    const { netDebt: B_debt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(290, 16)),
      extraParams: { from: bob },
    })
    const { netDebt: C_debt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(250, 16)),
      extraParams: { from: carol },
    })
    ...
    const expectedICR_Asset = A_coll_Asset.mul(price)
      .sub(partialRedemptionAmount.mul(mv._1e18BN))
      .div(A_totalDebt_Asset.sub(partialRedemptionAmount))
  ...
  it('getRedemptionHints(): returns 0 as partialRedemptionHintNICR when reaching _maxIterations', async () => {
    ...
    // Get hints for a redemption of 170 + 30 + some extra DCHF. At least 3 iterations are needed
    ...
    const { 1: partialRedemptionHintNICR_Asset } = await hintHelpers.getRedemptionHints(
      erc20.address,
      '210' + _18_zeros,
      price,
      2
  ...
  it('redeemCollateral(): cancels the provided DCHF with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(310, 16)),
      extraDCHFAmount: dec(10, 18),
      extraParams: { from: alice },
    })
    const { netDebt: B_netDebt } = await openTrove({
      ICR: toBN(dec(290, 16)),
      extraDCHFAmount: dec(8, 18),
      extraParams: { from: bob },
    })
    ...
    const { totalDebt: A_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(310, 16)),
      extraDCHFAmount: dec(10, 18),
      extraParams: { from: alice },
    })
    const { netDebt: B_netDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(290, 16)),
      extraDCHFAmount: dec(8, 18),
      extraParams: { from: bob },
    })
    const { netDebt: C_netDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(250, 16)),
      extraDCHFAmount: dec(10, 18),
      extraParams: { from: carol },
    ...
    await openTrove({
      ICR: toBN(dec(100, 18)),
      extraDCHFAmount: redemptionAmount,
      extraParams: { from: dennis },
    })
    await openTrove({
      asset: erc20.address,
    ...
    const dennis_DCHFBalance_Before = await DCHFToken.balanceOf(dennis)
    ...
    // Find hints for redeeming 20 DCHF
    const { firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
      ZERO_ADDRESS,
      redemptionAmount,
      price,
    ...
    const { 0: upperPartialRedemptionHint, 1: lowerPartialRedemptionHint } =
      await sortedTroves.findInsertPosition(ZERO_ADDRESS, partialRedemptionHintNICR, dennis, dennis)

    const { 0: upperPartialRedemptionHint_Asset, 1: lowerPartialRedemptionHint_Asset } =
      await sortedTroves.findInsertPosition(erc20.address, partialRedemptionHintNICR_Asset, dennis, dennis)

    // skip bootstrapping phase
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_WEEK * 2, web3.currentProvider)

    // Dennis redeems 20 DCHF
    ...
    const redemptionTx_Asset = await troveManager.redeemCollateral(
      erc20.address,
    ...
    const alice_Trove_After = await troveManagerHelpers.Troves(alice, ZERO_ADDRESS)
    const bob_Trove_After = await troveManagerHelpers.Troves(bob, ZERO_ADDRESS)
    const carol_Trove_After = await troveManagerHelpers.Troves(carol, ZERO_ADDRESS)

    const alice_Trove_After_Asset = await troveManagerHelpers.Troves(alice, erc20.address)
    const bob_Trove_After_Asset = await troveManagerHelpers.Troves(bob, erc20.address)
    const carol_Trove_After_Asset = await troveManagerHelpers.Troves(carol, erc20.address)
    ...
    /* check that Dennis' redeemed 20 DCHF has been cancelled with debt from Bobs's Trove (8) and Carol's Trove (10).
    The remaining lot (2) is sent to Alice's Trove, who had the best ICR.
    It leaves her with (3) DCHF debt + 50 for gas compensation. */
    ...
    const expectedTotalETHDrawn = redemptionAmount.div(toBN(200)) // convert redemptionAmount DCHF to ETH, at ETH:USD price 200
    ...
    const dennis_DCHFBalance_After = (await DCHFToken.balanceOf(dennis)).toString()
    assert.equal(
      dennis_DCHFBalance_After,
      dennis_DCHFBalance_Before.sub(redemptionAmount).sub(redemptionAmount_Asset)
    )
  })
  ...
  it('redeemCollateral(): with invalid first hint, zero address', async () => {
    ...
    // Find hints for redeeming 20 DCHF
    const { 1: partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
      ZERO_ADDRESS,
      redemptionAmount,
      price,
      0
    ...
    const expectedTotalETHDrawn_Asset = redemptionAmount_Asset.div(toBN(200)) // convert redemptionAmount DCHF to ETH, at ETH:USD price 200
    const expectedReceivedETH_Asset = expectedTotalETHDrawn_Asset.sub(toBN(ETHFee_Asset))
  ...
  it('redeemCollateral(): ends the redemption sequence when the token redemption request has been filled', async () => {
    ...
    const { netDebt: A_debt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(290, 16)),
      extraDCHFAmount: dec(20, 18),
      extraParams: { from: alice },
    ...
    const { totalDebt: D_totalDebt_Asset, collateral: D_coll_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(300, 16)),
      extraDCHFAmount: dec(10, 18),
      extraParams: { from: dennis },
    })
    const { totalDebt: E_totalDebt_Asset, collateral: E_coll_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(300, 16)),
      extraDCHFAmount: dec(10, 18),
      extraParams: { from: erin },
    ...
    // open trove from redeemer.  Redeemer has highest ICR (100ETH, 100 DCHF), 20000%
    const { DCHFAmount: F_DCHFAmount } = await openTrove({
      ICR: toBN(dec(200, 18)),
      extraDCHFAmount: redemptionAmount.mul(toBN(2)),
      extraParams: { from: flyn },
    })
    const { DCHFAmount: F_DCHFAmount_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 18)),
      extraDCHFAmount: redemptionAmount.mul(toBN(2)),
      extraParams: { from: flyn },
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-60) = 40 DCHF
    const flynBalance = await DCHFToken.balanceOf(flyn)
    th.assertIsApproximatelyEqual(
      flynBalance,
      F_DCHFAmount.add(F_DCHFAmount_Asset).sub(redemptionAmount).sub(redemptionAmount_Asset)
    )

    // Check debt of Alice, Bob, Carol
    const alice_Debt = await troveManagerHelpers.getTroveDebt(ZERO_ADDRESS, alice)
    const bob_Debt = await troveManagerHelpers.getTroveDebt(ZERO_ADDRESS, bob)
    const carol_Debt = await troveManagerHelpers.getTroveDebt(ZERO_ADDRESS, carol)

    const alice_Debt_Asset = await troveManagerHelpers.getTroveDebt(erc20.address, alice)
    const bob_Debt_Asset = await troveManagerHelpers.getTroveDebt(erc20.address, bob)
    const carol_Debt_Asset = await troveManagerHelpers.getTroveDebt(erc20.address, carol)
    ...
    const alice_Status = await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, alice)
    const bob_Status = await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, bob)
    const carol_Status = await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, carol)

    const alice_Status_Asset = await troveManagerHelpers.getTroveStatus(erc20.address, alice)
    const bob_Status_Asset = await troveManagerHelpers.getTroveStatus(erc20.address, bob)
    const carol_Status_Asset = await troveManagerHelpers.getTroveStatus(erc20.address, carol)
    ...
    const dennis_Debt = await troveManagerHelpers.getTroveDebt(ZERO_ADDRESS, dennis)
    const erin_Debt = await troveManagerHelpers.getTroveDebt(ZERO_ADDRESS, erin)

    const dennis_Debt_Asset = await troveManagerHelpers.getTroveDebt(erc20.address, dennis)
    const erin_Debt_Asset = await troveManagerHelpers.getTroveDebt(erc20.address, erin)
    ...
    const dennis_Coll = await troveManagerHelpers.getTroveColl(ZERO_ADDRESS, dennis)
    const erin_Coll = await troveManagerHelpers.getTroveColl(ZERO_ADDRESS, erin)

    const dennis_Coll_Asset = await troveManagerHelpers.getTroveColl(erc20.address, dennis)
    const erin_Coll_Asset = await troveManagerHelpers.getTroveColl(erc20.address, erin)
  ...
  it('redeemCollateral(): ends the redemption sequence when max iterations have been reached', async () => {
    ...
    const { netDebt: C_debt_Asset, totalDebt: C_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(286, 16)),
      extraDCHFAmount: dec(20, 18),
      extraParams: { from: carol },
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-40) = 60 DCHF
    const flynBalance = (await DCHFToken.balanceOf(flyn)).toString()
    th.assertIsApproximatelyEqual(
      flynBalance,
      F_DCHFAmount.add(F_DCHFAmount_Asset).sub(redemptionAmount).sub(redemptionAmount_Asset)
    )
  ...
  it('redeemCollateral(): performs partial redemption if resultant debt is > minimum net debt', async () => {
    ...
    await borrowerOperations.openTrove(
      ...
      0,
      th._100pct,
      await getOpenTroveDCHFAmount(dec(20000, 18), ZERO_ADDRESS),
      B,
      B,
    ...
    await borrowerOperations.openTrove(
      ...
      0,
      th._100pct,
      await getOpenTroveDCHFAmount(dec(30000, 18), ZERO_ADDRESS),
      C,
      C,
    ...
    await borrowerOperations.openTrove(
      ...
      dec(1000, 'ether'),
      th._100pct,
      await getOpenTroveDCHFAmount(dec(10000, 18), erc20.address),
      A,
      A,
      { from: A }
    )
    await borrowerOperations.openTrove(
      ...
      dec(1000, 'ether'),
      th._100pct,
      await getOpenTroveDCHFAmount(dec(20000, 18), erc20.address),
      B,
      B,
      { from: B }
    )
    await borrowerOperations.openTrove(
      ...
      dec(1000, 'ether'),
      th._100pct,
      await getOpenTroveDCHFAmount(dec(30000, 18), erc20.address),
      C,
      C,
      { from: C }
    )
    ...
    await DCHFToken.transfer(B, await DCHFToken.balanceOf(A), { from: A })
    await DCHFToken.transfer(B, await DCHFToken.balanceOf(C), { from: C })

    await troveManagerHelpers.setBaseRate(ZERO_ADDRESS, 0)
    await troveManagerHelpers.setBaseRate(erc20.address, 0)

    // skip bootstrapping phase
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_WEEK * 2, web3.currentProvider)

    // DCHF redemption is 55000 US
    const DCHFRedemption = dec(55000, 18)
    await th.redeemCollateralAndGetTxObject(B, contracts, DCHFRedemption, ZERO_ADDRESS)
    await th.redeemCollateralAndGetTxObject(B, contracts, DCHFRedemption, erc20.address)
    ...
    const A_debt = await troveManagerHelpers.getTroveDebt(ZERO_ADDRESS, A)
    const A_debt_Asset = await troveManagerHelpers.getTroveDebt(erc20.address, A)
  ...
  it("redeemCollateral(): doesn't perform partial redemption if resultant debt would be < minimum net debt", async () => {
    await borrowerOperations.openTrove(
      ...
      0,
      th._100pct,
      await getOpenTroveDCHFAmount(dec(6000, 18), ZERO_ADDRESS),
      A,
      A,
      { from: A, value: dec(1000, 'ether') }
    )
    ...
    await borrowerOperations.openTrove(
      ...
      dec(1000, 'ether'),
      th._100pct,
      await getOpenTroveDCHFAmount(dec(6000, 18), erc20.address),
      A,
      A,
      { from: A }
    )
  ...
  it('redeemCollateral(): doesnt perform the final partial redemption in the sequence if the hint is out-of-date', async () => {
    // --- SETUP ---
    await openTrove({ ICR: toBN(dec(363, 16)), extraDCHFAmount: dec(5, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({
      ICR: toBN(dec(344, 16)),
      extraDCHFAmount: dec(8, 18),
      extraParams: { from: bob },
    ...
    // Oops, another transaction gets in the way
    {
      ...
      // Alice redeems 1 DCHF from Carol's Trove
      await troveManager.redeemCollateral(
        ZERO_ADDRESS,
        frontRunRedepmtion,
        firstRedemptionHint,
    ...
    // Dennis tries to redeem 20 DCHF
    const redemptionTx = await troveManager.redeemCollateral(
      ZERO_ADDRESS,
      redemptionAmount,
      firstRedemptionHint,
    ...
    // Since Alice already redeemed 1 DCHF from Carol's Trove, Dennis was  able to redeem:
    //  - 9 DCHF from Carol's
    //  - 8 DCHF from Bob's
    // for a total of 17 DCHF.

    // Dennis calculated his hint for redeeming 2 DCHF from Alice's Trove, but after Alice's transaction
    // got in the way, he would have needed to redeem 3 DCHF to fully complete his redemption of 20 DCHF.
    ...
    const expectedTotalETHDrawn = fullfilledRedemptionAmount.sub(frontRunRedepmtion).div(toBN(200)) // redempted DCHF converted to ETH, at ETH:USD price 200
    const expectedTotalETHDrawn_Asset = fullfilledRedemptionAmount_Asset
  ...
  it.skip('redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool', async () => {
    ...
    const amount = await getOpenTroveDCHFAmount(dec(110, 18))
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(133, 16)), extraDCHFAmount: amount, extraParams: { from: bob } })

    await openTrove({ asset: erc20.address, ICR: toBN(dec(20, 18)), extraParams: { from: alice } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(133, 16)),
      extraDCHFAmount: amount,
      extraParams: { from: bob },
    })

    await DCHFToken.transfer(carol, amount.mul(toBN(2)), { from: bob })
    ...
    const expectedTotalETHDrawn = toBN(amount).div(toBN(100)) // convert 100 DCHF to ETH at ETH:USD price of 100
    const expectedReceivedETH = expectedTotalETHDrawn.sub(ETHFee)
    ...
    const carol_DCHFBalance_After = (await DCHFToken.balanceOf(carol)).toString()
    assert.equal(carol_DCHFBalance_After, '0')
  })
  it("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
    ...
    const { DCHFAmount: B_DCHFAmount, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(133, 16)),
      extraDCHFAmount: A_debt,
      extraParams: { from: bob },
    })
    ...
    const { DCHFAmount: B_DCHFAmount_Asset, totalDebt: B_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(133, 16)),
      extraDCHFAmount: A_debt,
      extraParams: { from: bob },
    ...
    await DCHFToken.transfer(carol, B_DCHFAmount.add(B_DCHFAmount_Asset), { from: bob })
    ...
    const { debt: alice_Debt_After } = await troveManagerHelpers.Troves(alice, ZERO_ADDRESS)
    assert.equal(alice_Debt_After, '0')

    const { debt: alice_Debt_After_Asset } = await troveManagerHelpers.Troves(alice, erc20.address)
    assert.equal(alice_Debt_After_Asset, '0')

    // Bob's Trove was left untouched
    const { debt: bob_Debt_After } = await troveManagerHelpers.Troves(bob, ZERO_ADDRESS)
    const { debt: bob_Debt_After_Asset } = await troveManagerHelpers.Troves(bob, erc20.address)
  ...
  it('redeemCollateral(): finds the last Trove with ICR == 110% even if there is more than one', async () => {
    ...
    const { totalDebt: B_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 16)),
      extraDCHFAmount: amount1,
      extraParams: { from: bob },
    })
    const { totalDebt: C_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 16)),
      extraDCHFAmount: amount1,
      extraParams: { from: carol },
    ...
    const { totalDebt: D_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(195, 16)),
      extraDCHFAmount: redemptionAmount_Asset,
      extraParams: { from: dennis },
    })
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraDCHFAmount: dec(10, 18), extraParams: { from: whale } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(100, 18)),
      extraDCHFAmount: dec(10, 18),
    ...
    const tx_Asset = await troveManager.redeemCollateral(
      erc20.address,
      redemptionAmount_Asset,
      carol, // try to trick redeemCollateral by passing a hint that doesn't exactly point to the
      // last Trove with ICR == 110% (which would be Alice's)
    ...
    const { debt: carol_Debt_After } = await troveManagerHelpers.Troves(carol, ZERO_ADDRESS)
    assert.equal(carol_Debt_After, '0')

    const { debt: carol_Debt_After_Asset } = await troveManagerHelpers.Troves(carol, erc20.address)
    assert.equal(carol_Debt_After_Asset, '0')

    const { debt: dennis_Debt_After } = await troveManagerHelpers.Troves(dennis, ZERO_ADDRESS)
    th.assertIsApproximatelyEqual(dennis_Debt_After, D_totalDebt)

    const { debt: dennis_Debt_After_Asset } = await troveManagerHelpers.Troves(dennis, erc20.address)
  ...
  it('redeemCollateral(): reverts when TCR < MCR', async () => {
    ...
    await assertRevert(
      th.redeemCollateral(carol, contracts, dec(270, 18), ZERO_ADDRESS),
      'TroveManager: Cannot redeem when TCR < MCR'
    )
    await assertRevert(
      th.redeemCollateral(carol, contracts, dec(270, 18), erc20.address),
      'TroveManager: Cannot redeem when TCR < MCR'
    )
  })
  ...
  it('redeemCollateral(): reverts when argument _amount is 0', async () => {
    ...
    // Alice opens trove and transfers 500DCHF to Erin, the would-be redeemer
    await openTrove({ ICR: toBN(dec(200, 16)), extraDCHFAmount: dec(500, 18), extraParams: { from: alice } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 16)),
      extraDCHFAmount: dec(500, 18),
      extraParams: { from: alice },
    })
    await DCHFToken.transfer(erin, dec(500, 18), { from: alice })
    ...
    const redemptionTxPromise_Asset = troveManager.redeemCollateral(
      erc20.address,
      0,
      erin,
      erin,
  ...
  it('redeemCollateral(): reverts if max fee > 100%', async () => {
    await openTrove({ ICR: toBN(dec(400, 16)), extraDCHFAmount: dec(10, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraDCHFAmount: dec(20, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraDCHFAmount: dec(30, 18), extraParams: { from: C } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraDCHFAmount: dec(40, 18), extraParams: { from: D } })
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(400, 16)),
      extraDCHFAmount: dec(30, 18),
      extraParams: { from: C },
    })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(400, 16)),
      extraDCHFAmount: dec(40, 18),
      extraParams: { from: D },
    })
    ...
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, dec(10, 18), ZERO_ADDRESS, dec(2, 18)),
      'Max fee percentage must be between 0.5% and 100%'
    )
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, dec(10, 18), ZERO_ADDRESS, '1000000000000000001'),
      'Max fee percentage must be between 0.5% and 100%'
    )

    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, dec(10, 18), erc20.address, dec(2, 18)),
      'Max fee percentage must be between 0.5% and 100%'
    )
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, dec(10, 18), erc20.address, '1000000000000000001'),
      'Max fee percentage must be between 0.5% and 100%'
    )
  })
  ...
  it('redeemCollateral(): reverts if max fee < 0.5%', async () => {
    ...
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, dec(10, 18), ZERO_ADDRESS, 0),
      'Max fee percentage must be between 0.5% and 100%'
    )
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, dec(10, 18), ZERO_ADDRESS, 1),
      'Max fee percentage must be between 0.5% and 100%'
    )
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, dec(10, 18), ZERO_ADDRESS, '4999999999999999'),
      'Max fee percentage must be between 0.5% and 100%'
    )

    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, dec(10, 18), erc20.address, 0),
      'Max fee percentage must be between 0.5% and 100%'
    )
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, dec(10, 18), erc20.address, 1),
      'Max fee percentage must be between 0.5% and 100%'
    )
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, dec(10, 18), erc20.address, '4999999999999999'),
      'Max fee percentage must be between 0.5% and 100%'
    )
  })
  ...
  it('redeemCollateral(): reverts if fee exceeds max fee percentage', async () => {
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraDCHFAmount: dec(80, 18),
      extraParams: { from: A },
    })
    const { totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraDCHFAmount: dec(90, 18),
      extraParams: { from: B },
    })
    const { totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraDCHFAmount: dec(100, 18),
      extraParams: { from: C },
    })
    ...
    // DCHF redemption is 27 USD: a redemption that incurs a fee of 27/(270 * 2) = 5%
    const attemptedDCHFRedemption = expectedTotalSupply.div(toBN(10))
    const attemptedDCHFRedemption_Asset = expectedTotalSupply_Asset.div(toBN(10))

    // Max fee is <5%
    const lessThan5pct = '49999999999999999'
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedDCHFRedemption, ZERO_ADDRESS, lessThan5pct),
      'Fee exceeded provided maximum'
    )
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedDCHFRedemption, erc20.address, lessThan5pct),
      'Fee exceeded provided maximum'
    )

    await troveManagerHelpers.setBaseRate(ZERO_ADDRESS, 0) // artificially zero the baseRate
    await troveManagerHelpers.setBaseRate(erc20.address, 0)

    // Max fee is 1%
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedDCHFRedemption, ZERO_ADDRESS, dec(1, 16)),
      'Fee exceeded provided maximum'
    )
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedDCHFRedemption, erc20.address, dec(1, 16)),
      'Fee exceeded provided maximum'
    )
    ...
    // Max fee is 3.754%
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedDCHFRedemption, ZERO_ADDRESS, dec(3754, 13)),
      'Fee exceeded provided maximum'
    )
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedDCHFRedemption, erc20.address, dec(3754, 13)),
      'Fee exceeded provided maximum'
    )
    ...
    // Max fee is 0.5%
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedDCHFRedemption, ZERO_ADDRESS, dec(5, 15)),
      'Fee exceeded provided maximum'
    )
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedDCHFRedemption, erc20.address, dec(5, 15)),
      'Fee exceeded provided maximum'
    )
  })
  ...
  it('redeemCollateral(): succeeds if fee is less than max fee percentage', async () => {
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraDCHFAmount: dec(9500, 18),
      extraParams: { from: A },
    })
    const { totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(395, 16)),
      extraDCHFAmount: dec(9000, 18),
      extraParams: { from: B },
    })
    const { totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(390, 16)),
      extraDCHFAmount: dec(10000, 18),
      extraParams: { from: C },
    })
    ...
    // DCHF redemption fee with 10% of the supply will be 0.5% + 1/(10*2)
    const attemptedDCHFRedemption = expectedTotalSupply.div(toBN(10))
    const attemptedDCHFRedemption_Asset = expectedTotalSupply_Asset.div(toBN(10))

    // Attempt with maxFee > 5.5%
    const price = await priceFeed.getPrice()
    const ETHDrawn = attemptedDCHFRedemption.mul(mv._1e18BN).div(price)
    const ETHDrawn_Asset = attemptedDCHFRedemption_Asset.mul(mv._1e18BN).div(price)

    const slightlyMoreThanFee = await troveManagerHelpers.getRedemptionFeeWithDecay(ZERO_ADDRESS, ETHDrawn)
    const slightlyMoreThanFee_Asset = await troveManagerHelpers.getRedemptionFeeWithDecay(
      erc20.address,
      ETHDrawn
    )
    ...
    // Attempt with maxFee = 5.5%
    const exactSameFee = await troveManagerHelpers.getRedemptionFeeWithDecay(ZERO_ADDRESS, ETHDrawn)
    const exactSameFee_Asset = await troveManagerHelpers.getRedemptionFeeWithDecay(
      erc20.address,
      ETHDrawn_Asset
    )
  ...
  it("redeemCollateral(): doesn't affect the Stability Pool deposits or ETH gain of redeemed-from troves", async () => {
    ...
    const { totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(195, 16)),
      extraDCHFAmount: dec(200, 18),
      extraParams: { from: carol },
    })
    const { totalDebt: D_totalDebt } = await openTrove({
      ICR: toBN(dec(190, 16)),
      extraDCHFAmount: dec(400, 18),
      extraParams: { from: dennis },
    })
    ...
    const { totalDebt: F_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 16)),
      extraDCHFAmount: dec(100, 18),
      extraParams: { from: flyn },
    ...
    // Alice opens trove and transfers DCHF to Erin, the would-be redeemer
    await openTrove({
      ICR: toBN(dec(300, 16)),
      extraDCHFAmount: redemptionAmount,
      extraParams: { from: alice },
    ...
    await DCHFToken.transfer(erin, redemptionAmount.add(redemptionAmount_Asset), { from: alice })
    ...
    const bob_ICR_before = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price)
    const carol_ICR_before = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)
    const dennis_ICR_before = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, dennis, price)

    const bob_ICR_before_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)
    const carol_ICR_before_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)
    const dennis_ICR_before_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, dennis, price)
    ...
    const bob_SPDeposit_before = (await stabilityPool.getCompoundedDCHFDeposit(bob)).toString()
    const carol_SPDeposit_before = (await stabilityPool.getCompoundedDCHFDeposit(carol)).toString()
    const dennis_SPDeposit_before = (await stabilityPool.getCompoundedDCHFDeposit(dennis)).toString()

    const bob_SPDeposit_before_Asset = (await stabilityPoolERC20.getCompoundedDCHFDeposit(bob)).toString()
    const carol_SPDeposit_before_Asset = (await stabilityPoolERC20.getCompoundedDCHFDeposit(carol)).toString()
    const dennis_SPDeposit_before_Asset = (
      await stabilityPoolERC20.getCompoundedDCHFDeposit(dennis)
    ).toString()
    ...
    // Check the remaining DCHF and ETH in Stability Pool after liquidation is non-zero
    const DCHFinSP = await stabilityPool.getTotalDCHFDeposits()
    const ETHinSP = await stabilityPool.getAssetBalance()

    const DCHFinSP_Asset = await stabilityPoolERC20.getTotalDCHFDeposits()
    const ETHinSP_Asset = await stabilityPoolERC20.getAssetBalance()

    assert.isTrue(DCHFinSP.gte(mv._zeroBN))
    assert.isTrue(ETHinSP.gte(mv._zeroBN))

    assert.isTrue(DCHFinSP_Asset.gte(mv._zeroBN))
    ...
    const bob_ICR_after = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price)
    const carol_ICR_after = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)
    const dennis_ICR_after = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, dennis, price)

    const bob_ICR_after_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)
    const carol_ICR_after_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)
    const dennis_ICR_after_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, dennis, price)
    ...
    const bob_SPDeposit_after = (await stabilityPool.getCompoundedDCHFDeposit(bob)).toString()
    const carol_SPDeposit_after = (await stabilityPool.getCompoundedDCHFDeposit(carol)).toString()
    const dennis_SPDeposit_after = (await stabilityPool.getCompoundedDCHFDeposit(dennis)).toString()

    const bob_SPDeposit_after_Asset = (await stabilityPoolERC20.getCompoundedDCHFDeposit(bob)).toString()
    const carol_SPDeposit_after_Asset = (await stabilityPoolERC20.getCompoundedDCHFDeposit(carol)).toString()
    const dennis_SPDeposit_after_Asset = (
      await stabilityPoolERC20.getCompoundedDCHFDeposit(dennis)
    ).toString()
  ...
  it('redeemCollateral(): caller can redeem their entire DCHFToken balance', async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({
      ICR: toBN(dec(20, 18)),
      extraParams: { from: whale },
    })
    const { collateral: W_coll_Asset, totalDebt: W_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(20, 18)),
      extraParams: { from: whale },
    })

    // Alice opens trove and transfers 400 DCHF to Erin, the would-be redeemer
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraDCHFAmount: dec(400, 18),
      extraParams: { from: alice },
    ...
    await DCHFToken.transfer(erin, toBN(dec(400, 18)).mul(toBN(2)), { from: alice })

    // Check Erin's balance before
    const erin_balance_before = await DCHFToken.balanceOf(erin)
    ...
    const { collateral: D_coll_Asset, totalDebt: D_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(500, 16)),
      extraDCHFAmount: dec(1990, 18),
      extraParams: { from: dennis },
    ...
    const totalDebt_Asset = W_totalDebt_Asset.add(A_totalDebt_Asset)
      ...
      .add(D_totalDebt_Asset)
    const totalColl_Asset = W_coll_Asset.add(A_coll_Asset)
      .add(B_coll_Asset)
      .add(C_coll_Asset)
      .add(D_coll_Asset)

    // Get active debt and coll before redemption
    const activePool_debt_before = await activePool.getDCHFDebt(ZERO_ADDRESS)
    const activePool_coll_before = await activePool.getAssetBalance(ZERO_ADDRESS)

    const activePool_debt_before_Asset = await activePool.getDCHFDebt(erc20.address)
    ...
    // Erin attempts to redeem 400 DCHF
    const { firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
      ZERO_ADDRESS,
      dec(400, 18),
      price,
    ...
    const { 0: upperPartialRedemptionHint, 1: lowerPartialRedemptionHint } =
      await sortedTroves.findInsertPosition(ZERO_ADDRESS, partialRedemptionHintNICR, erin, erin)

    const { 0: upperPartialRedemptionHint_Asset, 1: lowerPartialRedemptionHint_Asset } =
      await sortedTroves.findInsertPosition(erc20.address, partialRedemptionHintNICR, erin, erin)
    ...
    // Check activePool debt reduced by  400 DCHF
    const activePool_debt_after = await activePool.getDCHFDebt(ZERO_ADDRESS)
    assert.equal(activePool_debt_before.sub(activePool_debt_after), dec(400, 18))

    const activePool_debt_after_Asset = await activePool.getDCHFDebt(erc20.address)
    ...
    */
    ...
    const erin_balance_after = (await DCHFToken.balanceOf(erin)).toString()
    assert.equal(erin_balance_after, '0')
  })

  it("redeemCollateral(): reverts when requested redemption amount exceeds caller's DCHF token balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({
      ICR: toBN(dec(20, 18)),
      extraParams: { from: whale },
    })
    ...
    // Erin tries to redeem 1000 DCHF
    try {
      ...
      assert.include(error.message, 'RR')
    }
    ...
    try {
      ...
      const { 0: upperPartialRedemptionHint_1_Asset, 1: lowerPartialRedemptionHint_1_Asset } =
        await sortedTroves.findInsertPosition(erc20.address, partialRedemptionHintNICR_Asset, erin, erin)
    ...
    // Erin tries to redeem 239482309 DCHF
    try {
      ;({ firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
        ZERO_ADDRESS,
    ...
    // Erin tries to redeem 2^256 - 1 DCHF
  ...
  it('redeemCollateral(): value of issued ETH == face value of redeemed DCHF (assuming 1 DCHF has value of $1)', async () => {
    const { collateral: W_coll } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    const { collateral: W_coll_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(20, 18)),
      extraParams: { from: whale },
    })

    // Alice opens trove and transfers 1000 DCHF each to Erin, Flyn, Graham
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraDCHFAmount: dec(4990, 18),
      extraParams: { from: alice },
    })
    const { collateral: A_coll_Asset, totalDebt: A_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
    ...
    await DCHFToken.transfer(erin, toBN(dec(1000, 18)).mul(toBN(2)), { from: alice })
    await DCHFToken.transfer(flyn, toBN(dec(1000, 18)).mul(toBN(2)), { from: alice })
    await DCHFToken.transfer(graham, toBN(dec(1000, 18)).mul(toBN(2)), { from: alice })

    // B, C, D open trove
    const { collateral: B_coll } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraDCHFAmount: dec(1590, 18),
      extraParams: { from: bob },
    })
    const { collateral: C_coll } = await openTrove({
      ICR: toBN(dec(600, 16)),
      extraDCHFAmount: dec(1090, 18),
      extraParams: { from: carol },
    })
    ...
    const { collateral: B_coll_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(300, 16)),
      extraDCHFAmount: dec(1590, 18),
      extraParams: { from: bob },
    })
    const { collateral: C_coll_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(600, 16)),
      extraDCHFAmount: dec(1090, 18),
      extraParams: { from: carol },
    })
    const { collateral: D_coll_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(800, 16)),
      extraDCHFAmount: dec(1090, 18),
      extraParams: { from: dennis },
    ...
    const _120_DCHF = '120000000000000000000'
    const _373_DCHF = '373000000000000000000'
    const _950_DCHF = '950000000000000000000'
    ...
    await ({ 0: firstRedemptionHint_Asset, 1: partialRedemptionHintNICR_Asset } =
      await hintHelpers.getRedemptionHints(erc20.address, _120_DCHF, price, 0))
    ...
    const redemption_1_Asset = await troveManager.redeemCollateral(
      erc20.address,
      _120_DCHF,
      firstRedemptionHint_Asset,
      upperPartialRedemptionHint_1_Asset,
    ...
    /* 120 DCHF redeemed.  Expect $120 worth of ETH removed. At ETH:USD price of $200,
    ...
    assert.equal(activeETH_1.toString(), activeETH_0.sub(toBN(_120_DCHF).mul(mv._1e18BN).div(price)))
    assert.equal(
      activeETH_1_Asset.toString(),
      activeETH_0_Asset.sub(toBN(_120_DCHF).mul(mv._1e18BN).div(price))
    )

    // Flyn redeems 373 DCHF
    ;({ firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
      ZERO_ADDRESS,
      _373_DCHF,
      price,
    ...
    const { 0: upperPartialRedemptionHint_2, 1: lowerPartialRedemptionHint_2 } =
      await sortedTroves.findInsertPosition(ZERO_ADDRESS, partialRedemptionHintNICR, flyn, flyn)
    ...
    ;({ 0: firstRedemptionHint_Asset, 1: partialRedemptionHintNICR_Asset } =
      await hintHelpers.getRedemptionHints(erc20.address, _373_DCHF, price, 0))

    const { 0: upperPartialRedemptionHint_2_Asset, 1: lowerPartialRedemptionHint_2_Asset } =
      await sortedTroves.findInsertPosition(erc20.address, partialRedemptionHintNICR_Asset, flyn, flyn)

    const redemption_2_Asset = await troveManager.redeemCollateral(
      erc20.address,
      _373_DCHF,
      firstRedemptionHint_Asset,
      upperPartialRedemptionHint_2_Asset,
    ...
    /* 373 DCHF redeemed.  Expect $373 worth of ETH removed. At ETH:USD price of $200,
    ...
    assert.equal(activeETH_2.toString(), activeETH_1.sub(toBN(_373_DCHF).mul(mv._1e18BN).div(price)))
    assert.equal(
      activeETH_2_Asset.toString(),
      activeETH_1_Asset.sub(toBN(_373_DCHF).mul(mv._1e18BN).div(price))
    )

    // Graham redeems 950 DCHF
    ;({ firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
      ZERO_ADDRESS,
      _950_DCHF,
      price,
    ...
    const { 0: upperPartialRedemptionHint_3, 1: lowerPartialRedemptionHint_3 } =
      await sortedTroves.findInsertPosition(ZERO_ADDRESS, partialRedemptionHintNICR, graham, graham)
    ...
    ;({ 0: firstRedemptionHint_Asset, 1: partialRedemptionHintNICR_Asset } =
      await hintHelpers.getRedemptionHints(erc20.address, _950_DCHF, price, 0))

    const { 0: upperPartialRedemptionHint_3_Asset, 1: lowerPartialRedemptionHint_3_Asset } =
      await sortedTroves.findInsertPosition(erc20.address, partialRedemptionHintNICR_Asset, graham, graham)

    const redemption_3_Asset = await troveManager.redeemCollateral(
      erc20.address,
      _950_DCHF,
      firstRedemptionHint_Asset,
      upperPartialRedemptionHint_3_Asset,
    ...
    /* 950 DCHF redeemed.  Expect $950 worth of ETH removed. At ETH:USD price of $200,
    ...
    assert.equal(activeETH_3.toString(), activeETH_2.sub(toBN(_950_DCHF).mul(mv._1e18BN).div(price)))
    assert.equal(
      activeETH_3_Asset.toString(),
      activeETH_2_Asset.sub(toBN(_950_DCHF).mul(mv._1e18BN).div(price))
    )
  })
  ...
  it('redeemCollateral(): reverts if there is zero outstanding system debt', async () => {
    // --- SETUP --- illegally mint DCHF to Bob
    await DCHFToken.unprotectedMint(bob, toBN(dec(100, 18)).mul(toBN(2)))

    assert.equal(await DCHFToken.balanceOf(bob), toBN(dec(100, 18)).mul(toBN(2)).toString())
    ...
    const { 0: upperPartialRedemptionHint, 1: lowerPartialRedemptionHint } =
      await sortedTroves.findInsertPosition(ZERO_ADDRESS, partialRedemptionHintNICR, bob, bob)

    // Bob tries to redeem his illegally obtained DCHF
    try {
      await troveManager.redeemCollateral(
        ZERO_ADDRESS,
        dec(100, 18),
    ...
    const { 0: upperPartialRedemptionHint_Asset, 1: lowerPartialRedemptionHint_Asset } =
      await sortedTroves.findInsertPosition(erc20.address, partialRedemptionHintNICR_Asset, bob, bob)
  ...
  it("redeemCollateral(): reverts if caller's tries to redeem more than the outstanding system debt", async () => {
    // --- SETUP --- illegally mint DCHF to Bob
    await DCHFToken.unprotectedMint(bob, '202000000000000000000')

    assert.equal(await DCHFToken.balanceOf(bob), '202000000000000000000')
    ...
    th.assertIsApproximatelyEqual((await activePool.getDCHFDebt(ZERO_ADDRESS)).toString(), totalDebt)
    th.assertIsApproximatelyEqual((await activePool.getDCHFDebt(erc20.address)).toString(), totalDebt_Asset)
    ...
    // Bob attempts to redeem his ill-gotten 101 DCHF, from a system that has 100 DCHF outstanding debt
    try {
      const redemptionTx = await troveManager.redeemCollateral(
        ZERO_ADDRESS,
  ...
  it('redeemCollateral(): a redemption made when base rate is zero increases the base rate', async () => {
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraDCHFAmount: dec(100, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(190, 16)), extraDCHFAmount: dec(100, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(180, 16)), extraDCHFAmount: dec(100, 18), extraParams: { from: C } })
    ...
    const A_balanceBefore = await DCHFToken.balanceOf(A)
    ...
    // Check A's balance has decreased by 10 DCHF
    assert.equal(await DCHFToken.balanceOf(A), A_balanceBefore.sub(toBN(dec(10, 18)).mul(toBN(2))).toString())
  ...
  it('redeemCollateral(): a redemption made when base rate is non-zero increases the base rate, for negligible time passed', async () => {
    // time fast-forwards 1 year, and multisig stakes 1 MON
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await monToken.approve(monStaking.address, dec(1, 18), { from: multisig })
    await monStaking.stake(dec(1, 18), { from: multisig })
    ...
    const A_balanceBefore = await DCHFToken.balanceOf(A)
    const B_balanceBefore = await DCHFToken.balanceOf(B)
    ...
    // Check B's balance has decreased by 10 DCHF
    assert.equal(await DCHFToken.balanceOf(B), B_balanceBefore.sub(toBN(dec(10, 18)).mul(toBN(2))).toString())
    ...
    const baseRate_2 = await troveManagerHelpers.baseRate(ZERO_ADDRESS)
    const baseRate_2_Asset = await troveManagerHelpers.baseRate(erc20.address)
  ...
  it("redeemCollateral(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation [ @skip-on-coverage ]", async () => {
    ...
    // Check A's balance has decreased by 10 DCHF
    assert.equal(
      A_balanceBefore.sub(await DCHFToken.balanceOf(A)).toString(),
      toBN(dec(10, 18)).mul(toBN(2)).toString()
    )
    ...
    const lastFeeOpTime_1 = await troveManagerHelpers.lastFeeOperationTime(ZERO_ADDRESS)
    const lastFeeOpTime_1_Asset = await troveManagerHelpers.lastFeeOperationTime(erc20.address)
    ...
    const lastFeeOpTime_2 = await troveManagerHelpers.lastFeeOperationTime(ZERO_ADDRESS)
    const lastFeeOpTime_2_Asset = await troveManagerHelpers.lastFeeOperationTime(erc20.address)
    ...
    const lastFeeOpTime_3 = await troveManagerHelpers.lastFeeOperationTime(ZERO_ADDRESS)
    const lastFeeOpTime_3_Asset = await troveManagerHelpers.lastFeeOperationTime(erc20.address)
  ...
  it('redeemCollateral(): a redemption made at zero base rate send a non-zero ETHFee to MON staking contract', async () => {
    // time fast-forwards 1 year, and multisig stakes 1 MON
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await monToken.approve(monStaking.address, dec(1, 18), { from: multisig })
    await monStaking.stake(dec(1, 18), { from: multisig })
    ...
    // Check MON Staking contract balance before is zero
    const MONStakingBalance_Before = await web3.eth.getBalance(monStaking.address)
    const MONStakingBalance_Before_Asset = await erc20.balanceOf(monStaking.address)
    assert.equal(MONStakingBalance_Before, '0')
    assert.equal(MONStakingBalance_Before_Asset, '0')
    ...
    // Check MON Staking contract balance after is non-zero
    const MONStakingBalance_After = toBN(await web3.eth.getBalance(monStaking.address))
    assert.isTrue(MONStakingBalance_After.gt(toBN('0')))

    const MONStakingBalance_After_Asset = toBN(await erc20.balanceOf(monStaking.address))
    assert.isTrue(MONStakingBalance_After_Asset.gt(toBN('0')))
  })

  it('redeemCollateral(): a redemption made at zero base increases the ETH-fees-per-MON-staked in MON Staking contract', async () => {
    // time fast-forwards 1 year, and multisig stakes 1 MON
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await monToken.approve(monStaking.address, dec(1, 18), { from: multisig })
    await monStaking.stake(dec(1, 18), { from: multisig })
    ...
    // Check MON Staking ETH-fees-per-MON-staked before is zero
    ...
    // Check MON Staking ETH-fees-per-MON-staked after is non-zero
    assert.isTrue((await monStaking.F_ASSETS(ZERO_ADDRESS)).gt('0'))
    assert.isTrue((await monStaking.F_ASSETS(erc20.address)).gt('0'))
  })

  it('redeemCollateral(): a redemption made at a non-zero base rate send a non-zero ETHFee to MON staking contract', async () => {
    // time fast-forwards 1 year, and multisig stakes 1 MON
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await monToken.approve(monStaking.address, dec(1, 18), { from: multisig })
    await monStaking.stake(dec(1, 18), { from: multisig })
    ...
    const MONStakingBalance_Before = toBN(await web3.eth.getBalance(monStaking.address))
    const MONStakingBalance_Before_Asset = toBN(await erc20.balanceOf(monStaking.address))
    ...
    // check MON Staking balance has increased
    assert.isTrue(MONStakingBalance_After.gt(MONStakingBalance_Before))
    assert.isTrue(MONStakingBalance_After_Asset.gt(MONStakingBalance_Before_Asset))
  })

  it('redeemCollateral(): a redemption made at a non-zero base rate increases ETH-per-MON-staked in the staking contract', async () => {
    // time fast-forwards 1 year, and multisig stakes 1 MON
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await monToken.approve(monStaking.address, dec(1, 18), { from: multisig })
    await monStaking.stake(dec(1, 18), { from: multisig })
  ...
  it('redeemCollateral(): a redemption sends the ETH remainder (ETHDrawn - ETHFee) to the redeemer', async () => {
    ...
    const { totalDebt: W_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(20, 18)),
      extraParams: { from: whale },
    })
    ...
    const baseRate = await troveManagerHelpers.baseRate(ZERO_ADDRESS)
    assert.equal(baseRate, '0')

    const baseRate_Asset = await troveManagerHelpers.baseRate(erc20.address)
    assert.equal(baseRate_Asset, '0')

    // Check total DCHF supply
    const activeDCHF = await activePool.getDCHFDebt(ZERO_ADDRESS)
    const defaultDCHF = await defaultPool.getDCHFDebt(ZERO_ADDRESS)

    const activeDCHF_Asset = await activePool.getDCHFDebt(erc20.address)
    const defaultDCHF_Asset = await defaultPool.getDCHFDebt(erc20.address)

    const totalDCHFSupply = activeDCHF.add(defaultDCHF)
    const totalDCHFSupply_Asset = activeDCHF_Asset.add(defaultDCHF_Asset)
    th.assertIsApproximatelyEqual(totalDCHFSupply, totalDebt)
    th.assertIsApproximatelyEqual(totalDCHFSupply_Asset, totalDebt_Asset)
    ...
    */
    ...
    th.assertIsApproximatelyEqual(
      ...
      ETHDrawn.sub(
        toBN(dec(5, 15))
          .add(redemptionAmount.mul(mv._1e18BN).div(totalDebt_Asset).div(toBN(2)))
          .mul(ETHDrawn)
          .div(mv._1e18BN)
      ).div(toBN(10 ** 10)),
      100000
  ...
  it('redeemCollateral(): a full redemption (leaving trove with 0 debt), closes the trove', async () => {
    ...
    const { netDebt: W_netDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(20, 18)),
      extraDCHFAmount: dec(10000, 18),
      extraParams: { from: whale },
    })
    const { netDebt: A_netDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 16)),
      extraDCHFAmount: dec(100, 18),
      extraParams: { from: A },
    ...
    const { netDebt: D_netDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(280, 16)),
      extraDCHFAmount: dec(100, 18),
      extraParams: { from: D },
    ...
    // whale redeems 360 DCHF.  Expect this to fully redeem A, B, C, and partially redeem D.
  ...
  const redeemCollateral3Full1Partial = async () => {
    ...
    const { netDebt: A_netDebt_Asset, collateral: A_coll_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 16)),
      extraDCHFAmount: dec(100, 18),
      extraParams: { from: A },
    })
    const { netDebt: B_netDebt_Asset, collateral: B_coll_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(190, 16)),
      extraDCHFAmount: dec(100, 18),
      extraParams: { from: B },
    })
    const { netDebt: C_netDebt_Asset, collateral: C_coll_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(180, 16)),
      extraDCHFAmount: dec(100, 18),
      extraParams: { from: C },
    ...
    const A_collBefore = await troveManagerHelpers.getTroveColl(ZERO_ADDRESS, A)
    const B_collBefore = await troveManagerHelpers.getTroveColl(ZERO_ADDRESS, B)
    const C_collBefore = await troveManagerHelpers.getTroveColl(ZERO_ADDRESS, C)
    const D_collBefore = await troveManagerHelpers.getTroveColl(ZERO_ADDRESS, D)

    const A_collBefore_Asset = await troveManagerHelpers.getTroveColl(erc20.address, A)
    const B_collBefore_Asset = await troveManagerHelpers.getTroveColl(erc20.address, B)
    const C_collBefore_Asset = await troveManagerHelpers.getTroveColl(erc20.address, C)
    const D_collBefore_Asset = await troveManagerHelpers.getTroveColl(erc20.address, D)
    ...
    // whale redeems DCHF.  Expect this to fully redeem A, B, C, and partially redeem D.
    await th.redeemCollateral(whale, contracts, redemptionAmount, ZERO_ADDRESS)
    ...
    */
    ...
    const A_collAfter = await troveManagerHelpers.getTroveColl(ZERO_ADDRESS, A)
    const B_collAfter = await troveManagerHelpers.getTroveColl(ZERO_ADDRESS, B)
    const C_collAfter = await troveManagerHelpers.getTroveColl(ZERO_ADDRESS, C)

    const A_collAfter_Asset = await troveManagerHelpers.getTroveColl(erc20.address, A)
    const B_collAfter_Asset = await troveManagerHelpers.getTroveColl(erc20.address, B)
    const C_collAfter_Asset = await troveManagerHelpers.getTroveColl(erc20.address, C)
    ...
    const D_collAfter = await troveManagerHelpers.getTroveColl(ZERO_ADDRESS, D)
    assert.isTrue(D_collAfter.lt(D_collBefore))

    const D_collAfter_Asset = await troveManagerHelpers.getTroveColl(erc20.address, D)
    ...
    await assertRevert(
      borrowerOperations.openTrove(ZERO_ADDRESS, 0, th._100pct, 0, ZERO_ADDRESS, ZERO_ADDRESS, {
        from: D,
        value: dec(10, 18),
      }),
    ...
    await assertRevert(
      borrowerOperations.openTrove(erc20.address, dec(10, 18), th._100pct, 0, ZERO_ADDRESS, ZERO_ADDRESS, {
        from: D,
      }),
      'BorrowerOps: Trove is active'
    )
  ...
  it("redeemCollateral(): emits correct debt and coll values in each redeemed trove's TroveUpdated event", async () => {
    ...
    // whale redeems DCHF.  Expect this to fully redeem A, B, C, and partially redeem 15 DCHF from D.
    ...
    const [A_emittedDebt_Asset, A_emittedColl_Asset] = th.getDebtAndCollFromTroveUpdatedEvents(
      troveUpdatedEvents_Asset,
      A
    )
    const [B_emittedDebt_Asset, B_emittedColl_Asset] = th.getDebtAndCollFromTroveUpdatedEvents(
      troveUpdatedEvents_Asset,
      B
    )
    const [C_emittedDebt_Asset, C_emittedColl_Asset] = th.getDebtAndCollFromTroveUpdatedEvents(
      troveUpdatedEvents_Asset,
      C
    )
    const [D_emittedDebt_Asset, D_emittedColl_Asset] = th.getDebtAndCollFromTroveUpdatedEvents(
      troveUpdatedEvents_Asset,
      D
    )
    ...
    th.assertIsApproximatelyEqual(
      D_emittedColl_Asset,
      D_coll_Asset.sub(partialAmount.mul(mv._1e18BN).div(price))
    )
  })
  ...
  it("redeemCollateral(): a redemption that closes a trove leaves the trove's ETH surplus (collateral - ETH drawn) available for the trove owner to claim", async () => {
    ...
    await assertRevert(
      collSurplusPool.claimColl(ZERO_ADDRESS, A),
    ...
    th.assertIsApproximatelyEqual(
      A_balanceAfter_Asset,
      A_balanceBefore_Asset.add(
        A_coll_Asset.sub(A_netDebt_Asset.mul(mv._1e18BN).div(price)).div(toBN(10 ** 10))
      )
    )
    th.assertIsApproximatelyEqual(
      B_balanceAfter_Asset,
      B_balanceBefore_Asset.add(
        B_coll_Asset.sub(B_netDebt_Asset.mul(mv._1e18BN).div(price)).div(toBN(10 ** 10))
      )
    )
    th.assertIsApproximatelyEqual(
      C_balanceAfter_Asset,
      C_balanceBefore_Asset.add(
        C_coll_Asset.sub(C_netDebt_Asset.mul(mv._1e18BN).div(price)).div(toBN(10 ** 10))
      )
    )
  })
  ...
  it("redeemCollateral(): a redemption that closes a trove leaves the trove's ETH surplus (collateral - ETH drawn) available for the trove owner after re-opening trove", async () => {
    ...
    const { collateral: A_coll_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 16)),
      extraDCHFAmount: dec(100, 18),
      extraParams: { from: A },
    ...
    th.assertIsApproximatelyEqual(
      A_balanceAfter_Asset,
      A_balanceBefore_Asset.add(A_surplus_Asset.div(toBN(10 ** 10)))
    )
    th.assertIsApproximatelyEqual(
      B_balanceAfter_Asset,
      B_balanceBefore_Asset.add(B_surplus_Asset.div(toBN(10 ** 10)))
    )
    th.assertIsApproximatelyEqual(
      C_balanceAfter_Asset,
      C_balanceBefore_Asset.add(C_surplus_Asset.div(toBN(10 ** 10)))
    )
  })

  it('redeemCollateral(): reverts if fee eats up all returned collateral', async () => {
    // --- SETUP ---
    const { DCHFAmount } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraDCHFAmount: dec(1, 24),
      extraParams: { from: alice },
    })
    ...
    const { DCHFAmount: DCHFAmount_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 16)),
      extraDCHFAmount: dec(1, 24),
      extraParams: { from: alice },
    ...
    for (let i = 0; i < 2; i++) {
      ...
      await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: bob } })
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, DCHFAmount, true, alice, alice, {
        from: alice,
        value: DCHFAmount.mul(mv._1e18BN).div(price),
      })
    }
    ...
    for (let i = 0; i < 2; i++) {
      // Find hints for redeeming
      const { 0: firstRedemptionHint_Asset, 1: partialRedemptionHintNICR_Asset } =
        await hintHelpers.getRedemptionHints(erc20.address, DCHFAmount_Asset, price, 0)
      ...
      await borrowerOperations.adjustTrove(
        erc20.address,
        DCHFAmount_Asset.mul(mv._1e18BN).div(price),
        th._100pct,
        0,
        DCHFAmount_Asset,
        true,
  ...
  it('getPendingDCHFDebtReward(): Returns 0 if there is no pending DCHFDebt reward', async () => {
    // Make some troves
    ...
    await openTrove({ ICR: toBN(dec(3, 18)), extraDCHFAmount: dec(20, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(20, 18)), extraDCHFAmount: totalDebt, extraParams: { from: whale } })

    const { totalDebt: totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(2, 18)),
      extraDCHFAmount: dec(100, 18),
      extraParams: { from: defaulter_1 },
    ...
    const current_L_DCHFDebt = await troveManagerHelpers.L_DCHFDebts(ZERO_ADDRESS)
    assert.equal(current_L_DCHFDebt, 0)

    const current_L_DCHFDebt_Asset = await troveManagerHelpers.L_DCHFDebts(erc20.address)
    assert.equal(current_L_DCHFDebt_Asset, 0)

    const carolSnapshot_L_DCHFDebt = (await troveManagerHelpers.getRewardSnapshots(ZERO_ADDRESS, carol))[1]
    assert.equal(carolSnapshot_L_DCHFDebt, 0)

    const carolSnapshot_L_DCHFDebt_Asset = (
      await troveManagerHelpers.getRewardSnapshots(erc20.address, carol)
    )[1]
    assert.equal(carolSnapshot_L_DCHFDebt_Asset, 0)

    const carol_PendingDCHFDebtReward = await troveManagerHelpers.getPendingDCHFDebtReward(
      ZERO_ADDRESS,
      carol
    )
    assert.equal(carol_PendingDCHFDebtReward, 0)

    const carol_PendingDCHFDebtReward_Asset = await troveManagerHelpers.getPendingDCHFDebtReward(
      erc20.address,
      carol
    )
    assert.equal(carol_PendingDCHFDebtReward_Asset, 0)
  })
  ...
  it('getPendingETHReward(): Returns 0 if there is no pending ETH reward', async () => {
    ...
    const { totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(2, 18)),
      extraDCHFAmount: dec(100, 18),
      extraParams: { from: defaulter_1 },
    ...
    // Confirm there are no pending rewards from liquidation
    const current_L_ETH = await troveManagerHelpers.L_ASSETS(ZERO_ADDRESS)
    assert.equal(current_L_ETH, 0)

    const carolSnapshot_L_ETH = (await troveManagerHelpers.getRewardSnapshots(ZERO_ADDRESS, carol))[0]
    assert.equal(carolSnapshot_L_ETH, 0)

    const carol_PendingETHReward = await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, carol)
    assert.equal(carol_PendingETHReward, 0)

    const current_L_ETH_Asset = await troveManagerHelpers.L_ASSETS(erc20.address)
    assert.equal(current_L_ETH_Asset, 0)

    const carolSnapshot_L_ETH_Asset = (await troveManagerHelpers.getRewardSnapshots(erc20.address, carol))[0]
    assert.equal(carolSnapshot_L_ETH_Asset, 0)

    const carol_PendingETHReward_Asset = await troveManagerHelpers.getPendingAssetReward(erc20.address, carol)
  ...
  it('computeICR(): Returns 2^256-1 for ETH:USD = 100, coll = 1 ETH, debt = 100 DCHF', async () => {
    const price = dec(100, 18)
    const coll = dec(1, 'ether')
    const debt = dec(100, 18)
  ...
  it('computeICR(): returns correct ICR for ETH:USD = 100, coll = 200 ETH, debt = 30 DCHF', async () => {
    const price = dec(100, 18)
  ...
  it('computeICR(): returns correct ICR for ETH:USD = 250, coll = 1350 ETH, debt = 127 DCHF', async () => {
  ...
  it('computeICR(): returns correct ICR for ETH:USD = 100, coll = 1 ETH, debt = 54321 DCHF', async () => {
    const price = dec(100, 18)
    const coll = dec(1, 'ether')
  ...
  it('getTroveStake(): Returns stake', async () => {
    ...
    const A_Stake = await troveManagerHelpers.getTroveStake(ZERO_ADDRESS, A)
    const B_Stake = await troveManagerHelpers.getTroveStake(ZERO_ADDRESS, B)

    const A_Stake_Asset = await troveManagerHelpers.getTroveStake(erc20.address, A)
    const B_Stake_Asset = await troveManagerHelpers.getTroveStake(erc20.address, B)
  ...
  it('getTroveColl(): Returns coll', async () => {
    ...
    assert.equal(await troveManagerHelpers.getTroveColl(ZERO_ADDRESS, A), A_coll.toString())
    assert.equal(await troveManagerHelpers.getTroveColl(ZERO_ADDRESS, B), B_coll.toString())

    assert.equal(await troveManagerHelpers.getTroveColl(erc20.address, A), A_coll.toString())
    assert.equal(await troveManagerHelpers.getTroveColl(erc20.address, B), B_coll.toString())
  })
  ...
  it('getTroveDebt(): Returns debt', async () => {
    ...
    const { totalDebt: totalDebtA_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(150, 16)),
      extraParams: { from: A },
    })
    const { totalDebt: totalDebtB_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(150, 16)),
      extraParams: { from: B },
    })
    ...
    const A_Debt = await troveManagerHelpers.getTroveDebt(ZERO_ADDRESS, A)
    const B_Debt = await troveManagerHelpers.getTroveDebt(ZERO_ADDRESS, B)

    const A_Debt_Asset = await troveManagerHelpers.getTroveDebt(erc20.address, A)
    const B_Debt_Asset = await troveManagerHelpers.getTroveDebt(erc20.address, B)
  ...
  it('getTroveStatus(): Returns status', async () => {
    ...
    await openTrove({ ICR: toBN(dec(150, 16)), extraDCHFAmount: B_totalDebt, extraParams: { from: A } })
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(150, 16)),
      extraDCHFAmount: B_totalDebt,
      extraParams: { from: A },
    })

    // to be able to repay:
    await DCHFToken.transfer(B, B_totalDebt, { from: A })
    await DCHFToken.transfer(B, B_totalDebt_Asset, { from: A })

    await borrowerOperations.closeTrove(ZERO_ADDRESS, { from: B })
    await borrowerOperations.closeTrove(erc20.address, { from: B })

    const A_Status = await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, A)
    const B_Status = await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, B)
    const C_Status = await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, C)

    const A_Status_Asset = await troveManagerHelpers.getTroveStatus(erc20.address, A)
    const B_Status_Asset = await troveManagerHelpers.getTroveStatus(erc20.address, B)
    const C_Status_Asset = await troveManagerHelpers.getTroveStatus(erc20.address, C)
  ...
  it('hasPendingRewards(): Returns false it trove is not active', async () => {
    assert.isFalse(await troveManagerHelpers.hasPendingRewards(ZERO_ADDRESS, alice))
    assert.isFalse(await troveManagerHelpers.hasPendingRewards(erc20.address, alice))
  })
})
