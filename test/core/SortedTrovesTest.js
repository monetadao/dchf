...
contract('SortedTroves', async (accounts) => {
  ...
  const assertSortedListIsOrdered = async (contracts) => {
    ...
    while (trove !== (await contracts.sortedTroves.getFirst(ZERO_ADDRESS))) {
      ...
      const troveICR = await contracts.troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, trove, price)
      const prevTroveICR = await contracts.troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, prevTrove, price)

      assert.isTrue(prevTroveICR.gte(troveICR))

      const troveNICR = await contracts.troveManagerHelpers.getNominalICR(ZERO_ADDRESS, trove)
      const prevTroveNICR = await contracts.troveManagerHelpers.getNominalICR(ZERO_ADDRESS, prevTrove)
  ...
  describe('SortedTroves', () => {
    ...
    it('contains(): returns true for addresses that have opened troves', async () => {
      ...
      assert.equal((await troveManagerHelpers.Troves(alice, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX], '1')
      assert.equal((await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX], '1')
      assert.equal((await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX], '1')
    ...
    it('contains(): returns false for addresses that have not opened troves', async () => {
      ...
      assert.equal((await troveManagerHelpers.Troves(dennis, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX], '0')
      assert.equal((await troveManagerHelpers.Troves(erin, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX], '0')
    ...
    it('contains(): returns false for addresses that opened and then closed a trove', async () => {
      ...
      assert.equal((await troveManagerHelpers.Troves(alice, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX], '2')
      assert.equal((await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX], '2')
      assert.equal((await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX], '2')
    ...
    it('Finds the correct insert position given two addresses that loosely bound the correct position', async () => {
      ...
      // Pass addresses that loosely bound the right position
    ...
    //--- Ordering ---
    // infinite ICR (zero collateral) is not possible anymore, therefore, skipping
    it.skip("stays ordered after troves with 'infinite' ICR receive a redistribution", async () => {
      ...
      await borrowerOperations.openTrove(ZERO_ADDRESS, 0, th._100pct, 0, whale, whale, {
        from: whale,
        value: dec(50, 'ether'),
      })
      await borrowerOperations.openTrove(ZERO_ADDRESS, 0, th._100pct, 0, A, A, {
        from: A,
        value: dec(1, 'ether'),
      })
      await borrowerOperations.openTrove(ZERO_ADDRESS, 0, th._100pct, 0, B, B, {
        from: B,
        value: dec(37, 'ether'),
      })
      await borrowerOperations.openTrove(ZERO_ADDRESS, 0, th._100pct, 0, C, C, {
        from: C,
        value: dec(5, 'ether'),
      })
      await borrowerOperations.openTrove(ZERO_ADDRESS, 0, th._100pct, 0, D, D, {
        from: D,
        value: dec(4, 'ether'),
      })
      await borrowerOperations.openTrove(ZERO_ADDRESS, 0, th._100pct, 0, E, E, {
        from: E,
        value: dec(19, 'ether'),
      })

      // Make some troves with non-zero debt, in random order
      await borrowerOperations.openTrove(ZERO_ADDRESS, 0, th._100pct, dec(5, 19), F, F, {
        from: F,
        value: dec(1, 'ether'),
      })
      await borrowerOperations.openTrove(ZERO_ADDRESS, 0, th._100pct, dec(3, 18), G, G, {
        from: G,
        value: dec(37, 'ether'),
      })
      await borrowerOperations.openTrove(ZERO_ADDRESS, 0, th._100pct, dec(2, 20), H, H, {
        from: H,
        value: dec(5, 'ether'),
      })
      await borrowerOperations.openTrove(ZERO_ADDRESS, 0, th._100pct, dec(17, 18), I, I, {
        from: I,
        value: dec(4, 'ether'),
      })
      await borrowerOperations.openTrove(ZERO_ADDRESS, 0, th._100pct, dec(5, 21), J, J, {
        from: J,
    ...
    context('when params are properly set', () => {
      it('insert(): fails if list already contains the node', async () => {
        ...
        await th.assertRevert(
          sortedTrovesTester.insert(ZERO_ADDRESS, alice, 1, alice, alice),
      ...
      it('insert(): fails if id is zero', async () => {
        await th.assertRevert(
          sortedTrovesTester.insert(ZERO_ADDRESS, th.ZERO_ADDRESS, 1, alice, alice),
      ...
      it('insert(): fails if NICR is zero', async () => {
        await th.assertRevert(
          sortedTrovesTester.insert(ZERO_ADDRESS, alice, 0, alice, alice),
      ...
      it('remove(): fails if id is not in the list', async () => {
        await th.assertRevert(
          sortedTrovesTester.remove(ZERO_ADDRESS, alice),
      ...
      it('reInsert(): fails if list doesn’t contain the node', async () => {
        await th.assertRevert(
          sortedTrovesTester.reInsert(ZERO_ADDRESS, alice, 1, alice, alice),
          'SortedTroves: List does not contain the id'
        )
      })
      ...
      it('reInsert(): fails if new NICR is zero', async () => {
        ...
        await th.assertRevert(
          sortedTrovesTester.reInsert(ZERO_ADDRESS, alice, 0, alice, alice),
          'SortedTroves: NICR must be positive'
        )
        assert.isTrue(await sortedTroves.contains(ZERO_ADDRESS, alice), 'list should contain element')
      })
