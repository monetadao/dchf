...
contract('Gas compensation tests', async (accounts) => {
  ...
  beforeEach(async () => {
    ...
    community = MONContracts.communityIssuance
  ...
  it('getCollGasCompensation(): returns the 0.5% of collateral if it is < $10 in value', async () => {
    ...
    -> Expect 0.5% of collateral as gas compensation */
    await priceFeed.setPrice(dec(1, 18))
    // const price_1 = await priceFeed.getPrice()

    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(ZERO_ADDRESS, dec(1, 'ether'))).toString(),
      dec(5, 15)
    )
    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(erc20.address, dec(1, 'ether'))).toString(),
      dec(5, 15)
    )
    ...
    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(ZERO_ADDRESS, dec(100, 'finney'))).toString(),
      dec(5, 14)
    )
    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(erc20.address, dec(100, 'finney'))).toString(),
      dec(5, 14)
    )
    ...
    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(ZERO_ADDRESS, '5000000000')).toString(),
      '25000000'
    )
    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(erc20.address, '5000000000')).toString(),
      '25000000'
    )
  })

  it('getCollGasCompensation(): returns 0.5% of collateral when 0.5% of collateral < $10 in value', async () => {
    ...
    -> Expect 0.5% of collateral as gas compensation */
    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(ZERO_ADDRESS, '9999000000000000000')).toString(),
      '49995000000000000'
    )
    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(erc20.address, '9999000000000000000')).toString(),
      '49995000000000000'
    )
    ...
    /* ETH:USD price = 200
     ...
     -> Expect 0.5% of collateral as gas compensation */
    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(ZERO_ADDRESS, '55000000000000000')).toString(),
      dec(275, 12)
    )
    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(erc20.address, '55000000000000000')).toString(),
      dec(275, 12)
    )
    ...
    -> Expect 0.5% of collateral as gas compensation */
    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(ZERO_ADDRESS, '6092324088087235800')).toString(),
      '30461620440436179'
    )
    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(erc20.address, '6092324088087235800')).toString(),
      '30461620440436179'
    )
  })
  ...
  it('getCollGasCompensation(): returns 0.5% of collateral when 0.5% of collateral = $10 in value', async () => {
    ...
    -> Expect 0.5% of collateral as gas compensation */
    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(ZERO_ADDRESS, dec(10, 'ether'))).toString(),
      '50000000000000000'
    )
    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(erc20.address, dec(10, 'ether'))).toString(),
      '50000000000000000'
    )
  })
  ...
  it('getCollGasCompensation(): returns 0.5% of collateral when 0.5% of collateral = $10 in value', async () => {
    ...
    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(ZERO_ADDRESS, dec(100, 'ether'))).toString(),
      dec(500, 'finney')
    )
    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(erc20.address, dec(100, 'ether'))).toString(),
      dec(500, 'finney')
    )
    ...
    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(ZERO_ADDRESS, '10001000000000000000')).toString(),
      '50005000000000000'
    )
    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(erc20.address, '10001000000000000000')).toString(),
      '50005000000000000'
    )
    ...
    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(ZERO_ADDRESS, '37500000000000000000')).toString(),
      '187500000000000000'
    )
    assert.equal(
      (await troveManagerHelpers.getCollGasCompensation(erc20.address, '37500000000000000000')).toString(),
      '187500000000000000'
    )
    ...
    assert.isAtMost(
      th.getDifference(
        await troveManagerHelpers.getCollGasCompensation(ZERO_ADDRESS, '94758230582309850000000'),
    ...
    assert.isAtMost(
      th.getDifference(
        await troveManagerHelpers.getCollGasCompensation(erc20.address, '94758230582309850000000'),
        '473791152911549000000'
      ),
      1000000
    )
    ...
    assert.equal(
      (
        await troveManagerHelpers.getCollGasCompensation(ZERO_ADDRESS, '300000000000000000000000000')
      ).toString(),
      '1500000000000000000000000'
    )
    assert.equal(
      (
        await troveManagerHelpers.getCollGasCompensation(erc20.address, '300000000000000000000000000')
      ).toString(),
      '1500000000000000000000000'
    )
  })
  ...
  it('_getCompositeDebt(): returns (debt + 50) when collateral < $10 in value', async () => {
    ...
    debt = 10 DCHF
    0.5% of coll = 0.04995 ETH. USD value: $9.99
    -> Expect composite debt = 10 + 200  = 2100 DCHF*/
    assert.equal(await troveManagerHelpers.getCompositeDebt(ZERO_ADDRESS, dec(10, 18)), dec(210, 18))
    assert.equal(await troveManagerHelpers.getCompositeDebt(erc20.address, dec(10, 18)), dec(210, 18))

    /* ETH:USD price = 200
     coll = 0.055 ETH  
     debt = 0 DCHF
     0.5% of coll = 0.000275 ETH. USD value: $0.055
     -> Expect composite debt = 0 + 200 = 200 DCHF*/
    assert.equal(await troveManagerHelpers.getCompositeDebt(ZERO_ADDRESS, 0), dec(200, 18))
    assert.equal(await troveManagerHelpers.getCompositeDebt(erc20.address, 0), dec(200, 18))

    // /* ETH:USD price = 200
    // coll = 6.09232408808723580 ETH
    // debt = 200 DCHF
    // 0.5% of coll = 0.004995 ETH. USD value: $6.09
    // -> Expect  composite debt =  200 + 200 = 400  DCHF */
    assert.equal(
      await troveManagerHelpers.getCompositeDebt(ZERO_ADDRESS, dec(200, 18)),
      '400000000000000000000'
    )
    assert.equal(
      await troveManagerHelpers.getCompositeDebt(erc20.address, dec(200, 18)),
      '400000000000000000000'
    )
  })
  ...
  it('getCompositeDebt(): returns (debt + 50) collateral = $10 in value', async () => {
    ...
    debt = 123.45 DCHF
    0.5% of coll = 0.5 ETH. USD value: $10
    -> Expect composite debt = (123.45 + 200) = 323.45 DCHF  */
    assert.equal(
      await troveManagerHelpers.getCompositeDebt(ZERO_ADDRESS, '123450000000000000000'),
      '323450000000000000000'
    )
    assert.equal(
      await troveManagerHelpers.getCompositeDebt(erc20.address, '123450000000000000000'),
      '323450000000000000000'
    )
  })
  ...
  it('getCompositeDebt(): returns (debt + 50) when 0.5% of collateral > $10 in value', async () => {
    ...
    coll = 100 ETH  
    debt = 2000 DCHF
    -> Expect composite debt = (2000 + 200) = 2200 DCHF  */
    assert.equal(
      (await troveManagerHelpers.getCompositeDebt(ZERO_ADDRESS, dec(2000, 18))).toString(),
      '2200000000000000000000'
    )
    assert.equal(
      (await troveManagerHelpers.getCompositeDebt(erc20.address, dec(2000, 18))).toString(),
      '2200000000000000000000'
    )
    ...
    -> Expect composite debt = (200 + 200) = 400 DCHF  */
    assert.equal(
      (await troveManagerHelpers.getCompositeDebt(ZERO_ADDRESS, dec(200, 18))).toString(),
      '400000000000000000000'
    )
    assert.equal(
      (await troveManagerHelpers.getCompositeDebt(erc20.address, dec(200, 18))).toString(),
      '400000000000000000000'
    )
    ...
    coll = 37.5 ETH  
    debt = 500 DCHF
    -> Expect composite debt = (500 + 200) = 700 DCHF  */
    assert.equal(
      (await troveManagerHelpers.getCompositeDebt(ZERO_ADDRESS, dec(500, 18))).toString(),
      '700000000000000000000'
    )
    assert.equal(
      (await troveManagerHelpers.getCompositeDebt(erc20.address, dec(500, 18))).toString(),
      '700000000000000000000'
    )
    ...
    coll = 94758.230582309850 ETH  
    debt = 1 billion DCHF
    -> Expect composite debt = (1000000000 + 200) = 1000000200 DCHF  */
    await priceFeed.setPrice('45323545420000000000000')
    assert.isAtMost(
      th.getDifference(
        (await troveManagerHelpers.getCompositeDebt(ZERO_ADDRESS, dec(1, 27))).toString(),
    ...
    assert.isAtMost(
      th.getDifference(
        (await troveManagerHelpers.getCompositeDebt(erc20.address, dec(1, 27))).toString(),
        '1000000200000000000000000000'
      ),
      100000000000
    )
    ...
    ETH:USD price = 1000000 $/E (1 million)
    coll = 300000000 ETH   (300 million)
    debt = 54321.123456789 DCHF
   -> Expect composite debt = (54321.123456789 + 200) = 54521.123456789 DCHF */
    await priceFeed.setPrice(dec(1, 24))
    assert.equal(
      (await troveManagerHelpers.getCompositeDebt(ZERO_ADDRESS, '54321123456789000000000')).toString(),
      '54521123456789000000000'
    )
    assert.equal(
      (await troveManagerHelpers.getCompositeDebt(erc20.address, '54321123456789000000000')).toString(),
      '54521123456789000000000'
    )
  })
  ...
  it('getCurrentICR(): Incorporates virtual debt, and returns the correct ICR for new troves', async () => {
    ...
    const alice_ICRERC20 = (await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)).toString()
    ...
    const bob_ICRERC20 = (await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)).toString()
    ...
    // F opens with 1 ETH, 100 DCHF
    await openTrove({ ICR: toBN(dec(2, 18)), extraDCHFAmount: dec(100, 18), extraParams: { from: flyn } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(2, 18)),
    ...
    const flyn_ICR = (await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, flyn, price)).toString()
    const flyn_ICRERC20 = (await troveManagerHelpers.getCurrentICR(erc20.address, flyn, price)).toString()
    ...
    const carol_ICR = (await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)).toString()
    const carol_ICRERC20 = (await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)).toString()
    ...
    const dennis_ICRERC20 = (await troveManagerHelpers.getCurrentICR(erc20.address, dennis, price)).toString()
    ...
    // E opens with 4405.45 ETH, 32598.35 DCHF
    ...
    const erin_ICR = (await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, erin, price)).toString()
    const erin_ICRERC20 = (await troveManagerHelpers.getCurrentICR(erc20.address, erin, price)).toString()
    ...
    const harriet_ICR = (await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, harriet, price)).toString()
    const harriet_ICRERC20 = (
      await troveManagerHelpers.getCurrentICR(erc20.address, harriet, price)
    ).toString()
  ...
  it('Gas compensation from pool-offset liquidations. All collateral paid as compensation', async () => {
    ...
    await openTrove({ ICR: toBN(dec(2, 18)), extraDCHFAmount: A_totalDebt, extraParams: { from: dennis } })
    await openTrove({
      ICR: toBN(dec(2, 18)),
      extraDCHFAmount: B_totalDebt.add(C_totalDebt),
      extraParams: { from: erin },
    })

    await openTrove({ asset: erc20.address, ICR: toBN(dec(2000, 18)), extraParams: { from: whale } })
    const { totalDebt: A_totalDebtERC20 } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(2, 18)),
      extraDCHFAmount: dec(100, 18),
      extraParams: { from: alice },
    })
    const { totalDebt: B_totalDebtERC20 } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(2, 18)),
      extraDCHFAmount: dec(200, 18),
      extraParams: { from: bob },
    })
    const { totalDebt: C_totalDebtERC20 } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(2, 18)),
      extraDCHFAmount: dec(300, 18),
      extraParams: { from: carol },
    ...
    console.log((await community.MONSupplyCaps(stabilityPool.address)).toString())
    console.log((await community.MONSupplyCaps(stabilityPoolERC20.address)).toString())

    // D, E each provide DCHF to SP
    ...
    const DCHFinSP_0 = await stabilityPool.getTotalDCHFDeposits()
    const DCHFinSP_0ERC20 = await stabilityPoolERC20.getTotalDCHFDeposits()
    ...
    /* 
    ETH:USD price = 9.99
    -> Expect 0.5% of collateral to be sent to liquidator, as gas compensation */

    // Check collateral value in USD is < $10
    const aliceColl = (await troveManagerHelpers.Troves(alice, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
    const aliceCollERC20 = (await troveManagerHelpers.Troves(alice, erc20.address))[th.TROVE_COLL_INDEX]
    ...
    const compensationReceived_AERC20 = liquidatorBalance_after_AERC20
      .sub(liquidatorBalance_before_AERC20)
      .toString()
    ...
    // Check SP DCHF has decreased due to the liquidation
    const DCHFinSP_A = await stabilityPool.getTotalDCHFDeposits()
    const DCHFinSP_AERC20 = await stabilityPoolERC20.getTotalDCHFDeposits()
    assert.isTrue(DCHFinSP_A.lte(DCHFinSP_0))
    assert.isTrue(DCHFinSP_AERC20.lte(DCHFinSP_0ERC20))
    ...
    assert.equal(
      (await stabilityPoolERC20.getAssetBalance()).toString(),
    ...
    // Check collateral value in USD is < $10
    const bobColl = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
    const bobCollERC20 = (await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_COLL_INDEX]
    ...
    const compensationReceived_BERC20 = liquidatorBalance_after_BERC20
      .sub(liquidatorBalance_before_BERC20)
      .toString()
    ...
    // Check SP DCHF has decreased due to the liquidation of B
    const DCHFinSP_B = await stabilityPool.getTotalDCHFDeposits()
    const DCHFinSP_BERC20 = await stabilityPool.getTotalDCHFDeposits()

    assert.isTrue(DCHFinSP_B.lt(DCHFinSP_A))
    assert.isTrue(DCHFinSP_BERC20.lt(DCHFinSP_AERC20))

    // Check ETH in SP has received the liquidation
    assert.equal(
      (await stabilityPool.getAssetBalance()).toString(),
    ...
    // Check collateral value in USD is < $10
    const carolColl = (await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
    const carolCollERC20 = (await troveManagerHelpers.Troves(carol, erc20.address))[th.TROVE_COLL_INDEX]
    ...
    const compensationReceived_CERC20 = liquidatorBalance_after_CERC20
      .sub(liquidatorBalance_before_CERC20)
      .toString()
    ...
    // Check SP DCHF has decreased due to the liquidation of C
    assert.isTrue((await stabilityPool.getTotalDCHFDeposits()).lt(DCHFinSP_B))
    assert.isTrue((await stabilityPoolERC20.getTotalDCHFDeposits()).lt(DCHFinSP_BERC20))
  ...
  it('gas compensation from pool-offset liquidations: 0.5% collateral < $10 in value. Compensates $10 worth of collateral, liquidates the remainder', async () => {
    ...
    await openTrove({ ICR: toBN(dec(2000, 18)), extraParams: { from: whale } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraDCHFAmount: dec(200, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(120, 16)), extraDCHFAmount: dec(5000, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(60, 18)), extraDCHFAmount: dec(600, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(80, 18)), extraDCHFAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(80, 18)), extraDCHFAmount: dec(1, 23), extraParams: { from: erin } })
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(60, 18)),
      extraDCHFAmount: dec(600, 18),
      extraParams: { from: carol },
    })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(80, 18)),
      extraDCHFAmount: dec(1, 23),
      extraParams: { from: dennis },
    })
    await openTrove({
      asset: erc20.address,
    ...
    // D, E each provide 10000 DCHF to SP
    ...
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, alice, price_1)).lt(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, alice, price_1)).lt(mv._MCR))
    ...
    // Check SP DCHF has decreased due to the liquidation of A
    const DCHFinSP_A = await stabilityPool.getTotalDCHFDeposits()
    const DCHFinSP_AERC20 = await stabilityPoolERC20.getTotalDCHFDeposits()
    assert.isTrue(DCHFinSP_A.lte(DCHFinSP_0))
    assert.isTrue(DCHFinSP_AERC20.lte(DCHFinSP_0ERC20))
    ...
    const bobICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price_2)
    assert.isTrue(bobICR.lte(mv._MCR))

    const bobCollERC20 = (await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_COLL_INDEX]
    assert.isFalse(await th.checkRecoveryMode(contracts, erc20.address))

    const bobICRERC20 = await troveManagerHelpers.getCurrentICR(erc20.address, bob, price_2)
    ...
    // Check SP DCHF has decreased due to the liquidation of B
    assert.isTrue((await stabilityPool.getTotalDCHFDeposits()).lt(DCHFinSP_A))
    assert.isTrue((await stabilityPoolERC20.getTotalDCHFDeposits()).lt(DCHFinSP_AERC20))
  ...
  it('gas compensation from pool-offset liquidations: 0.5% collateral > $10 in value. Compensates 0.5% of  collateral, liquidates the remainder', async () => {
    ...
    await priceFeed.setPrice(dec(400, 18))
    await openTrove({ ICR: toBN(dec(200, 18)), extraParams: { from: whale } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraDCHFAmount: dec(2000, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(1875, 15)), extraDCHFAmount: dec(8000, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraDCHFAmount: dec(600, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(4, 18)), extraDCHFAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(4, 18)), extraDCHFAmount: dec(1, 23), extraParams: { from: erin } })
   ...
   and (37.5 - 0.1875 ETH) ETH remainder liquidated */
    ...
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price_1)).lt(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, bob, price_1)).lt(mv._MCR))
  ...
  it('Gas compensation from pool-offset liquidations. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    ...
    await openTrove({ ICR: toBN(dec(2, 18)), extraDCHFAmount: dec(300, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraDCHFAmount: A_totalDebt, extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraDCHFAmount: B_totalDebt, extraParams: { from: erin } })
    ...
    th.logBN('TCR', await troveManagerHelpers.getTCR(ZERO_ADDRESS, await priceFeed.getPrice()))
    // --- Price drops to 9.99 ---
    await priceFeed.setPrice('9990000000000000000')
    ...
    // Check collateral value in USD is < $10
    const aliceColl = (await troveManagerHelpers.Troves(alice, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
    const aliceDebt = (await troveManagerHelpers.Troves(alice, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]

    const aliceCollERC20 = (await troveManagerHelpers.Troves(alice, erc20.address))[th.TROVE_COLL_INDEX]
    const aliceDebtERC20 = (await troveManagerHelpers.Troves(alice, erc20.address))[th.TROVE_DEBT_INDEX]
    ...
    // Liquidate A (use 0 gas price to easily check the amount the compensation amount the liquidator receives)
    const liquidationTxA = await troveManager.liquidate(ZERO_ADDRESS, alice, {
      from: liquidator,
      gasPrice: 0,
    })
    ...
    const liquidationTxAERC20 = await troveManager.liquidate(erc20.address, alice, {
      from: liquidator,
      gasPrice: 0,
    })
    ...
    // Check collateral value in USD is < $10
    const bobColl = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
    const bobDebt = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]

    const bobCollERC20 = (await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_COLL_INDEX]
    const bobDebtERC20 = (await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_DEBT_INDEX]
    ...
    const liquidationTxBERC20 = await troveManager.liquidate(erc20.address, bob, {
      from: liquidator,
      gasPrice: 0,
    })
  ...
  it('gas compensation from pool-offset liquidations. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    ...
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price_2)).lte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, bob, price_2)).lte(mv._MCR))
  ...
  it('liquidateTroves(): full offset.  Compensates the correct amount, and liquidates the remainder', async () => {
    ...
    await openTrove({ ICR: toBN(dec(2000, 18)), extraParams: { from: whale } })
    await openTrove({ ICR: toBN(dec(118, 16)), extraDCHFAmount: dec(2000, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(526, 16)), extraDCHFAmount: dec(8000, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(488, 16)), extraDCHFAmount: dec(600, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(545, 16)), extraDCHFAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraDCHFAmount: dec(1, 23), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraDCHFAmount: dec(1, 23), extraParams: { from: flyn } })
    ...
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, erin, price)).gt(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, flyn, price)).gt(mv._MCR))

    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, erin, price)).gt(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, flyn, price)).gt(mv._MCR))

    // --- Check value of of A's collateral is < $10, and value of B,C,D collateral are > $10  ---
    const aliceColl = (await troveManagerHelpers.Troves(alice, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
    const bobColl = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
    const carolColl = (await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
    const dennisColl = (await troveManagerHelpers.Troves(dennis, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]

    const aliceCollERC20 = (await troveManagerHelpers.Troves(alice, erc20.address))[th.TROVE_COLL_INDEX]
    const bobCollERC20 = (await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_COLL_INDEX]
    const carolCollERC20 = (await troveManagerHelpers.Troves(carol, erc20.address))[th.TROVE_COLL_INDEX]
    const dennisCollERC20 = (await troveManagerHelpers.Troves(dennis, erc20.address))[th.TROVE_COLL_INDEX]
    ...
    const collGasCompensation = await troveManagerHelpers.getCollGasCompensation(ZERO_ADDRESS, price)
    const collGasCompensationERC20 = await troveManagerHelpers.getCollGasCompensation(erc20.address, price)
    ...
    */
    ...
    // Check DCHF in SP has decreased
    assert.isTrue((await stabilityPool.getTotalDCHFDeposits()).lt(DCHFinSP_0))
    assert.isTrue((await stabilityPoolERC20.getTotalDCHFDeposits()).lt(DCHFinSP_0ERC20))
    ...
    const compensationReceivedERC20 = liquidatorBalance_afterERC20
      .sub(liquidatorBalance_beforeERC20)
      .toString()
  ...
  it('liquidateTroves(): full redistribution. Compensates the correct amount, and liquidates the remainder', async () => {
    ...
    const DCHFinDefaultPool_0 = await defaultPool.getDCHFDebt(ZERO_ADDRESS)
    const DCHFinDefaultPool_0ERC20 = await defaultPool.getDCHFDebt(erc20.address)
    ...
    assert.equal(await troveManagerHelpers.getCollGasCompensation(ZERO_ADDRESS, price), dec(1, 18))
    assert.equal(await troveManagerHelpers.getCollGasCompensation(erc20.address, price), dec(1, 18))
    ...
    */
    ...
    // Check DCHF in DefaultPool has decreased
    assert.isTrue((await defaultPool.getDCHFDebt(ZERO_ADDRESS)).gt(DCHFinDefaultPool_0))
    assert.isTrue((await defaultPool.getDCHFDebt(erc20.address)).gt(DCHFinDefaultPool_0))
    ...
    assert.isAtMost(
      th.getDifference(expectedGasCompERC20.div(toBN(10 ** 10)), compensationReceivedERC20),
      1000
    )
  ...
  it('liquidateTroves(): full offset. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    ...
    const { totalDebt: D_totalDebtERC20 } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(545, 16)),
      extraDCHFAmount: dec(1, 23),
      extraParams: { from: dennis },
    ...
    */
    ...
    // Expect liquidatedDebt = 51 + 190 + 1025 + 13510 = 14646 DCHF
    ...
    // Liquidate troves A-D
    const liquidationTxData = await troveManager.liquidateTroves(ZERO_ADDRESS, 4, {
      from: liquidator,
      gasPrice: 0,
    })
    const liquidationTxDataERC20 = await troveManager.liquidateTroves(erc20.address, 4, {
      from: liquidator,
      gasPrice: 0,
    })
  ...
  it('Trove ordering: same collateral, decreasing debt. Price successively increases. Troves should maintain ordering by ICR', async () => {
    ...
    while (price < 210) {
      ...
      const ICRList = []
      const coll_firstTrove = (await troveManagerHelpers.Troves(_10_accounts[0], ZERO_ADDRESS))[
        th.TROVE_COLL_INDEX
      ]
      const gasComp_firstTrove = (
        await troveManagerHelpers.getCollGasCompensation(ZERO_ADDRESS, coll_firstTrove)
      ).toString()

      const ICRListERC20 = []
      const coll_firstTroveERC20 = (await troveManagerHelpers.Troves(_10_accounts[0], erc20.address))[
        th.TROVE_COLL_INDEX
      ]
      const gasComp_firstTroveERC20 = (
        await troveManagerHelpers.getCollGasCompensation(erc20.address, coll_firstTroveERC20)
      ).toString()

      for (account of _10_accounts) {
        // Check gas compensation is the same for all troves
        const coll = (await troveManagerHelpers.Troves(account, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
        const gasCompensation = (
          await troveManagerHelpers.getCollGasCompensation(ZERO_ADDRESS, coll)
        ).toString()

        const collERC20 = (await troveManagerHelpers.Troves(account, erc20.address))[th.TROVE_COLL_INDEX]
        const gasCompensationERC20 = (
          await troveManagerHelpers.getCollGasCompensation(erc20.address, collERC20)
        ).toString()

        assert.equal(gasCompensation, gasComp_firstTrove)
        assert.equal(gasCompensationERC20, gasComp_firstTroveERC20)

        const ICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, account, price)
        const ICRERC20 = await troveManagerHelpers.getCurrentICR(erc20.address, account, price)
        ICRList.push(ICR)
  ...
  it('Trove ordering: increasing collateral, constant debt. Price successively increases. Troves should maintain ordering by ICR', async () => {
    ...
    let coll = 50
    // create 20 troves, increasing collateral, constant debt = 100DCHF
    for (const account of _20_accounts) {
      const collString = coll.toString().concat('000000000000000000')
      await openTrove({ extraDCHFAmount: dec(100, 18), extraParams: { from: account, value: collString } })
      await openTrove({
        asset: erc20.address,
