...
contract('HintHelpers', async (accounts) => {
  ...
  /* Open a Trove for each account. DCHF debt is 200 DCHF each, with collateral beginning at 1.5 ether, and rising by 0.01 ether per Trove.
   *  Hence, the ICR of account (i + 1) is always 1% greater than the ICR of account i.
   */
  ...
  const withdrawDCHFfromTrove = async (account) => {
    await borrowerOperations.withdrawDCHF(th._100pct, '100000000000000000000', account, account, {
      from: account,
    })
  }
  ...
  it('setup: makes accounts with nominal ICRs increasing by 1% consecutively', async () => {
    ...
    const ICR_0 = await troveManagerHelpers.getNominalICR(ZERO_ADDRESS, accounts[0])
    const ICR_1 = await troveManagerHelpers.getNominalICR(ZERO_ADDRESS, accounts[1])
    const ICR_2 = await troveManagerHelpers.getNominalICR(ZERO_ADDRESS, accounts[2])
    const ICR_3 = await troveManagerHelpers.getNominalICR(ZERO_ADDRESS, accounts[3])
    const ICR_4 = await troveManagerHelpers.getNominalICR(ZERO_ADDRESS, accounts[4])
    const ICR_5 = await troveManagerHelpers.getNominalICR(ZERO_ADDRESS, accounts[5])
    const ICR_6 = await troveManagerHelpers.getNominalICR(ZERO_ADDRESS, accounts[6])
    const ICR_7 = await troveManagerHelpers.getNominalICR(ZERO_ADDRESS, accounts[7])
    const ICR_8 = await troveManagerHelpers.getNominalICR(ZERO_ADDRESS, accounts[8])
    const ICR_9 = await troveManagerHelpers.getNominalICR(ZERO_ADDRESS, accounts[9])
  ...
  it('getApproxHint(): returns the address of a Trove within sqrt(length) positions of the correct insert position', async () => {
    ...
    /* As per the setup, the ICRs of Troves are monotonic and separated by 1% intervals. Therefore, the difference in ICR between 
    ...
    const ICR_hintAddress_250 = await troveManagerHelpers.getNominalICR(ZERO_ADDRESS, hintAddress)
    ...
    const ICR_hintAddress_287 = await troveManagerHelpers.getNominalICR(ZERO_ADDRESS, hintAddress)
    ...
    const ICR_hintAddress_213 = await troveManagerHelpers.getNominalICR(ZERO_ADDRESS, hintAddress)
    ...
    const ICR_hintAddress_201 = await troveManagerHelpers.getNominalICR(ZERO_ADDRESS, hintAddress)
  ...
  //       latestRandomSeed
  //     )
  //     const ICR_hintAddress = await troveManagerHelpers.getNominalICR(ZERO_ADDRESS, hintAddress)
  ...
  it('getApproxHint(): returns the head of the list if the CR is the max uint256 value', async () => {
    ...
    const ICR_hintAddress_Max = await troveManagerHelpers.getNominalICR(ZERO_ADDRESS, hintAddress)
    ...
    const ICR_FirstTrove = await troveManagerHelpers.getNominalICR(ZERO_ADDRESS, firstTrove)
  ...
  it('getApproxHint(): returns the tail of the list if the CR is lower than ICR of any Trove', async () => {
    ...
    const ICR_hintAddress_Min = await troveManagerHelpers.getNominalICR(ZERO_ADDRESS, hintAddress)
    ...
    const ICR_LastTrove = await troveManagerHelpers.getNominalICR(ZERO_ADDRESS, lastTrove)
