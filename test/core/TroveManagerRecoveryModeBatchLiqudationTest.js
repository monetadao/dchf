...
contract('TroveManager - in Recovery Mode - back to normal mode in 1 tx', async (accounts) => {
  ...
  context('Batch liquidations', () => {
    ...
    it('Two troves over MCR are liquidated', async () => {
      ...
      assert.equal((await troveManagerHelpers.Troves(alice, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX], '3')
      ...
      assert.equal((await troveManagerHelpers.Troves(alice, erc20.address))[th.TROVE_STATUS_INDEX], '3')
    ...
    it('Stability Pool profit matches', async () => {
      ...
      const realGainInDCHF = spEthAfter
        .sub(spEthBefore)
        .mul(price)
        .div(mv._1e18BN)
        .sub(spDCHFBefore.sub(spMONfter))

      const expectedGainInDCHF_Asset = expectedCollateralLiquidatedA_Asset
        .mul(price)
        .div(mv._1e18BN)
        .sub(A_totalDebt_Asset)
      const realGainInDCHF_Asset = spEthAfter_Asset
        .sub(spEthBefore_Asset)
        .mul(price)
        .div(mv._1e18BN)
        .sub(spDCHFBefore_Asset.sub(spMONfter_Asset))
      ...
      assert.equal(
        spDCHFBefore.sub(spMONfter).toString(),
        A_totalDebt.toString(),
        'Stability Pool DCHF doesn’t match'
      )
      assert.equal(
        realGainInDCHF.toString(),
        expectedGainInDCHF.toString(),
      ...
      assert.equal(
        spDCHFBefore_Asset.sub(spMONfter_Asset).toString(),
        A_totalDebt_Asset.toString(),
        'Stability Pool DCHF doesn’t match'
      )
      assert.equal(
        realGainInDCHF_Asset.toString(),
        expectedGainInDCHF_Asset.toString(),
        'Stability Pool gains don’t match'
      )
    })
    ...
    it('A trove over TCR is not liquidated', async () => {
      ...
      assert.equal((await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_STATUS_INDEX], '3')
      assert.equal((await troveManagerHelpers.Troves(alice, erc20.address))[th.TROVE_STATUS_INDEX], '1')
      assert.equal((await troveManagerHelpers.Troves(carol, erc20.address))[th.TROVE_STATUS_INDEX], '1')
    })
  })
