...
/* NOTE: Some of the borrowing tests do not test for specific DCHF fee values. They only test that the
 * fees are non-zero when they should occur, and that they decay over time.
 *
 * Specific DCHF fee values will depend on the final fee schedule used, and the final choice for
...
contract('BorrowerOperations', async (accounts) => {
  ...
  let DCHF_GAS_COMPENSATION_ERC20
  ...
  const testCorpus = ({ withProxy = false }) => {
    beforeEach(async () => {
      ...
      DCHF_GAS_COMPENSATION = await dfrancParams.DCHF_GAS_COMPENSATION(ZERO_ADDRESS)
      ...
      DCHF_GAS_COMPENSATION_ERC20 = await dfrancParams.DCHF_GAS_COMPENSATION(erc20.address)
    ...
    it('addColl(): reverts when top-up would leave trove with ICR < MCR', async () => {
      ...
      assert.isFalse(await troveManagerHelpers.checkRecoveryMode(ZERO_ADDRESS, price))
      assert.isTrue(
        (await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, alice, price)).lt(toBN(dec(110, 16)))
      )
      assert.isFalse(await troveManagerHelpers.checkRecoveryMode(erc20.address, price))
      assert.isTrue(
        (await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)).lt(toBN(dec(110, 16)))
      )

      const collTopUp = 1 // 1 wei top up

      await assertRevert(
        borrowerOperations.addColl(ZERO_ADDRESS, collTopUp, alice, alice, { from: alice, value: collTopUp }),
    ...
    it('addColl(): Increases the activePool ETH and raw ether balance by correct amount', async () => {
      ...
      const { collateral: aliceCollAsset } = await openTrove({
        asset: erc20.address,
        ICR: toBN(dec(3, 18)),
        extraParams: { from: alice },
      })
      ...
      const activePool_RawEther_Before_Asset = toBN(await erc20.balanceOf(activePool.address)).mul(
        toBN(10 ** 10)
      )
      ...
      await borrowerOperations.addColl(ZERO_ADDRESS, dec(1, 'ether'), alice, alice, {
        from: alice,
        value: dec(1, 'ether'),
      })
      ...
      assert.isTrue(
        activePool_RawEther_After_Asset.eq(aliceCollAsset.div(toBN(10 ** 10)).add(toBN(dec(1, 8))))
      )
    })
    ...
    it('addColl(), active Trove: adds the correct collateral amount to the Trove', async () => {
      ...
      const alice_Trove_Before = await troveManagerHelpers.Troves(alice, ZERO_ADDRESS)
      const coll_before = alice_Trove_Before[th.TROVE_COLL_INDEX]
      const status_Before = alice_Trove_Before[th.TROVE_STATUS_INDEX]

      const alice_Trove_Before_Asset = await troveManagerHelpers.Troves(alice, erc20.address)
    ...
    it('addColl(), active Trove: updates the stake and updates the total stakes', async () => {
      ...
      const totalStakes_Before = await troveManagerHelpers.totalStakes(ZERO_ADDRESS)

      const alice_Trove_Before_Asset = await troveManagerHelpers.Troves(alice, erc20.address)
      const alice_Stake_Before_Asset = alice_Trove_Before_Asset[th.TROVE_STAKE_INDEX]
      const totalStakes_Before_Asset = await troveManagerHelpers.totalStakes(erc20.address)
      ...
      await borrowerOperations.addColl(ZERO_ADDRESS, dec(2, 'ether'), alice, alice, {
        from: alice,
        value: dec(2, 'ether'),
      })
      ...
      const totalStakes_After = await troveManagerHelpers.totalStakes(ZERO_ADDRESS)

      const alice_Trove_After_Asset = await troveManagerHelpers.Troves(alice, erc20.address)
      const alice_Stake_After_Asset = alice_Trove_After_Asset[th.TROVE_STAKE_INDEX]
      const totalStakes_After_Asset = await troveManagerHelpers.totalStakes(erc20.address)
    ...
    it("addColl(), active Trove: applies pending rewards and updates user's L_ETH, L_DCHFDebt snapshots", async () => {
      // --- SETUP ---
      ...
      const { collateral: aliceCollBeforeAsset, totalDebt: aliceDebtBeforeAsset } = await openTrove({
        asset: erc20.address,
        extraDCHFAmount: toBN(dec(15000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      })
      const { collateral: bobCollBeforeAsset, totalDebt: bobDebtBeforeAsset } = await openTrove({
        asset: erc20.address,
        extraDCHFAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: bob },
      ...
      // price drops to 1ETH:100DCHF, reducing Carol's ICR below MCR
      ...
      const L_ETH = await troveManagerHelpers.L_ASSETS(ZERO_ADDRESS)
      const L_DCHFDebt = await troveManagerHelpers.L_DCHFDebts(ZERO_ADDRESS)

      const L_Asset = await troveManagerHelpers.L_ASSETS(erc20.address)
      const L_DCHFDebt_Asset = await troveManagerHelpers.L_DCHFDebts(erc20.address)
      ...
      const alice_DCHFDebtRewardSnapshot_Before = alice_rewardSnapshot_Before[1]
      ...
      const alice_DCHFDebtRewardSnapshot_Before_Asset = alice_rewardSnapshot_Before_Asset[1]
      ...
      const bob_DCHFDebtRewardSnapshot_Before = bob_rewardSnapshot_Before[1]
      ...
      const bob_DCHFDebtRewardSnapshot_Before_Asset = bob_rewardSnapshot_Before_Asset[1]

      assert.equal(alice_ETHrewardSnapshot_Before, 0)
      assert.equal(alice_DCHFDebtRewardSnapshot_Before, 0)
      assert.equal(bob_ETHrewardSnapshot_Before, 0)
      assert.equal(bob_DCHFDebtRewardSnapshot_Before, 0)

      assert.equal(alice_ETHrewardSnapshot_Before_Asset, 0)
      assert.equal(alice_DCHFDebtRewardSnapshot_Before_Asset, 0)
      assert.equal(bob_ETHrewardSnapshot_Before_Asset, 0)
      assert.equal(bob_DCHFDebtRewardSnapshot_Before_Asset, 0)

      const alicePendingETHReward = await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, alice)
      const bobPendingETHReward = await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, bob)
      const alicePendingDCHFDebtReward = await troveManagerHelpers.getPendingDCHFDebtReward(
        ZERO_ADDRESS,
        alice
      )
      const bobPendingDCHFDebtReward = await troveManagerHelpers.getPendingDCHFDebtReward(ZERO_ADDRESS, bob)

      const alicePendingETHRewardAsset = await troveManagerHelpers.getPendingAssetReward(erc20.address, alice)
      const bobPendingETHRewardAsset = await troveManagerHelpers.getPendingAssetReward(erc20.address, bob)
      const alicePendingDCHFDebtRewardAsset = await troveManagerHelpers.getPendingDCHFDebtReward(
        erc20.address,
        alice
      )
      const bobPendingDCHFDebtRewardAsset = await troveManagerHelpers.getPendingDCHFDebtReward(
        erc20.address,
        bob
      )
      ...
      for (reward of [
        ...
        bobPendingETHReward,
        alicePendingDCHFDebtReward,
        bobPendingDCHFDebtReward,
        alicePendingETHRewardAsset,
        bobPendingETHRewardAsset,
        alicePendingDCHFDebtRewardAsset,
        bobPendingDCHFDebtRewardAsset,
      ]) {
      ...
      await borrowerOperations.addColl(ZERO_ADDRESS, aliceTopUp, alice, alice, {
        from: alice,
        value: aliceTopUp,
      })
      ...
      assert.isTrue(aliceNewDebt.eq(aliceDebtBefore.add(alicePendingDCHFDebtReward)))
      assert.isTrue(bobNewColl.eq(bobCollBefore.add(bobPendingETHReward).add(bobTopUp)))
      assert.isTrue(bobNewDebt.eq(bobDebtBefore.add(bobPendingDCHFDebtReward)))

      assert.isTrue(
        aliceNewColl_Asset.eq(aliceCollBeforeAsset.add(alicePendingETHRewardAsset).add(aliceTopUp))
      )
      assert.isTrue(aliceNewDebt_Asset.eq(aliceDebtBeforeAsset.add(alicePendingDCHFDebtRewardAsset)))
      assert.isTrue(bobNewColl_Asset.eq(bobCollBeforeAsset.add(bobPendingETHRewardAsset).add(bobTopUp)))
      assert.isTrue(bobNewDebt_Asset.eq(bobDebtBeforeAsset.add(bobPendingDCHFDebtRewardAsset)))

      /* Check that both Alice and Bob's snapshots of the rewards-per-unit-staked metrics should be updated
       to the latest values of L_ETH and L_DCHFDebt */
      ...
      const alice_DCHFDebtRewardSnapshot_After = alice_rewardSnapshot_After[1]
      ...
      const alice_DCHFDebtRewardSnapshot_After_Asset = alice_rewardSnapshot_After_Asset[1]
      ...
      const bob_DCHFDebtRewardSnapshot_After = bob_rewardSnapshot_After[1]
      ...
      const bob_DCHFDebtRewardSnapshot_After_Asset = bob_rewardSnapshot_After_Asset[1]

      assert.isAtMost(th.getDifference(alice_ETHrewardSnapshot_After, L_ETH), 100)
      assert.isAtMost(th.getDifference(alice_DCHFDebtRewardSnapshot_After, L_DCHFDebt), 100)
      assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot_After, L_ETH), 100)
      assert.isAtMost(th.getDifference(bob_DCHFDebtRewardSnapshot_After, L_DCHFDebt), 100)

      assert.isAtMost(th.getDifference(alice_ETHrewardSnapshot_After_Asset, L_Asset), 100)
      assert.isAtMost(th.getDifference(alice_DCHFDebtRewardSnapshot_After_Asset, L_DCHFDebt_Asset), 100)
      assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot_After_Asset, L_Asset), 100)
      assert.isAtMost(th.getDifference(bob_DCHFDebtRewardSnapshot_After_Asset, L_DCHFDebt_Asset), 100)
    })
    ...
    it('addColl(), reverts if trove is non-existent or closed', async () => {
      ...
      try {
        const txCarol = await borrowerOperations.addColl(ZERO_ADDRESS, dec(1, 'ether'), carol, carol, {
          from: carol,
          value: dec(1, 'ether'),
        })
      ...
      // Carol attempts to add collateral to her non-existent trove
      try {
        const txCarol = await borrowerOperations.addColl(erc20.address, dec(1, 'ether'), carol, carol, {
          from: carol,
        })
        assert.isFalse(txCarol.receipt.status)
      } catch (error) {
      ...
      try {
        const txBob = await borrowerOperations.addColl(ZERO_ADDRESS, dec(1, 'ether'), bob, bob, {
          from: bob,
          value: dec(1, 'ether'),
        })
      ...
      // Bob attempts to add collateral to his closed trove
      try {
        const txBob = await borrowerOperations.addColl(erc20.address, dec(1, 'ether'), bob, bob, {
          from: bob,
        })
        assert.isFalse(txBob.receipt.status)
      } catch (error) {
    ...
    it('addColl(): can add collateral in Recovery Mode', async () => {
      ...
      await borrowerOperations.addColl(ZERO_ADDRESS, collTopUp, alice, alice, {
        from: alice,
        value: collTopUp,
      })
      ...
      const aliceCollAssetAfter = (await troveManagerHelpers.Troves(alice, erc20.address))[
        th.TROVE_COLL_INDEX
      ]
    ...
    it('withdrawColl(): reverts when withdrawal would leave trove with ICR < MCR', async () => {
      ...
      await assertRevert(
        borrowerOperations.withdrawColl(ZERO_ADDRESS, 1, alice, alice, { from: alice }),
        'BorrowerOps: An operation that would result in ICR < MCR is not permitted'
      )
    ...
    it('withdrawColl(): reverts when calling address does not have active trove', async () => {
      ...
      const txBob = await borrowerOperations.withdrawColl(ZERO_ADDRESS, dec(100, 'finney'), bob, bob, {
        from: bob,
      })
      ...
      try {
        const txCarol = await borrowerOperations.withdrawColl(ZERO_ADDRESS, dec(1, 'ether'), carol, carol, {
          from: carol,
        })
        assert.isFalse(txCarol.receipt.status)
      } catch (err) {
      ...
      // Carol with no active trove attempts to withdraw
      try {
        const txCarolAsset = await borrowerOperations.withdrawColl(
          erc20.address,
          dec(1, 'ether'),
          carol,
          carol,
    ...
    it('withdrawColl(): reverts when system is in Recovery Mode', async () => {
      ...
      const txAliceAsset = await borrowerOperations.withdrawColl(erc20.address, 1000, alice, alice, {
        from: alice,
      })
    ...
    it("withdrawColl(): reverts when requested ETH withdrawal is > the trove's collateral", async () => {
      ...
      try {
        const txBob = await borrowerOperations.withdrawColl(ZERO_ADDRESS, bobColl.add(toBN(1)), bob, bob, {
          from: bob,
        })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
      ...
      try {
        const txBobAsset = await borrowerOperations.withdrawColl(
          erc20.address,
    ...
    it('withdrawColl(): reverts if system is in Recovery Mode', async () => {
      ...
      // price drops to 1ETH:150DCHF, reducing TCR below 150%
      ...
      try {
        const txDataAsset = await borrowerOperations.withdrawColl(erc20.address, '1', alice, alice, {
          from: alice,
        })
    ...
    it('withdrawColl(): doesn’t allow a user to completely withdraw all collateral from their Trove (due to gas compensation)', async () => {
      ...
      const aliceColl = (await troveManagerHelpers.getEntireDebtAndColl(ZERO_ADDRESS, alice))[
        th.TROVE_COLL_INDEX
      ]
      const aliceCollAsset = (await troveManagerHelpers.getEntireDebtAndColl(erc20.address, alice))[
        th.TROVE_COLL_INDEX
      ]
    ...
    it('withdrawColl(): sends the correct amount of ETH to the user', async () => {
      ...
      await borrowerOperations.withdrawColl(ZERO_ADDRESS, dec(1, 'ether'), alice, alice, {
        from: alice,
        gasPrice: 0,
      })
    ...
    it("withdrawColl(): applies pending rewards and updates user's L_ETH, L_DCHFDebt snapshots", async () => {
      // --- SETUP ---
      ...
      // close Carol's Trove, liquidating her 1 ether and 180DCHF.
      await troveManager.liquidate(ZERO_ADDRESS, carol, { from: owner })
      await troveManager.liquidate(erc20.address, carol, { from: owner })
      ...
      const pendingCollReward_A = await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, alice)
      const pendingDebtReward_A = await troveManagerHelpers.getPendingDCHFDebtReward(ZERO_ADDRESS, alice)
      const pendingCollReward_B = await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, bob)
      const pendingDebtReward_B = await troveManagerHelpers.getPendingDCHFDebtReward(ZERO_ADDRESS, bob)
      for (reward of [pendingCollReward_A, pendingDebtReward_A, pendingCollReward_B, pendingDebtReward_B]) {
        assert.isTrue(reward.gt(toBN('0')))
      }

      const pendingCollReward_A_Asset = await troveManagerHelpers.getPendingAssetReward(erc20.address, alice)
      const pendingDebtReward_A_Asset = await troveManagerHelpers.getPendingDCHFDebtReward(
        erc20.address,
        alice
      )
      const pendingCollReward_B_Asset = await troveManagerHelpers.getPendingAssetReward(erc20.address, bob)
      const pendingDebtReward_B_Asset = await troveManagerHelpers.getPendingDCHFDebtReward(erc20.address, bob)
      for (reward of [
      ...
      th.assertIsApproximatelyEqual(
        aliceDebtAfter_Asset,
        aliceDebtBefore_Asset.add(pendingDebtReward_A_Asset),
        10000
      )
      th.assertIsApproximatelyEqual(
        bobCollAfter_Asset,
      ...
      th.assertIsApproximatelyEqual(
        bobDebtAfter_Asset,
        bobDebtBefore_Asset.add(pendingDebtReward_B_Asset),
        10000
      )
    ...
    it('withdrawDCHF(): reverts when withdrawal would leave trove with ICR < MCR', async () => {
      // alice creates a Trove and adds first collateral
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: bob } })
      ...
      const DCHFwithdrawal = 1 // withdraw 1 wei DCHF

      await assertRevert(
        borrowerOperations.withdrawDCHF(ZERO_ADDRESS, th._100pct, DCHFwithdrawal, alice, alice, {
          from: alice,
        }),
        'BorrowerOps: An operation that would result in ICR < MCR is not permitted'
      )
      ...
      await assertRevert(
        borrowerOperations.withdrawDCHF(erc20.address, th._100pct, DCHFwithdrawal, alice, alice, {
          from: alice,
        }),
        'BorrowerOps: An operation that would result in ICR < MCR is not permitted'
      )
    ...
    it('withdrawDCHF(): decays a non-zero base rate', async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ asset: erc20.address, ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      await openTrove({ extraDCHFAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDCHFAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDCHFAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraDCHFAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })

      await openTrove({
        asset: erc20.address,
        extraDCHFAmount: toBN(dec(20, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
      })
      await openTrove({
      ...
      const A_DCHFBal = await dchfToken.balanceOf(A)
      await troveManagerHelpers.setBaseRate(ZERO_ADDRESS, dec(5, 16))
      await troveManagerHelpers.setBaseRate(erc20.address, dec(5, 16))
      ...
      // D withdraws DCHF
      await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, th._100pct, dec(1, 18), A, A, { from: D })
      await borrowerOperations.withdrawDCHF(erc20.address, th._100pct, dec(1, 18), A, A, { from: D })
      ...
      // E withdraws DCHF
      await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, th._100pct, dec(1, 18), A, A, { from: E })
      await borrowerOperations.withdrawDCHF(erc20.address, th._100pct, dec(1, 18), A, A, { from: E })

      const baseRate_3 = await troveManagerHelpers.baseRate(ZERO_ADDRESS)
      assert.isTrue(baseRate_3.lt(baseRate_2))

      const baseRate_3_Asset = await troveManagerHelpers.baseRate(erc20.address)
      assert.isTrue(baseRate_3_Asset.lt(baseRate_2_Asset))
    })

    it('withdrawDCHF(): reverts if max fee > 100%', async () => {
      await openTrove({ extraDCHFAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDCHFAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDCHFAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraDCHFAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })

      await openTrove({
        asset: erc20.address,
        extraDCHFAmount: toBN(dec(10, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
      })
      await openTrove({
      ...
      await openTrove({
        asset: erc20.address,
        extraDCHFAmount: toBN(dec(40, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C },
      })
      await openTrove({
      ...
      await assertRevert(
        borrowerOperations.withdrawDCHF(ZERO_ADDRESS, dec(2, 18), dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )
      await assertRevert(
        borrowerOperations.withdrawDCHF(ZERO_ADDRESS, '1000000000000000001', dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )

      await assertRevert(
        borrowerOperations.withdrawDCHF(erc20.address, dec(2, 18), dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )
      await assertRevert(
        borrowerOperations.withdrawDCHF(erc20.address, '1000000000000000001', dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )
    })

    it('withdrawDCHF(): reverts if max fee < 0.5% in Normal mode', async () => {
      await openTrove({ extraDCHFAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDCHFAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDCHFAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraDCHFAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      ...
      await assertRevert(
        borrowerOperations.withdrawDCHF(ZERO_ADDRESS, 0, dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )
      await assertRevert(
        borrowerOperations.withdrawDCHF(ZERO_ADDRESS, 1, dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )
      await assertRevert(
        borrowerOperations.withdrawDCHF(ZERO_ADDRESS, '4999999999999999', dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )

      await assertRevert(
        borrowerOperations.withdrawDCHF(erc20.address, 0, dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )
      await assertRevert(
        borrowerOperations.withdrawDCHF(erc20.address, 1, dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )
      await assertRevert(
        borrowerOperations.withdrawDCHF(erc20.address, '4999999999999999', dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )
    })

    it('withdrawDCHF(): reverts if fee exceeds max fee percentage', async () => {
      await openTrove({ extraDCHFAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDCHFAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDCHFAmount: toBN(dec(70, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraDCHFAmount: toBN(dec(80, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({
        extraDCHFAmount: toBN(dec(180, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: E },
      })
      ...
      await openTrove({
        asset: erc20.address,
        extraDCHFAmount: toBN(dec(60, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
      })
      await openTrove({
      ...
      await openTrove({
        asset: erc20.address,
        extraDCHFAmount: toBN(dec(70, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C },
      })
      await openTrove({
        asset: erc20.address,
        extraDCHFAmount: toBN(dec(80, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: D },
      })
      await openTrove({
      ...
      await troveManagerHelpers.setBaseRate(ZERO_ADDRESS, dec(5, 16))
      await troveManagerHelpers.setLastFeeOpTimeToNow(ZERO_ADDRESS)

      let baseRate = await troveManagerHelpers.baseRate(ZERO_ADDRESS) // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))

      await troveManagerHelpers.setBaseRate(erc20.address, dec(5, 16))
      await troveManagerHelpers.setLastFeeOpTimeToNow(erc20.address)

      let baseRate_Asset = await troveManagerHelpers.baseRate(erc20.address) // expect 5% base rate
      ...
      await assertRevert(
        borrowerOperations.withdrawDCHF(ZERO_ADDRESS, lessThan5pct, dec(3, 18), A, A, { from: A }),
        'Fee exceeded provided maximum'
      )
      await assertRevert(
        borrowerOperations.withdrawDCHF(erc20.address, lessThan5pct, dec(3, 18), A, A, { from: A }),
        'Fee exceeded provided maximum'
      )

      baseRate = await troveManagerHelpers.baseRate(ZERO_ADDRESS) // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))

      baseRate_Asset = await troveManagerHelpers.baseRate(erc20.address) // expect 5% base rate
      assert.equal(baseRate_Asset, dec(5, 16))
      // Attempt with maxFee 1%
      await assertRevert(
        borrowerOperations.withdrawDCHF(ZERO_ADDRESS, dec(1, 16), dec(1, 18), A, A, { from: B }),
        'Fee exceeded provided maximum'
      )
      await assertRevert(
        borrowerOperations.withdrawDCHF(erc20.address, dec(1, 16), dec(1, 18), A, A, { from: B }),
        'Fee exceeded provided maximum'
      )
      ...
      await assertRevert(
        borrowerOperations.withdrawDCHF(ZERO_ADDRESS, dec(3754, 13), dec(1, 18), A, A, { from: C }),
        'Fee exceeded provided maximum'
      )
      await assertRevert(
        borrowerOperations.withdrawDCHF(erc20.address, dec(3754, 13), dec(1, 18), A, A, { from: C }),
        'Fee exceeded provided maximum'
      )
      ...
      await assertRevert(
        borrowerOperations.withdrawDCHF(ZERO_ADDRESS, dec(5, 15), dec(1, 18), A, A, { from: D }),
        'Fee exceeded provided maximum'
      )
      await assertRevert(
        borrowerOperations.withdrawDCHF(erc20.address, dec(5, 15), dec(1, 18), A, A, { from: D }),
        'Fee exceeded provided maximum'
      )
    })

    it('withdrawDCHF(): succeeds when fee is less than max fee percentage', async () => {
      await openTrove({ extraDCHFAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDCHFAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDCHFAmount: toBN(dec(70, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraDCHFAmount: toBN(dec(80, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      ...
      const tx1 = await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, moreThan5pct, dec(1, 18), A, A, {
        from: A,
      })
      const tx1_Asset = await borrowerOperations.withdrawDCHF(erc20.address, moreThan5pct, dec(1, 18), A, A, {
        from: A,
      })
      ...
      // Attempt with maxFee = 5%
      const tx2 = await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, dec(5, 16), dec(1, 18), A, A, {
        from: B,
      })
      const tx2_Asset = await borrowerOperations.withdrawDCHF(erc20.address, dec(5, 16), dec(1, 18), A, A, {
        from: B,
      })
      ...
      const tx3 = await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, dec(1, 17), dec(1, 18), A, A, {
        from: C,
      })
      const tx3_Asset = await borrowerOperations.withdrawDCHF(erc20.address, dec(1, 17), dec(1, 18), A, A, {
        from: C,
      })
      ...
      const tx4 = await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, dec(37659, 13), dec(1, 18), A, A, {
        from: D,
      })
      const tx4_Asset = await borrowerOperations.withdrawDCHF(
        erc20.address,
        dec(37659, 13),
        dec(1, 18),
        A,
      ...
      const tx5 = await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, dec(1, 18), dec(1, 18), A, A, {
        from: E,
      })
      const tx5_Asset = await borrowerOperations.withdrawDCHF(erc20.address, dec(1, 18), dec(1, 18), A, A, {
        from: E,
      })
    ...
    it("withdrawDCHF(): doesn't change base rate if it is already zero", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ asset: erc20.address, ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      await openTrove({ extraDCHFAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDCHFAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDCHFAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraDCHFAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraDCHFAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })

      await openTrove({
        asset: erc20.address,
        extraDCHFAmount: toBN(dec(30, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
      })
      await openTrove({
      ...
      await openTrove({
        asset: erc20.address,
        extraDCHFAmount: toBN(dec(50, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C },
      })
      await openTrove({
      ...
      // D withdraws DCHF
      await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, th._100pct, dec(37, 18), A, A, { from: D })
      await borrowerOperations.withdrawDCHF(erc20.address, th._100pct, dec(37, 18), A, A, { from: D })
      ...
      // E opens trove
      await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, th._100pct, dec(12, 18), A, A, { from: E })
      await borrowerOperations.withdrawDCHF(erc20.address, th._100pct, dec(12, 18), A, A, { from: E })
    ...
    it("withdrawDCHF(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ asset: erc20.address, ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, th._100pct, dec(1, 18), C, C, { from: C })
      await borrowerOperations.withdrawDCHF(erc20.address, th._100pct, dec(1, 18), C, C, { from: C })
    ...
    it("withdrawDCHF(): borrower can't grief the baseRate and stop it decaying by issuing debt at higher frequency than the decay granularity", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraDCHFAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDCHFAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDCHFAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it('withdrawDCHF(): borrowing at non-zero base rate sends DCHF fee to MON staking contract', async () => {
      ...
      // Check MON DCHF balance before == 0
      const MONStaking_DCHFBalance_Before = await dchfToken.balanceOf(monStaking.address)
      assert.equal(MONStaking_DCHFBalance_Before, '0')
      ...
      // D withdraws DCHF
      await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, th._100pct, dec(37, 18), C, C, { from: D })
      await borrowerOperations.withdrawDCHF(erc20.address, th._100pct, dec(37, 18), C, C, { from: D })

      // Check MON DCHF balance after has increased
      const MONStaking_DCHFBalance_After = await dchfToken.balanceOf(monStaking.address)
      assert.isTrue(MONStaking_DCHFBalance_After.gt(MONStaking_DCHFBalance_Before))
    })

    if (!withProxy) {
      // TODO: use rawLogs instead of logs
      it('withdrawDCHF(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct', async () => {
        // time fast-forwards 1 year, and multisig stakes 1 MON
        await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
        await monToken.approve(monStaking.address, dec(1, 18), { from: multisig })
        await monStaking.stake(dec(1, 18), { from: multisig })
        ...
        const withdrawalTx = await borrowerOperations.withdrawDCHF(
          ZERO_ADDRESS,
          th._100pct,
          toBN(dec(37, 18)),
          D,
        ...
        const withdrawalTx_Asset = await borrowerOperations.withdrawDCHF(
          erc20.address,
          th._100pct,
          toBN(dec(37, 18)),
          D,
        ...
        const emittedFee = toBN(th.getDCHFFeeFromDCHFBorrowingEvent(withdrawalTx))
        const emittedFee_Asset = toBN(th.getDCHFFeeFromDCHFBorrowingEvent(withdrawalTx_Asset))
        assert.isTrue(emittedFee.gt(toBN('0')))
        assert.isTrue(emittedFee_Asset.gt(toBN('0')))

        const newDebt = (await troveManagerHelpers.Troves(D, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]
        const newDebt_Asset = (await troveManagerHelpers.Troves(D, erc20.address))[th.TROVE_DEBT_INDEX]
        ...
        th.assertIsApproximatelyEqual(
          newDebt_Asset,
          D_debtBefore_Asset.add(withdrawal_D).add(emittedFee_Asset),
          10000
        )
      })
    }

    it('withdrawDCHF(): Borrowing at non-zero base rate increases the MON staking contract DCHF fees-per-unit-staked', async () => {
      // time fast-forwards 1 year, and multisig stakes 1 MON
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await monToken.approve(monStaking.address, dec(1, 18), { from: multisig })
      await monStaking.stake(dec(1, 18), { from: multisig })

      // Check MON contract DCHF fees-per-unit-staked is zero
      const F_DCHF_Before = await monStaking.F_DCHF()
      assert.equal(F_DCHF_Before, '0')
      ...
      // D withdraws DCHF
      await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, th._100pct, toBN(dec(37, 18)), D, D, { from: D })
      await borrowerOperations.withdrawDCHF(erc20.address, th._100pct, toBN(dec(37, 18)), D, D, { from: D })

      // Check MON contract DCHF fees-per-unit-staked has increased
      const F_DCHF_After = await monStaking.F_DCHF()
      assert.isTrue(F_DCHF_After.gt(F_DCHF_Before))
    })

    it('withdrawDCHF(): Borrowing at non-zero base rate sends requested amount to the user', async () => {
      // time fast-forwards 1 year, and multisig stakes 1 MON
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await monToken.approve(monStaking.address, dec(1, 18), { from: multisig })
      await monStaking.stake(dec(1, 18), { from: multisig })

      // Check MON Staking contract balance before == 0
      const MONStaking_DCHFBalance_Before = await dchfToken.balanceOf(monStaking.address)
      assert.equal(MONStaking_DCHFBalance_Before, '0')
      ...
      let D_DCHFBalanceBefore = await dchfToken.balanceOf(D)

      // D withdraws DCHF
      const D_DCHFRequest = toBN(dec(37, 18))
      await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, th._100pct, D_DCHFRequest, D, D, { from: D })

      // Check MON staking DCHF balance has increased
      let MONStaking_DCHFBalance_After = await dchfToken.balanceOf(monStaking.address)
      assert.isTrue(MONStaking_DCHFBalance_After.gt(MONStaking_DCHFBalance_Before))

      // Check D's DCHF balance now equals their initial balance plus request DCHF
      let D_DCHFBalanceAfter = await dchfToken.balanceOf(D)
      assert.isTrue(D_DCHFBalanceAfter.eq(D_DCHFBalanceBefore.add(D_DCHFRequest)))

      //Asset:
      D_DCHFBalanceBefore = await dchfToken.balanceOf(D)
      await borrowerOperations.withdrawDCHF(erc20.address, th._100pct, D_DCHFRequest, D, D, { from: D })

      MONStaking_DCHFBalance_After = await dchfToken.balanceOf(monStaking.address)
      assert.isTrue(MONStaking_DCHFBalance_After.gt(MONStaking_DCHFBalance_Before))

      D_DCHFBalanceAfter = await dchfToken.balanceOf(D)
      assert.isTrue(D_DCHFBalanceAfter.eq(D_DCHFBalanceBefore.add(D_DCHFRequest)))
    })

    it('withdrawDCHF(): Borrowing at zero base rate changes DCHF fees-per-unit-staked', async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraDCHFAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDCHFAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDCHFAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      // A artificially receives MON, then stakes it
      await monToken.unprotectedMint(A, dec(100, 18))
      ...
      // D withdraws DCHF
      await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, th._100pct, dec(37, 18), D, D, { from: D })

      // Check MON DCHF balance after > 0
      const F_DCHF_After = await monStaking.F_DCHF()
      assert.isTrue(F_DCHF_After.gt('0'))
    })

    it('withdrawDCHF(): Borrowing at zero base rate sends debt request to user', async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraDCHFAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDCHFAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDCHFAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      let D_DCHFBalanceAfter = await dchfToken.balanceOf(D)
      // Check D's trove debt == D's DCHF balance + liquidation reserve
      assert.isTrue(D_DCHFBalanceAfter.eq(D_DCHFBalanceBefore.add(D_DCHFRequest)))

      D_DCHFBalanceBefore = await dchfToken.balanceOf(D)
      await borrowerOperations.withdrawDCHF(erc20.address, th._100pct, dec(37, 18), D, D, { from: D })

      D_DCHFBalanceAfter = await dchfToken.balanceOf(D)
      assert.isTrue(D_DCHFBalanceAfter.eq(D_DCHFBalanceBefore.add(D_DCHFRequest)))
    })

    it('withdrawDCHF(): reverts when calling address does not have active trove', async () => {
      ...
      // Bob successfully withdraws DCHF
      const txBob = await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, th._100pct, dec(100, 18), bob, bob, {
        from: bob,
      })
      const txBob_Asset = await borrowerOperations.withdrawDCHF(
        erc20.address,
        th._100pct,
        dec(100, 18),
        bob,
      ...
      // Carol with no active trove attempts to withdraw DCHF
      try {
        const txCarol = await borrowerOperations.withdrawDCHF(
          ZERO_ADDRESS,
          th._100pct,
          dec(100, 18),
          carol,
      ...
      try {
        const txCarol_Asset = await borrowerOperations.withdrawDCHF(
          erc20.address,
          th._100pct,
          dec(100, 18),
          carol,
    ...
    it('withdrawDCHF(): reverts when requested withdrawal amount is zero DCHF', async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ asset: erc20.address, ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ asset: erc20.address, ICR: toBN(dec(2, 18)), extraParams: { from: bob } })

      // Bob successfully withdraws 1e-18 DCHF
      const txBob = await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, th._100pct, 1, bob, bob, {
        from: bob,
      })
      const txBob_Asset = await borrowerOperations.withdrawDCHF(erc20.address, th._100pct, 1, bob, bob, {
        from: bob,
      })
      assert.isTrue(txBob.receipt.status)
      assert.isTrue(txBob_Asset.receipt.status)

      // Alice attempts to withdraw 0 DCHF
      try {
        const txAlice = await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, th._100pct, 0, alice, alice, {
          from: alice,
        })
      ...
      try {
        const txAlice_Asset = await borrowerOperations.withdrawDCHF(
          erc20.address,
          th._100pct,
          0,
          alice,
    ...
    it('withdrawDCHF(): reverts when system is in Recovery Mode', async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
      ...
      // Withdrawal possible when recoveryMode == false
      const txAlice = await borrowerOperations.withdrawDCHF(
        ZERO_ADDRESS,
        th._100pct,
        dec(100, 18),
        alice,
      ...
      //Check DCHF withdrawal impossible when recoveryMode == true
      try {
        const txBob = await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, th._100pct, 1, bob, bob, {
          from: bob,
        })
    ...
    it("withdrawDCHF(): reverts when withdrawal would bring the trove's ICR < MCR", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      ...
      // Bob tries to withdraw DCHF that would bring his ICR < MCR
      try {
        const txBob = await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, th._100pct, 1, bob, bob, {
          from: bob,
        })
    ...
    it('withdrawDCHF(): reverts when a withdrawal would cause the TCR of the system to fall below the CCR', async () => {
      await priceFeed.setPrice(dec(100, 18))
      const price = await priceFeed.getPrice()
      ...
      // Bob attempts to withdraw 1 DCHF.
      // System TCR would be: ((3+3) * 100 ) / (200+201) = 600/401 = 149.62%, i.e. below CCR of 150%.
      try {
        const txBob = await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, th._100pct, dec(1, 18), bob, bob, {
          from: bob,
        })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
    ...
    it('withdrawDCHF(): reverts if system is in Recovery Mode', async () => {
      // --- SETUP ---
      await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: bob } })
      ...
      try {
        const txData = await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, th._100pct, '200', alice, alice, {
          from: alice,
        })
        assert.isFalse(txData.receipt.status)
      } catch (err) {
      ...
      try {
        const txData = await borrowerOperations.withdrawDCHF(erc20.address, th._100pct, '200', alice, alice, {
          from: alice,
        })
        assert.isFalse(txData.receipt.status)
      } catch (err) {
    ...
    it("withdrawDCHF(): increases the Trove's DCHF debt by the correct amount", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ asset: erc20.address, ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      await borrowerOperations.withdrawDCHF(
        ZERO_ADDRESS,
        th._100pct,
        await getNetBorrowingAmount(100, ZERO_ADDRESS),
        alice,
        alice,
        { from: alice }
      )
    ...
    it('withdrawDCHF(): increases DCHF debt in ActivePool by correct amount', async () => {
      await openTrove({
        ICR: toBN(dec(10, 18)),
      ...
      // check before
      const activePool_DCHF_Before = await activePool.getDCHFDebt(ZERO_ADDRESS)
      const activePool_DCHF_Before_Asset = await activePool.getDCHFDebt(erc20.address)
      assert.isTrue(activePool_DCHF_Before.eq(aliceDebtBefore))
      assert.isTrue(activePool_DCHF_Before_Asset.eq(aliceDebtBefore_Asset))

      await borrowerOperations.withdrawDCHF(
        ZERO_ADDRESS,
        th._100pct,
        await getNetBorrowingAmount(dec(10000, 18), ZERO_ADDRESS),
        alice,
        alice,
        { from: alice }
      )
      ...
      // check after
      const activePool_DCHF_After = await activePool.getDCHFDebt(ZERO_ADDRESS)
      const activePool_DCHF_After_Asset = await activePool.getDCHFDebt(erc20.address)
      th.assertIsApproximatelyEqual(activePool_DCHF_After, activePool_DCHF_Before.add(toBN(dec(10000, 18))))
      th.assertIsApproximatelyEqual(
        activePool_DCHF_After_Asset,
        activePool_DCHF_Before_Asset.add(toBN(dec(10000, 18)))
      )
    })

    it('withdrawDCHF(): increases user DCHFToken balance by correct amount', async () => {
      ...
      // check before
      let alice_DCHFTokenBalance_Before = await dchfToken.balanceOf(alice)
      assert.isTrue(alice_DCHFTokenBalance_Before.gt(toBN('0')))

      await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, th._100pct, dec(10000, 18), alice, alice, {
        from: alice,
      })

      let alice_DCHFTokenBalance_After = await dchfToken.balanceOf(alice)
      assert.isTrue(alice_DCHFTokenBalance_After.eq(alice_DCHFTokenBalance_Before.add(toBN(dec(10000, 18)))))
      alice_DCHFTokenBalance_Before = await dchfToken.balanceOf(alice)

      await borrowerOperations.withdrawDCHF(erc20.address, th._100pct, dec(10000, 18), alice, alice, {
        from: alice,
      })

      alice_DCHFTokenBalance_After = await dchfToken.balanceOf(alice)
      assert.isTrue(alice_DCHFTokenBalance_After.eq(alice_DCHFTokenBalance_Before.add(toBN(dec(10000, 18)))))
    })

    // --- repayDCHF() ---
    it('repayDCHF(): reverts when repayment would leave trove with ICR < MCR', async () => {
      // alice creates a Trove and adds first collateral
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: bob } })
      ...
      const DCHFRepayment = 1 // 1 wei repayment

      await assertRevert(
        borrowerOperations.repayDCHF(ZERO_ADDRESS, DCHFRepayment, alice, alice, { from: alice }),
        'BorrowerOps: An operation that would result in ICR < MCR is not permitted'
      )
      await assertRevert(
        borrowerOperations.repayDCHF(erc20.address, DCHFRepayment, alice, alice, { from: alice }),
        'BorrowerOps: An operation that would result in ICR < MCR is not permitted'
      )
    })

    it('repayDCHF(): Succeeds when it would leave trove with net debt >= minimum net debt', async () => {
      // Make the DCHF request 2 wei above min net debt to correct for floor division, and make net debt = min net debt + 1 wei
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
        await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN('2')), ZERO_ADDRESS),
        A,
        A,
      ...
      const repayTxA = await borrowerOperations.repayDCHF(ZERO_ADDRESS, 1, A, A, { from: A })
      const repayTxA_Asset = await borrowerOperations.repayDCHF(erc20.address, 1, A, A, { from: A })
      assert.isTrue(repayTxA.receipt.status)
      assert.isTrue(repayTxA_Asset.receipt.status)

      await borrowerOperations.openTrove(ZERO_ADDRESS, 0, th._100pct, dec(20, 25), B, B, {
        from: B,
        value: dec(100, 30),
      })
      await borrowerOperations.openTrove(erc20.address, dec(100, 30), th._100pct, dec(20, 25), B, B, {
        from: B,
      })

      const repayTxB = await borrowerOperations.repayDCHF(ZERO_ADDRESS, dec(19, 25), B, B, { from: B })
      const repayTxB_Asset = await borrowerOperations.repayDCHF(erc20.address, dec(19, 25), B, B, { from: B })
      assert.isTrue(repayTxB.receipt.status)
      assert.isTrue(repayTxB_Asset.receipt.status)
    })

    it('repayDCHF(): reverts when it would leave trove with net debt < minimum net debt', async () => {
      // Make the DCHF request 2 wei above min net debt to correct for floor division, and make net debt = min net debt + 1 wei
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
      ...
      const repayTxAPromise = borrowerOperations.repayDCHF(ZERO_ADDRESS, 2, A, A, { from: A })
      const repayTxAPromise_Asset = borrowerOperations.repayDCHF(erc20.address, 2, A, A, { from: A })
    ...
    it('repayDCHF(): reverts when calling address does not have active trove', async () => {
      await openTrove({
        extraDCHFAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      // Bob successfully repays some DCHF
      const txBob = await borrowerOperations.repayDCHF(ZERO_ADDRESS, dec(10, 18), bob, bob, { from: bob })
      const txBob_Asset = await borrowerOperations.repayDCHF(erc20.address, dec(10, 18), bob, bob, {
        from: bob,
      })
      assert.isTrue(txBob.receipt.status)
      assert.isTrue(txBob_Asset.receipt.status)

      // Carol with no active trove attempts to withdrawDCHF
      try {
        const txCarol = await borrowerOperations.repayDCHF(ZERO_ADDRESS, dec(10, 18), carol, carol, {
          from: carol,
        })
        assert.isFalse(txCarol.receipt.status)
      } catch (err) {
      ...
      try {
        const txCarol_Asset = await borrowerOperations.repayDCHF(erc20.address, dec(10, 18), carol, carol, {
          from: carol,
        })
        assert.isFalse(txCarol_Asset.receipt.status)
      } catch (err) {
    ...
    it('repayDCHF(): reverts when attempted repayment is > the debt of the trove', async () => {
      await openTrove({
        extraDCHFAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      try {
        const txAlice = await borrowerOperations.repayDCHF(
          ZERO_ADDRESS,
      ...
      try {
        const txAlice_Asset = await borrowerOperations.repayDCHF(
          erc20.address,
    ...
    //withdrawDCHF: reduces DCHF debt in Trove
    it("repayDCHF(): reduces the Trove's DCHF debt by the correct amount", async () => {
      await openTrove({
        extraDCHFAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      await borrowerOperations.repayDCHF(ZERO_ADDRESS, aliceDebtBefore.div(toBN(10)), alice, alice, {
        from: alice,
      }) // Repays 1/10 her debt
      await borrowerOperations.repayDCHF(erc20.address, aliceDebtBefore_Asset.div(toBN(10)), alice, alice, {
        from: alice,
      }) // Repays 1/10 her debt
    ...
    it('repayDCHF(): decreases DCHF debt in ActivePool by correct amount', async () => {
      await openTrove({
        extraDCHFAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      const activePool_DCHF_Before = await activePool.getDCHFDebt(ZERO_ADDRESS)
      const activePool_DCHF_Before_Asset = await activePool.getDCHFDebt(erc20.address)
      assert.isTrue(activePool_DCHF_Before.gt(toBN('0')))
      assert.isTrue(activePool_DCHF_Before_Asset.gt(toBN('0')))
      ...
      const activePool_DCHF_After_Asset = await activePool.getDCHFDebt(erc20.address)
      th.assertIsApproximatelyEqual(
        activePool_DCHF_After,
        activePool_DCHF_Before.sub(aliceDebtBefore.div(toBN(10)))
      )
      th.assertIsApproximatelyEqual(
        activePool_DCHF_After_Asset,
        activePool_DCHF_Before_Asset.sub(aliceDebtBefore_Asset.div(toBN(10)))
      )
    })

    it('repayDCHF(): decreases user DCHFToken balance by correct amount', async () => {
      await openTrove({
        extraDCHFAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      let alice_DCHFTokenBalance_After = await dchfToken.balanceOf(alice)
      th.assertIsApproximatelyEqual(
        alice_DCHFTokenBalance_After,
        alice_DCHFTokenBalance_Before.sub(aliceDebtBefore.div(toBN(10)))
      )
      alice_DCHFTokenBalance_Before = await dchfToken.balanceOf(alice)
      ...
      th.assertIsApproximatelyEqual(
        alice_DCHFTokenBalance_After,
        alice_DCHFTokenBalance_Before.sub(aliceDebtBefore_Asset.div(toBN(10)))
      )
    })

    it('repayDCHF(): can repay debt in Recovery Mode', async () => {
      await openTrove({
        extraDCHFAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      const tx = await borrowerOperations.repayDCHF(
        ZERO_ADDRESS,
      ...
      const tx_Asset = await borrowerOperations.repayDCHF(
        erc20.address,
    ...
    it('repayDCHF(): Reverts if borrower has insufficient DCHF balance to cover his debt repayment', async () => {
      await openTrove({
        extraDCHFAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      const bobBalBefore = await dchfToken.balanceOf(B)
      assert.isTrue(bobBalBefore.gt(toBN('0')))

      // Bob transfers all but 5 of his DCHF to Carol
      await dchfToken.transfer(C, bobBalBefore.sub(toBN(dec(5, 18))), { from: B })

      //Confirm B's DCHF balance has decreased to 5 DCHF
      const bobBalAfter = await dchfToken.balanceOf(B)

      assert.isTrue(bobBalAfter.eq(toBN(dec(5, 18))))

      // Bob tries to repay 6 DCHF
      const repayDCHFPromise_B = borrowerOperations.repayDCHF(ZERO_ADDRESS, toBN(dec(6, 18)), B, B, {
        from: B,
      })
      const repayDCHFPromise_B_Asset = borrowerOperations.repayDCHF(erc20.address, toBN(dec(6, 18)), B, B, {
        from: B,
      })

      await assertRevert(repayDCHFPromise_B, 'Caller doesnt have enough DCHF to make repayment')
      await assertRevert(repayDCHFPromise_B_Asset, 'Caller doesnt have enough DCHF to make repayment')
    })
    ...
    it('adjustTrove(): reverts when adjustment would leave trove with ICR < MCR', async () => {
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, DCHFRepayment, false, alice, alice, {
          from: alice,
          value: collTopUp,
        }),
        'BorrowerOps: An operation that would result in ICR < MCR is not permitted'
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(erc20.address, 0, th._100pct, 0, DCHFRepayment, false, alice, alice, {
          from: alice,
          value: collTopUp,
        }),
        'BorrowerOps: An operation that would result in ICR < MCR is not permitted'
    ...
    it('adjustTrove(): reverts if max fee < 0.5% in Normal mode', async () => {
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, 0, 0, dec(1, 18), true, A, A, {
          from: A,
          value: dec(2, 16),
        }),
        'Max fee percentage must be between 0.5% and 100%'
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, 1, 0, dec(1, 18), true, A, A, {
          from: A,
          value: dec(2, 16),
        }),
        'Max fee percentage must be between 0.5% and 100%'
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, '4999999999999999', 0, dec(1, 18), true, A, A, {
          from: A,
          value: dec(2, 16),
        }),
        'Max fee percentage must be between 0.5% and 100%'
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(erc20.address, dec(2, 16), 0, 0, dec(1, 18), true, A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )
      await assertRevert(
        borrowerOperations.adjustTrove(erc20.address, dec(2, 16), 1, 0, dec(1, 18), true, A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )
      await assertRevert(
    ...
    it('adjustTrove(): allows max fee < 0.5% in Recovery mode', async () => {
      ...
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, 0, 0, dec(1, 9), true, A, A, {
        from: A,
        value: dec(300, 18),
      })
      await borrowerOperations.adjustTrove(erc20.address, dec(300, 18), 0, 0, dec(1, 9), true, A, A, {
        from: A,
      })
      await priceFeed.setPrice(dec(1, 18))
      assert.isTrue(await th.checkRecoveryMode(contracts))
      assert.isTrue(await th.checkRecoveryMode(contracts, erc20.address))
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, 1, 0, dec(1, 9), true, A, A, {
        from: A,
        value: dec(30000, 18),
      })
      await borrowerOperations.adjustTrove(erc20.address, dec(30000, 18), 1, 0, dec(1, 9), true, A, A, {
        from: A,
      })
      await priceFeed.setPrice(dec(1, 16))
      assert.isTrue(await th.checkRecoveryMode(contracts))
      assert.isTrue(await th.checkRecoveryMode(contracts, erc20.address))
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, '4999999999999999', 0, dec(1, 9), true, A, A, {
        from: A,
    ...
    it('adjustTrove(): decays a non-zero base rate', async () => {
      ...
      // D adjusts trove
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, dec(37, 18), true, D, D, {
        from: D,
      })
      await borrowerOperations.adjustTrove(erc20.address, 0, th._100pct, 0, dec(37, 18), true, D, D, {
        from: D,
      })
      ...
      // E adjusts trove
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, dec(37, 15), true, E, E, {
        from: D,
      })
      await borrowerOperations.adjustTrove(erc20.address, 0, th._100pct, 0, dec(37, 15), true, E, E, {
        from: D,
      })
    ...
    it("adjustTrove(): doesn't decay a non-zero base rate when user issues 0 debt", async () => {
      ...
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, 0, false, D, D, {
        from: D,
        value: dec(1, 'ether'),
      })
      await borrowerOperations.adjustTrove(erc20.address, dec(1, 'ether'), th._100pct, 0, 0, false, D, D, {
        from: D,
      })
    ...
    it("adjustTrove(): doesn't change base rate if it is already zero", async () => {
      await openTrove({ extraDCHFAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraDCHFAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("adjustTrove(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      ...
      // Borrower C triggers a fee
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, dec(1, 18), true, C, C, {
        from: C,
      })
      await borrowerOperations.adjustTrove(erc20.address, 0, th._100pct, 0, dec(1, 18), true, C, C, {
        from: C,
      })
    ...
    it('adjustTrove(): borrowing at non-zero base rate sends DCHF fee to MON staking contract', async () => {
      // time fast-forwards 1 year, and multisig stakes 1 MON
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await monToken.approve(monStaking.address, dec(1, 18), { from: multisig })
      await monStaking.stake(dec(1, 18), { from: multisig })
      ...
      // D adjusts trove
      await openTrove({ extraDCHFAmount: toBN(dec(37, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({
        asset: erc20.address,
        extraDCHFAmount: toBN(dec(37, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: D },
      })
    ...
    if (!withProxy) {
      ...
      it('adjustTrove(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct', async () => {
        ...
        const adjustmentTx_Asset = await borrowerOperations.adjustTrove(
          erc20.address,
          0,
          th._100pct,
          0,
        ...
        const emittedFee = toBN(th.getDCHFFeeFromDCHFBorrowingEvent(adjustmentTx))
        assert.isTrue(emittedFee.gt(toBN('0')))

        const emittedFee_Asset = toBN(th.getDCHFFeeFromDCHFBorrowingEvent(adjustmentTx_Asset))
        assert.isTrue(emittedFee_Asset.gt(toBN('0')))

        const D_newDebt = (await troveManagerHelpers.Troves(D, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]
        const D_newDebt_Asset = (await troveManagerHelpers.Troves(D, erc20.address))[th.TROVE_DEBT_INDEX]
    ...
    it('adjustTrove(): Borrowing at non-zero base rate increases the MON staking contract DCHF fees-per-unit-staked', async () => {
      // time fast-forwards 1 year, and multisig stakes 1 MON
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await monToken.approve(monStaking.address, dec(1, 18), { from: multisig })
      await monStaking.stake(dec(1, 18), { from: multisig })
      ...
      const F_DCHF_After_Asset = await monStaking.F_DCHF()
      assert.isTrue(F_DCHF_After_Asset.gt(F_DCHF_After))
    })
    ...
    it('adjustTrove(): Borrowing at non-zero base rate sends requested amount to the user', async () => {
      ...
      const D_DCHFBalanceBefore = await dchfToken.balanceOf(D)
      ...
      // D adjusts trove
      const DCHFRequest_D = toBN(dec(40, 18))
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, DCHFRequest_D, true, D, D, {
        from: D,
      })
      ...
      // Check D's DCHF balance has increased by their requested DCHF
      const D_DCHFBalanceAfter = await dchfToken.balanceOf(D)
      assert.isTrue(D_DCHFBalanceAfter.eq(D_DCHFBalanceBefore.add(DCHFRequest_D)))

      await borrowerOperations.adjustTrove(erc20.address, 0, th._100pct, 0, DCHFRequest_D, true, D, D, {
        from: D,
      })

      // Check MON staking DCHF balance has increased
      const MONStaking_DCHFBalance_After_Asset = await dchfToken.balanceOf(monStaking.address)
      assert.isTrue(MONStaking_DCHFBalance_After_Asset.gt(MONStaking_DCHFBalance_After))

      // Check D's DCHF balance has increased by their requested DCHF
      const D_DCHFBalanceAfter_Asset = await dchfToken.balanceOf(D)
      assert.isTrue(D_DCHFBalanceAfter_Asset.eq(D_DCHFBalanceAfter.add(DCHFRequest_D)))
    })

    it('adjustTrove(): Borrowing at zero base rate changes DCHF balance of MON staking contract', async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraDCHFAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDCHFAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDCHFAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      // Check staking DCHF balance before > 0
      const MONStaking_DCHFBalance_Before = await dchfToken.balanceOf(monStaking.address)
      assert.isTrue(MONStaking_DCHFBalance_Before.gt(toBN('0')))
    ...
    it('adjustTrove(): Borrowing at zero base rate changes MON staking contract DCHF fees-per-unit-staked', async () => {
      await openTrove({
      ...
      // Check staking DCHF balance before == 0
      const F_DCHF_Before = await monStaking.F_DCHF()
      assert.isTrue(F_DCHF_Before.eq(toBN('0')))
    ...
    it('adjustTrove(): Borrowing at zero base rate sends total requested DCHF to the user', async () => {
      await openTrove({
        extraDCHFAmount: toBN(dec(20000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: whale, value: toBN(dec(100, 'ether')) },
      ...
      const D_DCHFBalBefore = await dchfToken.balanceOf(D)
      // Check baseRate is zero
      const baseRate_1 = await troveManagerHelpers.baseRate(ZERO_ADDRESS)
      assert.equal(baseRate_1, '0')
      ...
      const DCHFBalanceAfter = await dchfToken.balanceOf(D)
      assert.isTrue(DCHFBalanceAfter.eq(D_DCHFBalBefore.add(DCHFRequest_D)))

      await borrowerOperations.adjustTrove(erc20.address, 0, th._100pct, 0, DCHFRequest_D, true, D, D, {
        from: D,
      })
      const DCHFBalanceAfter_Asset = await dchfToken.balanceOf(D)
      assert.isTrue(DCHFBalanceAfter_Asset.eq(DCHFBalanceAfter.add(DCHFRequest_D)))
    })
    ...
    it('adjustTrove(): reverts when calling address has no active trove', async () => {
      ...
      // Alice coll and debt increase(+1 ETH, +50DCHF)
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, dec(50, 18), true, alice, alice, {
        from: alice,
        value: dec(1, 'ether'),
      })
      await borrowerOperations.adjustTrove(
      ...
      try {
        const txCarolAsset = await borrowerOperations.adjustTrove(
          ZERO_ADDRESS,
          dec(1, 'ether'),
          th._100pct,
          0,
    ...
    it('adjustTrove(): reverts in Recovery Mode when the adjustment would reduce the TCR', async () => {
      ...
      const txAliceAsset = await borrowerOperations.adjustTrove(
        erc20.address,
        dec(1, 'ether'),
        th._100pct,
        0,
    ...
    it('adjustTrove(): collateral withdrawal reverts in Recovery Mode', async () => {
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 1, dec(5000, 18), false, alice, alice, {
          from: alice,
        }),
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(erc20.address, 0, th._100pct, 1, dec(5000, 18), false, alice, alice, {
          from: alice,
        }),
        'BorrowerOps: Collateral withdrawal not permitted Recovery Mode'
      )
    ...
    it('adjustTrove(): debt increase that would leave ICR < 150% reverts in Recovery Mode', async () => {
      ...
      const ICR_A = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, alice, price)
      const ICR_A_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)
      ...
      const newICR = await troveManagerHelpers.computeICR(
        aliceColl.add(collIncrease),
        aliceDebt.add(debtIncrease),
        price
      )
      const newICR_Asset = await troveManagerHelpers.computeICR(
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, debtIncrease, true, alice, alice, {
          from: alice,
          value: collIncrease,
        }),
    ...
    it('adjustTrove(): debt increase that would reduce the ICR reverts in Recovery Mode', async () => {
      ...
      const newICR_A = await troveManagerHelpers.computeICR(
        aliceColl.add(aliceCollIncrease),
        aliceDebt.add(aliceDebtIncrease),
        price
      )
      const newICR_A_Asset = await troveManagerHelpers.computeICR(
      ...
      const ICR_B = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price)
      const ICR_B_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)
      ...
      const newICR_B = await troveManagerHelpers.computeICR(
        bobColl.add(bobCollIncrease),
        bobDebt.add(bobDebtIncrease),
        price
      )
      const newICR_B_Asset = await troveManagerHelpers.computeICR(
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, bobDebtIncrease, true, bob, bob, {
          from: bob,
    ...
    it('adjustTrove(): A trove with ICR < CCR in Recovery Mode can adjust their trove to ICR > CCR', async () => {
      ...
      const tx_Asset = await borrowerOperations.adjustTrove(
        erc20.address,
        collIncrease,
        th._100pct,
        0,
      ...
      const actualNewICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, alice, price)
      assert.isTrue(actualNewICR.gt(CCR))

      const actualNewICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)
    ...
    it('adjustTrove(): A trove with ICR < CCR in Recovery Mode can adjust their trove to ICR > CCR But collateral is blocked from minting, then unblock it', async () => {
      ...
      await contracts.dchfToken.emergencyStopMinting(erc20.address, true)
      await contracts.dchfToken.emergencyStopMinting(ZERO_ADDRESS, true)
      ...
      await contracts.dchfToken.emergencyStopMinting(erc20.address, false)
      await contracts.dchfToken.emergencyStopMinting(ZERO_ADDRESS, false)
    ...
    it('adjustTrove(): A trove with ICR > CCR in Recovery Mode can improve their ICR', async () => {
      ...
      const initialICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, alice, price)
      const initialICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)
    ...
    it('adjustTrove(): debt increase in Recovery Mode charges no fee', async () => {
      ...
      // B stakes MON
      await monToken.unprotectedMint(bob, dec(100, 18))
      await monStaking.stake(dec(100, 18), { from: bob })

      const MONStakingDCHFBalanceBefore = await dchfToken.balanceOf(monStaking.address)
      assert.isTrue(MONStakingDCHFBalanceBefore.gt(toBN('0')))
      ...
      // Check emitted fee = 0
      const emittedFee = toBN(await th.getEventArgByName(txAlice, 'DCHFBorrowingFeePaid', '_DCHFFee'))
      assert.isTrue(emittedFee.eq(toBN('0')))

      const emittedFee_Asset = toBN(
        await th.getEventArgByName(txAlice_Asset, 'DCHFBorrowingFeePaid', '_DCHFFee')
      )
      ...
      const MONStakingDCHFBalanceAfter = await dchfToken.balanceOf(monStaking.address)
      assert.equal(MONStakingDCHFBalanceAfter.toString(), MONStakingDCHFBalanceBefore.toString())
    })
    ...
    it('adjustTrove(): reverts when change would cause the TCR of the system to fall below the CCR', async () => {
      ...
      try {
        const txBob_Asset = await borrowerOperations.adjustTrove(
          erc20.address,
          0,
          th._100pct,
          0,
    ...
    it('adjustTrove(): reverts when DCHF repaid is > debt of the trove', async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      const bobFee = toBN(await th.getEventArgByIndex(bobOpenTx, 'DCHFBorrowingFeePaid', 2))
      assert.isTrue(bobFee.gt(toBN('0')))

      const bobFee_Asset = toBN(await th.getEventArgByIndex(bobOpenTx_Asset, 'DCHFBorrowingFeePaid', 2))
      assert.isTrue(bobFee_Asset.gt(toBN('0')))

      // Alice transfers DCHF to bob to compensate borrowing fees
      await dchfToken.transfer(bob, bobFee, { from: alice })

      const remainingDebt = (await troveManagerHelpers.getTroveDebt(ZERO_ADDRESS, bob)).sub(
        DCHF_GAS_COMPENSATION
      )
      ...
      // Alice transfers DCHF to bob to compensate borrowing fees
      await dchfToken.transfer(bob, bobFee_Asset, { from: alice })

      const remainingDebt_Asset = (await troveManagerHelpers.getTroveDebt(erc20.address, bob)).sub(
        DCHF_GAS_COMPENSATION_ERC20
      )
    ...
    it('adjustTrove(): reverts when change would cause the ICR of the trove to fall below the MCR', async () => {
      ...
      // Bob attempts to increase debt by 100 DCHF and 1 ether, i.e. a change that constitutes a 100% ratio of coll:debt.
    ...
    it("adjustTrove(): With 0 coll change, doesnt change borrower's coll or ActivePool coll", async () => {
      ...
      // Alice adjusts trove. No coll change, and a debt increase (+50DCHF)
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, dec(50, 18), true, alice, alice, {
        from: alice,
        value: 0,
      })
      await borrowerOperations.adjustTrove(erc20.address, 0, th._100pct, 0, dec(50, 18), true, alice, alice, {
        from: alice,
      })
    ...
    it("adjustTrove(): With 0 debt change, doesnt change borrower's debt or ActivePool debt", async () => {
      ...
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      const activePoolDebtBefore = await activePool.getDCHFDebt(ZERO_ADDRESS)

      const aliceDebtBefore_Asset = await getTroveEntireDebt(alice, erc20.address)
      const activePoolDebtBefore_Asset = await activePool.getDCHFDebt(erc20.address)
      ...
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, 0, false, alice, alice, {
        from: alice,
        value: dec(1, 'ether'),
      })
      await borrowerOperations.adjustTrove(
      ...
      const aliceDebtAfter = await getTroveEntireDebt(alice)
      const activePoolDebtAfter = await activePool.getDCHFDebt(ZERO_ADDRESS)

      const aliceDebtAfter_Asset = await getTroveEntireDebt(alice, erc20.address)
      const activePoolDebtAfter_Asset = await activePool.getDCHFDebt(erc20.address)
    ...
    it("adjustTrove(): updates borrower's debt and coll with an increase in both", async () => {
      ...
      // Alice adjusts trove. Coll and debt increase(+1 ETH, +50DCHF)
      await borrowerOperations.adjustTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
        0,
        await getNetBorrowingAmount(dec(50, 18), ZERO_ADDRESS),
        true,
        alice,
        alice,
        { from: alice, value: dec(1, 'ether') }
    ...
    it("adjustTrove(): updates borrower's debt and coll with a decrease in both", async () => {
      ...
      // Alice adjusts trove coll and debt decrease (-0.5 ETH, -50DCHF)
      await borrowerOperations.adjustTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
    ...
    it("adjustTrove(): updates borrower's  debt and coll with coll increase, debt decrease", async () => {
      ...
      // Alice adjusts trove - coll increase and debt decrease (+0.5 ETH, -50DCHF)
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, dec(50, 18), false, alice, alice, {
        from: alice,
    ...
    it("adjustTrove(): updates borrower's debt and coll with coll decrease, debt increase", async () => {
      ...
      // Alice adjusts trove - coll decrease and debt increase (0.1 ETH, 10DCHF)
      await borrowerOperations.adjustTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
        dec(1, 17),
        await getNetBorrowingAmount(dec(1, 18), ZERO_ADDRESS),
        true,
        alice,
        alice,
        { from: alice }
    ...
    it("adjustTrove(): updates borrower's stake and totalStakes with a coll increase", async () => {
      ...
      const stakeBefore = await troveManagerHelpers.getTroveStake(ZERO_ADDRESS, alice)
      const totalStakesBefore = await troveManagerHelpers.totalStakes(ZERO_ADDRESS)
      assert.isTrue(stakeBefore.gt(toBN('0')))
      assert.isTrue(totalStakesBefore.gt(toBN('0')))

      const stakeBefore_Asset = await troveManagerHelpers.getTroveStake(erc20.address, alice)
      const totalStakesBefore_Asset = await troveManagerHelpers.totalStakes(erc20.address)
      assert.isTrue(stakeBefore_Asset.gt(toBN('0')))
      assert.isTrue(totalStakesBefore_Asset.gt(toBN('0')))

      // Alice adjusts trove - coll and debt increase (+1 ETH, +50 DCHF)
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, dec(50, 18), true, alice, alice, {
        from: alice,
        value: dec(1, 'ether'),
      })

      const stakeAfter = await troveManagerHelpers.getTroveStake(ZERO_ADDRESS, alice)
      const totalStakesAfter = await troveManagerHelpers.totalStakes(ZERO_ADDRESS)
      ...
      const stakeAfter_Asset = await troveManagerHelpers.getTroveStake(erc20.address, alice)
      const totalStakesAfter_Asset = await troveManagerHelpers.totalStakes(erc20.address)
    ...
    it('adjustTrove(): changes DCHFToken balance by the requested decrease', async () => {
      await openTrove({
        extraDCHFAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      ...
      const alice_DCHFTokenBalance_Before = await dchfToken.balanceOf(alice)
      assert.isTrue(alice_DCHFTokenBalance_Before.gt(toBN('0')))
      ...
      // check after
      const alice_DCHFTokenBalance_After = await dchfToken.balanceOf(alice)
      assert.isTrue(alice_DCHFTokenBalance_After.eq(alice_DCHFTokenBalance_Before.sub(toBN(dec(10, 18)))))
      ...
      // check after
      const alice_DCHFTokenBalance_After_Asset = await dchfToken.balanceOf(alice)
      assert.isTrue(
        alice_DCHFTokenBalance_After_Asset.eq(alice_DCHFTokenBalance_After.sub(toBN(dec(10, 18))))
      )
    })

    it('adjustTrove(): changes DCHFToken balance by the requested increase', async () => {
      await openTrove({
        extraDCHFAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      ...
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, dec(100, 18), true, alice, alice, {
        from: alice,
        value: dec(1, 'ether'),
      })
      ...
      // check after
      const alice_DCHFTokenBalance_After = await dchfToken.balanceOf(alice)
      assert.isTrue(alice_DCHFTokenBalance_After.eq(alice_DCHFTokenBalance_Before.add(toBN(dec(100, 18)))))
      ...
      const alice_DCHFTokenBalance_After_Asset = await dchfToken.balanceOf(alice)
      assert.isTrue(
        alice_DCHFTokenBalance_After_Asset.eq(alice_DCHFTokenBalance_After.add(toBN(dec(100, 18))))
      )
    })
    ...
    it('adjustTrove(): Changes the activePool ETH and raw ether balance by the requested decrease', async () => {
      ...
      assert.isTrue(
        activePool_RawEther_After_Asset.eq(
          activePool_ETH_Before_Asset.div(toBN(10 ** 10)).sub(toBN(dec(1, 7)))
        )
      )
    })
    ...
    it('adjustTrove(): Changes the activePool ETH and raw ether balance by the amount of ETH sent', async () => {
      ...
      assert.isTrue(
        activePool_RawEther_After_Asset.eq(
          activePool_ETH_Before_Asset.div(toBN(10 ** 10)).add(toBN(dec(1, 8)))
        )
      )
    })

    it('adjustTrove(): Changes the DCHF debt in ActivePool by requested decrease', async () => {
      await openTrove({
        extraDCHFAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      ...
      const activePooL_DCHFDebt_Before = await activePool.getDCHFDebt(ZERO_ADDRESS)
      assert.isTrue(activePooL_DCHFDebt_Before.gt(toBN('0')))

      const activePooL_DCHFDebt_Before_Asset = await activePool.getDCHFDebt(erc20.address)
      assert.isTrue(activePooL_DCHFDebt_Before_Asset.gt(toBN('0')))

      // Alice adjusts trove - coll increase and debt decrease
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, dec(30, 18), false, alice, alice, {
        from: alice,
        value: dec(1, 'ether'),
      })
      await borrowerOperations.adjustTrove(
      ...
      const activePooL_DCHFDebt_After = await activePool.getDCHFDebt(ZERO_ADDRESS)
      assert.isTrue(activePooL_DCHFDebt_After.eq(activePooL_DCHFDebt_Before.sub(toBN(dec(30, 18)))))

      const activePooL_DCHFDebt_After_Asset = await activePool.getDCHFDebt(erc20.address)
      assert.isTrue(
        activePooL_DCHFDebt_After_Asset.eq(activePooL_DCHFDebt_Before_Asset.sub(toBN(dec(30, 18))))
      )
    })

    it('adjustTrove(): Changes the DCHF debt in ActivePool by requested increase', async () => {
      await openTrove({
        extraDCHFAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      ...
      await borrowerOperations.adjustTrove(
        ...
        th._100pct,
        0,
        await getNetBorrowingAmount(dec(100, 18), ZERO_ADDRESS),
        true,
        alice,
        alice,
        { from: alice, value: dec(1, 'ether') }
      ...
      const activePooL_DCHFDebt_After = await activePool.getDCHFDebt(ZERO_ADDRESS)
      th.assertIsApproximatelyEqual(
        activePooL_DCHFDebt_After,
        activePooL_DCHFDebt_Before.add(toBN(dec(100, 18)))
      )

      const activePooL_DCHFDebt_After_Asset = await activePool.getDCHFDebt(erc20.address)
      th.assertIsApproximatelyEqual(
        activePooL_DCHFDebt_After_Asset,
        activePooL_DCHFDebt_Before_Asset.add(toBN(dec(100, 18)))
      )
    })
    ...
    it('adjustTrove(): new coll = 0 and new debt = 0 is not allowed, as gas compensation still counts toward ICR', async () => {
      ...
      const status_Before = await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, alice)
      ...
      const status_Before_Asset = await troveManagerHelpers.getTroveStatus(erc20.address, alice)
    ...
    it('adjustTrove(): Reverts if requested debt increase and amount is zero', async () => {
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, 0, true, alice, alice, {
          from: alice,
        }),
        'BorrowerOps: Debt increase requires non-zero debtChange'
      )
      await assertRevert(
        borrowerOperations.adjustTrove(erc20.address, 0, th._100pct, 0, 0, true, alice, alice, {
          from: alice,
        }),
        'BorrowerOps: Debt increase requires non-zero debtChange'
      )
    ...
    it('adjustTrove(): Reverts if it’s zero adjustment', async () => {
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, 0, false, alice, alice, {
          from: alice,
        }),
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(erc20.address, 0, th._100pct, 0, 0, false, alice, alice, {
          from: alice,
        }),
        'BorrowerOps: There must be either a collateral change or a debt change'
      )
    ...
    it('adjustTrove(): Reverts if borrower has insufficient DCHF balance to cover his debt repayment', async () => {
      await openTrove({
        extraDCHFAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      ...
      // Bob transfers some DCHF to carol
      await dchfToken.transfer(C, dec(10, 18), { from: B })

      //Confirm B's DCHF balance is less than 50 DCHF
      const B_DCHFBal = await dchfToken.balanceOf(B)
      assert.isTrue(B_DCHFBal.lt(bobDebt))

      const repayDCHFPromise_B = borrowerOperations.adjustTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
        0,
      ...
      await assertRevert(repayDCHFPromise_B, 'revert')

      await dchfToken.transfer(C, B_DCHFBal, { from: B })
      await openTrove({
        asset: erc20.address,
        extraDCHFAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
      ...
      const B_DCHFBal_Asset = await dchfToken.balanceOf(B)
      assert.isTrue(B_DCHFBal_Asset.lt(bobDebt_Asset))

      const repayDCHFPromise_B_Asset = borrowerOperations.adjustTrove(
        erc20.address,
        0,
        th._100pct,
        0,
      ...
      await assertRevert(repayDCHFPromise_B_Asset, 'revert')
    })
    ...
    it('closeTrove(): reverts when it would lower the TCR below CCR', async () => {
      ...
      await dchfToken.transfer(alice, dec(300, 18), { from: bob })
    ...
    it('closeTrove(): reverts when system is in Recovery Mode', async () => {
      ...
      // Alice transfers her DCHF to Bob and Carol so they can cover fees
      const aliceBal = await dchfToken.balanceOf(alice)
      await dchfToken.transfer(bob, aliceBal.div(toBN(2)), { from: alice })
      await dchfToken.transfer(carol, aliceBal.div(toBN(2)), { from: alice })
      ...
      await assertRevert(
        borrowerOperations.closeTrove(ZERO_ADDRESS, { from: carol }),
        'BorrowerOps: Operation not permitted during Recovery Mode'
      )
      await assertRevert(
        borrowerOperations.closeTrove(erc20.address, { from: carol }),
        'BorrowerOps: Operation not permitted during Recovery Mode'
      )
    })
    ...
    it('closeTrove(): reverts when trove is the only one in the system', async () => {
      ...
      await dchfToken.unprotectedMint(alice, dec(100000, 18))

      // Check she has more DCHF than her trove debt
      const aliceBal = await dchfToken.balanceOf(alice)
      const aliceDebt = await getTroveEntireDebt(alice)
    ...
    it("closeTrove(): reduces a Trove's collateral to zero", async () => {
      ...
      const aliceCollBefore = await getTroveEntireColl(alice)
      const dennisDCHF = await dchfToken.balanceOf(dennis)
      assert.isTrue(aliceCollBefore.gt(toBN('0')))
      assert.isTrue(dennisDCHF.gt(toBN('0')))

      const aliceCollBefore_Asset = await getTroveEntireColl(alice, erc20.address)
      assert.isTrue(aliceCollBefore_Asset.gt(toBN('0')))

      // To compensate borrowing fees
      await dchfToken.transfer(alice, dennisDCHF.div(toBN(2)), { from: dennis })
    ...
    it("closeTrove(): sets Trove's stake to zero", async () => {
      ...
      const stakeAfter = (await troveManagerHelpers.Troves(alice, ZERO_ADDRESS))[2].toString()
      assert.equal(stakeAfter, '0')

      const stakeAfter_Asset = (await troveManagerHelpers.Troves(alice, erc20.address))[2].toString()
    ...
    it("closeTrove(): zero's the troves reward snapshots", async () => {
      ...
      const L_DCHFDebt_Snapshot_A_AfterLiquidation = (
        await troveManagerHelpers.getRewardSnapshots(alice, ZERO_ADDRESS)
      )[1]
      ...
      const L_DCHFDebt_Snapshot_A_AfterLiquidation_Asset = (
        await troveManagerHelpers.getRewardSnapshots(alice, erc20.address)
      )[1]

      assert.isTrue(L_ETH_Snapshot_A_AfterLiquidation.gt(toBN('0')))
      assert.isTrue(L_DCHFDebt_Snapshot_A_AfterLiquidation.gt(toBN('0')))

      assert.isTrue(L_ETH_Snapshot_A_AfterLiquidation_Asset.gt(toBN('0')))
      assert.isTrue(L_DCHFDebt_Snapshot_A_AfterLiquidation_Asset.gt(toBN('0')))

      // to compensate borrowing fees
      await dchfToken.transfer(alice, await dchfToken.balanceOf(dennis), { from: dennis })
      ...
      const L_DCHFDebt_Snapshot_A_afterAliceCloses = (
        await troveManagerHelpers.getRewardSnapshots(alice, ZERO_ADDRESS)
      )[1]

      assert.equal(L_ETH_Snapshot_A_afterAliceCloses, '0')
      assert.equal(L_DCHFDebt_Snapshot_A_afterAliceCloses, '0')

      const L_ETH_Snapshot_A_afterAliceCloses_Asset = (
        await troveManagerHelpers.getRewardSnapshots(alice, erc20.address)
      )[0]
      const L_DCHFDebt_Snapshot_A_afterAliceCloses_Asset = (
        await troveManagerHelpers.getRewardSnapshots(alice, erc20.address)
      )[1]

      assert.equal(L_ETH_Snapshot_A_afterAliceCloses_Asset, '0')
      assert.equal(L_DCHFDebt_Snapshot_A_afterAliceCloses_Asset, '0')
    })
    ...
    it('closeTrove(): reduces ActivePool debt by correct amount', async () => {
      ...
      // Check after
      const activePool_Debt_After = (await activePool.getDCHFDebt(ZERO_ADDRESS)).toString()
      th.assertIsApproximatelyEqual(activePool_Debt_After, dennisDebt)
      const activePool_Debt_After_Asset = (await activePool.getDCHFDebt(erc20.address)).toString()
    ...
    it('closeTrove(): updates the the total stakes', async () => {
      ...
      assert.isTrue(
        totalStakesBefore_Asset.eq(
          aliceStakeBefore_Asset.add(bobStakeBefore_Asset).add(dennisStakeBefore_Asset)
        )
      )
    ...
    it("closeTrove(): subtracts the debt of the closed Trove from the Borrower's DCHFToken balance", async () => {
      await openTrove({
        extraDCHFAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: dennis },
      ...
      assert.isTrue(alice_DCHFBalance_Before.gt(toBN('0')))

      // close trove
      await borrowerOperations.closeTrove(ZERO_ADDRESS, { from: alice })
      await borrowerOperations.closeTrove(erc20.address, { from: alice })

      // check alice DCHF balance after
      const alice_DCHFBalance_After = await dchfToken.balanceOf(alice)
      const debtValue = aliceDebt.sub(DCHF_GAS_COMPENSATION)
      const debtValueERC20 = aliceDebt.sub(DCHF_GAS_COMPENSATION_ERC20)
      th.assertIsApproximatelyEqual(
        alice_DCHFBalance_After,
        alice_DCHFBalance_Before.sub(debtValue.add(debtValueERC20))
      )
    })
    ...
    it('closeTrove(): applies pending rewards', async () => {
      ...
      await dchfToken.transfer(bob, dec(10000, 18), { from: whale })
      ...
      const [liquidatedDebt_C_Asset, liquidatedColl_C_Asset] =
      ...
      const defaultPooL_DCHFDebt = await defaultPool.getDCHFDebt(ZERO_ADDRESS)

      const defaultPool_ETH_Asset = await defaultPool.getAssetBalance(erc20.address)
      const defaultPooL_DCHFDebt_Asset = await defaultPool.getDCHFDebt(erc20.address)

      // Carol's liquidated coll (1 ETH) and drawn debt should have entered the Default Pool
      assert.isAtMost(th.getDifference(defaultPool_ETH, liquidatedColl_C), 100)
      assert.isAtMost(th.getDifference(defaultPooL_DCHFDebt, liquidatedDebt_C), 100)

      assert.isAtMost(th.getDifference(defaultPool_ETH_Asset, liquidatedColl_C_Asset), 100)
      assert.isAtMost(th.getDifference(defaultPooL_DCHFDebt_Asset, liquidatedDebt_C_Asset), 100)
      ...
      const defaultPooL_DCHFDebt_afterAliceCloses = await defaultPool.getDCHFDebt(ZERO_ADDRESS)

      const defaultPool_ETH_afterAliceCloses_Asset = await defaultPool.getAssetBalance(erc20.address)
      const defaultPooL_DCHFDebt_afterAliceCloses_Asset = await defaultPool.getDCHFDebt(erc20.address)
      ...
      assert.isAtMost(
        th.getDifference(
          defaultPooL_DCHFDebt_afterAliceCloses,
          defaultPooL_DCHFDebt.sub(pendingDebtReward_A)
        ),
        1000
      )
      ...
      assert.isAtMost(
        th.getDifference(
          defaultPool_ETH_afterAliceCloses_Asset,
          defaultPool_ETH.sub(pendingCollReward_A_Asset)
        ),
        1000
      )
      assert.isAtMost(
        th.getDifference(
          defaultPooL_DCHFDebt_afterAliceCloses_Asset,
          defaultPooL_DCHFDebt.sub(pendingDebtReward_A_Asset)
        ),
        1000
      )

      // whale adjusts trove, pulling their rewards out of DefaultPool
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, dec(1, 18), true, whale, whale, {
        from: whale,
      })
      await borrowerOperations.adjustTrove(erc20.address, 0, th._100pct, 0, dec(1, 18), true, whale, whale, {
        from: whale,
      })
      ...
      const defaultPooL_DCHFDebt_afterBobCloses = await defaultPool.getDCHFDebt(ZERO_ADDRESS)

      const defaultPool_ETH_afterBobCloses_Asset = await defaultPool.getAssetBalance(erc20.address)
      const defaultPooL_DCHFDebt_afterBobCloses_Asset = await defaultPool.getDCHFDebt(erc20.address)

      assert.isAtMost(th.getDifference(defaultPool_ETH_afterBobCloses, 0), 100000)
      assert.isAtMost(th.getDifference(defaultPooL_DCHFDebt_afterBobCloses, 0), 100000)

      assert.isAtMost(th.getDifference(defaultPool_ETH_afterBobCloses_Asset, 0), 100000)
      assert.isAtMost(th.getDifference(defaultPooL_DCHFDebt_afterBobCloses_Asset, 0), 100000)
    })

    it('closeTrove(): reverts if borrower has insufficient DCHF balance to repay his entire debt', async () => {
      await openTrove({
        extraDCHFAmount: toBN(dec(15000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
      ...
      //Confirm Bob's DCHF balance is less than his trove debt
      const B_DCHFBal = await dchfToken.balanceOf(B)
      const B_troveDebt = await getTroveEntireDebt(B)

      assert.isTrue(B_DCHFBal.lt(B_troveDebt))

      const closeTrovePromise_B = borrowerOperations.closeTrove(ZERO_ADDRESS, { from: B })
      await assertRevert(closeTrovePromise_B, 'BorrowerOps: Caller doesnt have enough DCHF to make repayment')

      await dchfToken.transfer(C, await B_DCHFBal, { from: B })
      await openTrove({
        asset: erc20.address,
        extraDCHFAmount: toBN(dec(5000, 18)),
        ICR: toBN(dec(2, 18)),
    ...
    if (!withProxy) {
      ...
      it('openTrove(): emits a TroveUpdated event with the correct collateral and debt', async () => {
        ...
        const baseRateBefore = await troveManagerHelpers.baseRate(ZERO_ADDRESS)
        const baseRateBefore_Asset = await troveManagerHelpers.baseRate(erc20.address)
        ...
        assert.isTrue((await troveManagerHelpers.baseRate(ZERO_ADDRESS)).gt(baseRateBefore))
        assert.isTrue((await troveManagerHelpers.baseRate(erc20.address)).gt(baseRateBefore_Asset))
    ...
    it('openTrove(): Opens a trove with net debt >= minimum net debt', async () => {
      ...
      const txA = await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN(1)), ZERO_ADDRESS),
        A,
        A,
        { from: A, value: dec(100, 30) }
      )
      ...
      const txA_Asset = await borrowerOperations.openTrove(
        erc20.address,
        dec(100, 30),
        th._100pct,
        await getNetBorrowingAmount(MIN_NET_DEBT_ERC20.add(toBN(1)), ZERO_ADDRESS),
        A,
        A,
        { from: A }
      )
      ...
      const txC = await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN(dec(47789898, 22)), ZERO_ADDRESS)),
        A,
        A,
      ...
      const txC_Asset = await borrowerOperations.openTrove(
        ...
        dec(100, 30),
        th._100pct,
        await getNetBorrowingAmount(MIN_NET_DEBT_ERC20.add(toBN(dec(47789898, 22)), ZERO_ADDRESS)),
        A,
        A,
        { from: C }
      )
    ...
    it('openTrove(): reverts if net debt < minimum net debt', async () => {
      const txAPromise = borrowerOperations.openTrove(ZERO_ADDRESS, 0, th._100pct, 0, A, A, {
        from: A,
        value: dec(100, 30),
      })
      ...
      const txBPromise = borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getNetBorrowingAmount(MIN_NET_DEBT.sub(toBN(1)), ZERO_ADDRESS),
        B,
        B,
      ...
      const txAPromise_Asset = borrowerOperations.openTrove(
        erc20.address,
        dec(100, 30),
        th._100pct,
        0,
      ...
      const txBPromise_Asset = borrowerOperations.openTrove(
        erc20.address,
        dec(100, 30),
        th._100pct,
        await getNetBorrowingAmount(MIN_NET_DEBT_ERC20.sub(toBN(1)), ZERO_ADDRESS),
        B,
        B,
        { from: B }
      )
      ...
      const txCPromise_Asset = borrowerOperations.openTrove(
        erc20.address,
        dec(100, 30),
        th._100pct,
    ...
    it('openTrove(): decays a non-zero base rate', async () => {
      ...
      // E opens trove
      await openTrove({ extraDCHFAmount: toBN(dec(12, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({
        asset: erc20.address,
        extraDCHFAmount: toBN(dec(12, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: E },
      })
    ...
    it("openTrove(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      ...
      await openTrove({ extraDCHFAmount: toBN(dec(1, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({
        asset: erc20.address,
        extraDCHFAmount: toBN(dec(1, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: D },
      })
      ...
      await openTrove({ extraDCHFAmount: toBN(dec(1, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({
        asset: erc20.address,
        extraDCHFAmount: toBN(dec(1, 18)),
        ICR: toBN(dec(2, 18)),
    ...
    it('openTrove(): reverts if max fee > 100%', async () => {
      await assertRevert(
        borrowerOperations.openTrove(ZERO_ADDRESS, 0, dec(2, 18), dec(10000, 18), A, A, {
          from: A,
      ...
      await assertRevert(
        borrowerOperations.openTrove(ZERO_ADDRESS, 0, '1000000000000000001', dec(20000, 18), B, B, {
          from: B,
          value: dec(1000, 'ether'),
        }),
        'Max fee percentage must be between 0.5% and 100%'
      ...
      await assertRevert(
        borrowerOperations.openTrove(erc20.address, dec(1000, 'ether'), dec(2, 18), dec(10000, 18), A, A, {
          from: A,
        }),
        'Max fee percentage must be between 0.5% and 100%'
      )
    ...
    it('openTrove(): reverts if max fee < 0.5% in Normal mode', async () => {
      await assertRevert(
        borrowerOperations.openTrove(ZERO_ADDRESS, 0, 0, dec(195000, 18), A, A, {
          from: A,
      ...
      await assertRevert(
        borrowerOperations.openTrove(ZERO_ADDRESS, 0, 1, dec(195000, 18), A, A, {
          from: A,
          value: dec(1000, 'ether'),
        }),
        'Max fee percentage must be between 0.5% and 100%'
      ...
      await assertRevert(
        borrowerOperations.openTrove(ZERO_ADDRESS, 0, '4999999999999999', dec(195000, 18), B, B, {
          from: B,
          value: dec(1200, 'ether'),
        }),
        'Max fee percentage must be between 0.5% and 100%'
      ...
      await assertRevert(
        borrowerOperations.openTrove(erc20.address, dec(1200, 'ether'), 0, dec(195000, 18), A, A, {
          from: A,
        }),
        'Max fee percentage must be between 0.5% and 100%'
      )
      await assertRevert(
        borrowerOperations.openTrove(erc20.address, dec(1000, 'ether'), 1, dec(195000, 18), A, A, {
          from: A,
        }),
        'Max fee percentage must be between 0.5% and 100%'
      )
    ...
    it('openTrove(): allows max fee < 0.5% in Recovery Mode', async () => {
      await borrowerOperations.openTrove(ZERO_ADDRESS, 0, th._100pct, dec(195000, 18), A, A, {
        from: A,
      ...
      await borrowerOperations.openTrove(ZERO_ADDRESS, 0, 0, dec(19500, 18), B, B, {
        from: B,
        value: dec(3100, 'ether'),
      })
      await borrowerOperations.openTrove(erc20.address, dec(3100, 'ether'), 0, dec(19500, 18), B, B, {
        from: B,
      })
      await priceFeed.setPrice(dec(50, 18))
      assert.isTrue(await th.checkRecoveryMode(contracts))
      assert.isTrue(await th.checkRecoveryMode(contracts, erc20.address))
      await borrowerOperations.openTrove(ZERO_ADDRESS, 0, 1, dec(19500, 18), C, C, {
        from: C,
        value: dec(3100, 'ether'),
      })
      await borrowerOperations.openTrove(erc20.address, dec(3100, 'ether'), 1, dec(19500, 18), C, C, {
        from: C,
      })
      await priceFeed.setPrice(dec(25, 18))
      assert.isTrue(await th.checkRecoveryMode(contracts))
      assert.isTrue(await th.checkRecoveryMode(contracts, erc20.address))
      await borrowerOperations.openTrove(ZERO_ADDRESS, 0, '4999999999999999', dec(19500, 18), D, D, {
        from: D,
        value: dec(3100, 'ether'),
      })
      await borrowerOperations.openTrove(
    ...
    it('openTrove(): reverts if fee exceeds max fee percentage', async () => {
      ...
      let borrowingRate = await troveManagerHelpers.getBorrowingRate(ZERO_ADDRESS) // expect max(0.5 + 5%, 5%) rate
      assert.equal(borrowingRate, dec(5, 16))

      let borrowingRate_Asset = await troveManagerHelpers.getBorrowingRate(erc20.address) // expect max(0.5 + 5%, 5%) rate
      assert.equal(borrowingRate_Asset, dec(5, 16))

      const lessThan5pct = '49999999999999999'
      await assertRevert(
        borrowerOperations.openTrove(ZERO_ADDRESS, 0, lessThan5pct, dec(30000, 18), A, A, {
          from: D,
          value: dec(1000, 'ether'),
        }),
        'Fee exceeded provided maximum'
      ...
      await assertRevert(
        borrowerOperations.openTrove(erc20.address, dec(1000, 'ether'), lessThan5pct, dec(30000, 18), A, A, {
          from: D,
        }),
        'Fee exceeded provided maximum'
      )

      borrowingRate = await troveManagerHelpers.getBorrowingRate(ZERO_ADDRESS) // expect 5% rate
      assert.equal(borrowingRate, dec(5, 16))

      borrowingRate_Asset = await troveManagerHelpers.getBorrowingRate(erc20.address) // expect 5% rate
      assert.equal(borrowingRate_Asset, dec(5, 16))

      // Attempt with maxFee 1%
      await assertRevert(
        borrowerOperations.openTrove(ZERO_ADDRESS, 0, dec(1, 16), dec(30000, 18), A, A, {
          from: D,
          value: dec(1000, 'ether'),
        }),
        'Fee exceeded provided maximum'
      ...
      await assertRevert(
        borrowerOperations.openTrove(erc20.address, dec(1000, 'ether'), dec(1, 16), dec(30000, 18), A, A, {
          from: D,
        }),
        'Fee exceeded provided maximum'
      )
      ...
      borrowingRate_Asset = await troveManagerHelpers.getBorrowingRate(ZERO_ADDRESS) // expect 5% rate
      assert.equal(borrowingRate_Asset, dec(5, 16))
      // Attempt with maxFee 3.754%
      await assertRevert(
        borrowerOperations.openTrove(ZERO_ADDRESS, 0, dec(3754, 13), dec(30000, 18), A, A, {
          from: D,
          value: dec(1000, 'ether'),
        }),
        'Fee exceeded provided maximum'
      ...
      await assertRevert(
        borrowerOperations.openTrove(erc20.address, dec(1000, 'ether'), dec(3754, 13), dec(30000, 18), A, A, {
          from: D,
        }),
        'Fee exceeded provided maximum'
      )
      ...
      await assertRevert(
        borrowerOperations.openTrove(ZERO_ADDRESS, 0, dec(5, 15), dec(30000, 18), A, A, {
          from: D,
          value: dec(1000, 'ether'),
        }),
        'Fee exceeded provided maximum'
      ...
      await assertRevert(
        borrowerOperations.openTrove(erc20.address, dec(1000, 'ether'), dec(5, 15), dec(30000, 18), A, A, {
          from: D,
        }),
        'Fee exceeded provided maximum'
      )
    ...
    it('openTrove(): succeeds when fee is less than max fee percentage', async () => {
      ...
      let borrowingRate = await troveManagerHelpers.getBorrowingRate(ZERO_ADDRESS) // expect min(0.5 + 5%, 5%) rate
      assert.equal(borrowingRate, dec(5, 16))

      let borrowingRate_Asset = await troveManagerHelpers.getBorrowingRate(erc20.address) // expect min(0.5 + 5%, 5%) rate
      assert.equal(borrowingRate_Asset, dec(5, 16))

      // Attempt with maxFee > 5%
      const moreThan5pct = '50000000000000001'
      const tx1 = await borrowerOperations.openTrove(ZERO_ADDRESS, 0, moreThan5pct, dec(10000, 18), A, A, {
        from: D,
        value: dec(100, 'ether'),
      })
      assert.isTrue(tx1.receipt.status)

      const tx1_Asset = await borrowerOperations.openTrove(
        erc20.address,
        dec(100, 'ether'),
        moreThan5pct,
        dec(10000, 18),
      ...
      // Attempt with maxFee = 5%
      const tx2 = await borrowerOperations.openTrove(ZERO_ADDRESS, 0, dec(5, 16), dec(10000, 18), A, A, {
        from: H,
        value: dec(100, 'ether'),
      })
      assert.isTrue(tx2.receipt.status)

      const tx2_Asset = await borrowerOperations.openTrove(
        erc20.address,
        dec(100, 'ether'),
        dec(5, 16),
        dec(10000, 18),
      ...
      // Attempt with maxFee 10%
      const tx3 = await borrowerOperations.openTrove(ZERO_ADDRESS, 0, dec(1, 17), dec(10000, 18), A, A, {
        from: E,
        value: dec(100, 'ether'),
      })
      assert.isTrue(tx3.receipt.status)

      const tx3_Asset = await borrowerOperations.openTrove(
        erc20.address,
        dec(100, 'ether'),
        dec(1, 17),
        dec(10000, 18),
      ...
      // Attempt with maxFee 37.659%
      const tx4 = await borrowerOperations.openTrove(ZERO_ADDRESS, 0, dec(37659, 13), dec(10000, 18), A, A, {
        from: F,
        value: dec(100, 'ether'),
      })
      assert.isTrue(tx4.receipt.status)

      const tx4_Asset = await borrowerOperations.openTrove(
        erc20.address,
        dec(100, 'ether'),
        dec(37659, 13),
        dec(10000, 18),
      ...
      // Attempt with maxFee 100%
      const tx5 = await borrowerOperations.openTrove(ZERO_ADDRESS, 0, dec(1, 18), dec(10000, 18), A, A, {
        from: G,
        value: dec(100, 'ether'),
      })
      assert.isTrue(tx5.receipt.status)

      const tx5_Asset = await borrowerOperations.openTrove(
        erc20.address,
        dec(100, 'ether'),
        dec(1, 18),
        dec(10000, 18),
    ...
    it('openTrove(): borrowing at non-zero base rate sends DCHF fee to MON staking contract', async () => {
      // time fast-forwards 1 year, and multisig stakes 1 MON
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await monToken.approve(monStaking.address, dec(1, 18), { from: multisig })
      await monStaking.stake(dec(1, 18), { from: multisig })
    ...
    if (!withProxy) {
      ...
      it('openTrove(): borrowing at non-zero base records the (drawn debt + fee  + liq. reserve) on the Trove struct', async () => {
        ...
        const D_DCHFRequest = toBN(dec(20000, 18))
        ...
        const openTroveTx_Asset = await borrowerOperations.openTrove(
          erc20.address,
          dec(200, 'ether'),
          th._100pct,
          D_DCHFRequest,
        ...
        const emittedFee = toBN(th.getDCHFFeeFromDCHFBorrowingEvent(openTroveTx))
        assert.isTrue(toBN(emittedFee).gt(toBN('0')))

        const emittedFee_Asset = toBN(th.getDCHFFeeFromDCHFBorrowingEvent(openTroveTx_Asset))
        ...
        th.assertIsApproximatelyEqual(
          newDebt,
          D_DCHFRequest.add(emittedFee).add(DCHF_GAS_COMPENSATION),
          100000
        )
        th.assertIsApproximatelyEqual(
          newDebt_Asset,
          D_DCHFRequest.add(emittedFee_Asset).add(DCHF_GAS_COMPENSATION_ERC20),
          100000
        )
      })
    }

    it('openTrove(): Borrowing at non-zero base rate increases the MON staking contract DCHF fees-per-unit-staked', async () => {
      // time fast-forwards 1 year, and multisig stakes 1 MON
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await monToken.approve(monStaking.address, dec(1, 18), { from: multisig })
      await monStaking.stake(dec(1, 18), { from: multisig })
    ...
    it('openTrove(): Borrowing at non-zero base rate sends requested amount to the user', async () => {
      ...
      // D opens trove
      const DCHFRequest_D = toBN(dec(40000, 18))
      await borrowerOperations.openTrove(ZERO_ADDRESS, 0, th._100pct, DCHFRequest_D, D, D, {
        from: D,
        value: dec(500, 'ether'),
      })
      await borrowerOperations.openTrove(erc20.address, dec(500, 'ether'), th._100pct, DCHFRequest_D, D, D, {
        from: D,
      })
      ...
      // Check D's DCHF balance now equals their requested DCHF
      const DCHFBalance_D = await dchfToken.balanceOf(D)
      assert.isTrue(DCHFRequest_D.mul(toBN(2)).eq(DCHFBalance_D))
    })

    it('openTrove(): Borrowing at zero base rate changes the MON staking contract DCHF fees-per-unit-staked', async () => {
      await openTrove({
        extraDCHFAmount: toBN(dec(5000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
      ...
      const baseRate_1_Asset = await troveManagerHelpers.baseRate(ZERO_ADDRESS)
      assert.equal(baseRate_1_Asset, '0')

      // 2 hours pass
      th.fastForwardTime(7200, web3.currentProvider)

      // Check DCHF reward per MON staked == 0
      const F_DCHF_Before = await monStaking.F_DCHF()
      assert.equal(F_DCHF_Before, '0')
      ...
      // Check DCHF reward per MON staked > 0
      const F_DCHF_After = await monStaking.F_DCHF()
      assert.isTrue(F_DCHF_After.gt(toBN('0')))
    })
    ...
    it('openTrove(): Borrowing at zero base rate charges minimum fee', async () => {
      ...
      const DCHFRequest = toBN(dec(10000, 18))
      const txC = await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
      ...
      const _DCHFFee_Asset = toBN(th.getEventArgByName(txC_Asset, 'DCHFBorrowingFeePaid', '_DCHFFee'))

      const expectedFee = BORROWING_FEE_FLOOR.mul(toBN(DCHFRequest)).div(toBN(dec(1, 18)))
      const expectedFee_Asset = BORROWING_FEE_FLOOR_ERC20.mul(toBN(DCHFRequest)).div(toBN(dec(1, 18)))
      assert.isTrue(_DCHFFee.eq(expectedFee))
      assert.isTrue(_DCHFFee_Asset.eq(expectedFee_Asset))
    })
    ...
    it('openTrove(): reverts when system is in Recovery Mode and ICR < CCR', async () => {
      ...
      // Bob tries to open a trove with 149% ICR during Recovery Mode
      try {
        const txBob_Asset = await openTrove({
          asset: erc20.address,
          extraDCHFAmount: toBN(dec(5000, 18)),
          ICR: toBN(dec(149, 16)),
          extraParams: { from: alice },
    ...
    it('openTrove(): Can open a trove with ICR >= CCR when system is in Recovery Mode', async () => {
      ...
      const carol_TroveStatus = await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, carol)
      assert.equal(carol_TroveStatus, 1)

      const carol_TroveStatus_Asset = await troveManagerHelpers.getTroveStatus(erc20.address, carol)
      assert.equal(carol_TroveStatus_Asset, 1)

      const carolICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)
      assert.isTrue(carolICR.gt(toBN(dec(150, 16))))

      const carolICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)
    ...
    it('openTrove(): Reverts opening a trove with min debt when system is in Recovery Mode', async () => {
      ...
      await assertRevert(
        borrowerOperations.openTrove(
          ...
          0,
          th._100pct,
          await getNetBorrowingAmount(MIN_NET_DEBT, ZERO_ADDRESS),
          carol,
          carol,
          { from: carol, value: dec(1, 'ether') }
        )
    ...
    it('openTrove(): creates a new Trove and assigns the correct collateral and debt amount', async () => {
      ...
      const DCHFRequest = MIN_NET_DEBT
      const DCHFRequestERC20 = MIN_NET_DEBT_ERC20
      borrowerOperations.openTrove(ZERO_ADDRESS, 0, th._100pct, MIN_NET_DEBT, carol, carol, {
        from: alice,
        value: dec(100, 'ether'),
      })
      borrowerOperations.openTrove(
      ...
      // Get the expected debt based on the DCHF request (adding fee and liq. reserve on top)
      const expectedDebt = DCHFRequest.add(
        await troveManagerHelpers.getBorrowingFee(ZERO_ADDRESS, DCHFRequest)
      ).add(DCHF_GAS_COMPENSATION)

      const expectedDebt_Asset = DCHFRequestERC20.add(
        await troveManagerHelpers.getBorrowingFee(erc20.address, DCHFRequestERC20)
      ).add(DCHF_GAS_COMPENSATION_ERC20)

      const debt_After = await getTroveEntireDebt(alice)
      const coll_After = await getTroveEntireColl(alice)
      const status_After = await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, alice)
      ...
      const status_After_Asset = await troveManagerHelpers.getTroveStatus(erc20.address, alice)
    ...
    it('openTrove(): adds Trove owner to TroveOwners array', async () => {
      const TroveOwnersCount_Before = (await troveManagerHelpers.getTroveOwnersCount(ZERO_ADDRESS)).toString()
      assert.equal(TroveOwnersCount_Before, '0')

      const TroveOwnersCount_Before_Asset = (
        await troveManagerHelpers.getTroveOwnersCount(erc20.address)
      ).toString()
      ...
      const TroveOwnersCount_After = (await troveManagerHelpers.getTroveOwnersCount(ZERO_ADDRESS)).toString()
      const TroveOwnersCount_After_Asset = (
        await troveManagerHelpers.getTroveOwnersCount(erc20.address)
      ).toString()
    ...
    it('openTrove(): records up-to-date initial snapshots of L_ETH and L_DCHFDebt', async () => {
      // --- SETUP ---
      ...
      /* with total stakes = 10 ether, after liquidation, L_ETH should equal 1/10 ether per-ether-staked,
       and L_DCHF should equal 18 DCHF per-ether-staked. */

      const L_ETH = await troveManagerHelpers.L_ASSETS(ZERO_ADDRESS)
      const L_DCHF = await troveManagerHelpers.L_DCHFDebts(ZERO_ADDRESS)

      const L_Asset = await troveManagerHelpers.L_ASSETS(erc20.address)
      const L_DCHF_Asset = await troveManagerHelpers.L_DCHFDebts(erc20.address)

      assert.isTrue(L_ETH.gt(toBN('0')))
      assert.isTrue(L_DCHF.gt(toBN('0')))
      assert.isTrue(L_Asset.gt(toBN('0')))
      assert.isTrue(L_DCHF_Asset.gt(toBN('0')))
      ...
      // Check Bob's snapshots of L_ETH and L_DCHF equal the respective current values
      ...
      const bob_DCHFDebtRewardSnapshot = bob_rewardSnapshot[1]
      ...
      const bob_DCHFDebtRewardSnapshot_Asset = bob_rewardSnapshot_Asset[1]

      assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot, L_ETH), 1000)
      assert.isAtMost(th.getDifference(bob_DCHFDebtRewardSnapshot, L_DCHF), 1000)

      assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot, L_Asset), 1000)
      assert.isAtMost(th.getDifference(bob_DCHFDebtRewardSnapshot, L_DCHF_Asset), 1000)
    })
    ...
    it('openTrove(): allows a user to open a Trove, then close it, then re-open it', async () => {
      ...
      // Check Trove is active
      const alice_Trove_1 = await troveManagerHelpers.Troves(alice, ZERO_ADDRESS)
      const status_1 = alice_Trove_1[th.TROVE_STATUS_INDEX]
      assert.equal(status_1, 1)
      assert.isTrue(await sortedTroves.contains(ZERO_ADDRESS, alice))

      const alice_Trove_1_Asset = await troveManagerHelpers.Troves(alice, erc20.address)
      ...
      const alice_Trove_2 = await troveManagerHelpers.Troves(alice, ZERO_ADDRESS)
      const status_2 = alice_Trove_2[th.TROVE_STATUS_INDEX]
      assert.equal(status_2, 2)
      assert.isFalse(await sortedTroves.contains(ZERO_ADDRESS, alice))

      const alice_Trove_2_Asset = await troveManagerHelpers.Troves(alice, erc20.address)
      ...
      const alice_Trove_3 = await troveManagerHelpers.Troves(alice, ZERO_ADDRESS)
      const status_3 = alice_Trove_3[th.TROVE_STATUS_INDEX]
      assert.equal(status_3, 1)
      assert.isTrue(await sortedTroves.contains(ZERO_ADDRESS, alice))

      const alice_Trove_3_Asset = await troveManagerHelpers.Troves(alice, erc20.address)
      const status_3_Asset = alice_Trove_3_Asset[th.TROVE_STATUS_INDEX]
      assert.equal(status_3_Asset, 1)
      assert.isTrue(await sortedTroves.contains(erc20.address, alice))
    })

    it("openTrove(): increases the Trove's DCHF debt by the correct amount", async () => {
      // check before
      const alice_Trove_Before = await troveManagerHelpers.Troves(alice, ZERO_ADDRESS)
    ...
    it('openTrove(): increases DCHF debt in ActivePool by the debt of the trove', async () => {
      const activePooL_DCHFDebt_Before = await activePool.getDCHFDebt(ZERO_ADDRESS)
      const activePooL_DCHFDebt_Before_Asset = await activePool.getDCHFDebt(erc20.address)
      assert.equal(activePooL_DCHFDebt_Before, 0)
      assert.equal(activePooL_DCHFDebt_Before_Asset, 0)
      ...
      const activePooL_DCHFDebt_After = await activePool.getDCHFDebt(ZERO_ADDRESS)
      const activePooL_DCHFDebt_After_Asset = await activePool.getDCHFDebt(erc20.address)
      assert.isTrue(activePooL_DCHFDebt_After.eq(aliceDebt))
      assert.isTrue(activePooL_DCHFDebt_After_Asset.eq(aliceDebt_Asset))
    })

    it('openTrove(): increases user DCHFToken balance by correct amount', async () => {
      // check before
      const alice_DCHFTokenBalance_Before = await dchfToken.balanceOf(alice)
      assert.equal(alice_DCHFTokenBalance_Before, 0)
      const troveColl = dec(100, 'ether')

      await borrowerOperations.openTrove(ZERO_ADDRESS, troveColl, th._100pct, dec(10000, 18), alice, alice, {
        from: alice,
        value: troveColl,
      })
      await borrowerOperations.openTrove(erc20.address, troveColl, th._100pct, dec(10000, 18), alice, alice, {
        from: alice,
      })

      // check after
      const alice_DCHFTokenBalance_After = await dchfToken.balanceOf(alice)
      assert.equal(alice_DCHFTokenBalance_After, dec(20000, 18))
    })
    ...
    it('getCompositeDebt(): returns debt + gas comp', async () => {
      assert.equal(
        await borrowerOperations.getCompositeDebt(ZERO_ADDRESS, '0'),
        DCHF_GAS_COMPENSATION.toString()
      )

      th.assertIsApproximatelyEqual(
        await borrowerOperations.getCompositeDebt(ZERO_ADDRESS, dec(90, 18)),
        DCHF_GAS_COMPENSATION.add(toBN(dec(90, 18)))
      )
      th.assertIsApproximatelyEqual(
        await borrowerOperations.getCompositeDebt(ZERO_ADDRESS, dec(24423422357345049, 12)),
        DCHF_GAS_COMPENSATION.add(toBN(dec(24423422357345049, 12)))
      )

      assert.equal(
        await borrowerOperations.getCompositeDebt(erc20.address, '0'),
        DCHF_GAS_COMPENSATION_ERC20.toString()
      )

      th.assertIsApproximatelyEqual(
        await borrowerOperations.getCompositeDebt(erc20.address, dec(90, 18)),
        DCHF_GAS_COMPENSATION_ERC20.add(toBN(dec(90, 18)))
      )
      th.assertIsApproximatelyEqual(
        await borrowerOperations.getCompositeDebt(erc20.address, dec(24423422357345049, 12)),
        DCHF_GAS_COMPENSATION_ERC20.add(toBN(dec(24423422357345049, 12)))
      )
    })
    ...
    describe('getNewTCRFromTroveChange() returns the correct TCR', async () => {
      ...
      it('collChange = 0, debtChange = 0', async () => {
        ...
        const troveDCHFAmount = await getOpenTroveDCHFAmount(troveTotalDebt, ZERO_ADDRESS)
        const troveDCHFAmount_Asset = await getOpenTroveDCHFAmount(troveTotalDebt, erc20.address)
        ...
        await borrowerOperations.openTrove(ZERO_ADDRESS, troveColl, th._100pct, troveDCHFAmount, bob, bob, {
          from: bob,
          value: troveColl,
        })
        ...
        const newTCR_Asset = await borrowerOperations.getNewTCRFromTroveChange(
          erc20.address,
          collChange,
          true,
          debtChange,
        ...
        const expectedTCR_Asset = troveColl
    ...
    if (!withProxy) {
      it('closeTrove(): fails if owner cannot receive ETH', async () => {
        ...
        // Alice sends DCHF to NonPayable so its DCHF balance covers its debt
        await dchfToken.transfer(nonPayable.address, dec(40000, 18), { from: alice })
        ...
        const openTroveData = th.getTransactionData(
          ...
          [ZERO_ADDRESS, 0, _100pctHex, _1e25Hex, '0x0', '0x0']
        )
        const openTroveData_Asset = th.getTransactionData(
          'openTrove(address,uint256,uint256,uint256,address,address)',
        ...
        assert.equal(
          (await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, nonPayable.address)).toString(),
          '1',
          'NonPayable proxy should have a trove'
        )
        // assert.equal((await troveManagerHelpers.getTroveStatus(erc20.address, nonPayable.address)).toString(), '1', 'NonPayable proxy should have a trove')
...
/* TODO:
 ...
 changes with addColl, withdrawColl, withdrawDCHF, withdrawDCHF, etc. Can split them up and put them with
