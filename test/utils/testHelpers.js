...
class TestHelper {
  ...
  static async checkRecoveryMode(contracts, asset) {
    ...
    const price = await contracts.priceFeedTestnet.getPrice()
    return contracts.troveManagerHelpers.checkRecoveryMode(asset, price)
  }
  ...
  static async getTCR(contracts, asset) {
    ...
    const price = await contracts.priceFeedTestnet.getPrice()
    return contracts.troveManagerHelpers.getTCR(asset, price)
  }
  ...
  static async getTroveEntireColl(contracts, trove, asset) {
    if (!asset) asset = this.ZERO_ADDRESS
    return this.toBN((await contracts.troveManagerHelpers.getEntireDebtAndColl(asset, trove))[1])
  }

  static async getTroveEntireDebt(contracts, trove, asset) {
    if (!asset) asset = this.ZERO_ADDRESS
    return this.toBN((await contracts.troveManagerHelpers.getEntireDebtAndColl(asset, trove))[0])
  }

  static async getTroveStake(contracts, trove, asset) {
    if (!asset) asset = this.ZERO_ADDRESS
    return contracts.troveManagerHelpers.getTroveStake(asset, trove)
  }

  /*
   * given the requested DCHF amount in openTrove, returns the total debt
   * So, it adds the gas compensation and the borrowing fee
   */
  static async getOpenTroveTotalDebt(contracts, DCHFAmount, asset) {
    if (!asset) asset = this.ZERO_ADDRESS

    const fee = await contracts.troveManagerHelpers.getBorrowingFee(asset, DCHFAmount)
  ...
  static async getEntireCollAndDebt(contracts, account, asset) {
    ...
    const rawColl = (await contracts.troveManagerHelpers.Troves(account, asset))[this.TROVE_COLL_INDEX]
    const rawDebt = (await contracts.troveManagerHelpers.Troves(account, asset))[this.TROVE_DEBT_INDEX]
    const pendingAssetReward = await contracts.troveManagerHelpers.getPendingAssetReward(asset, account)
    const pendingDCHFDebtReward = await contracts.troveManagerHelpers.getPendingDCHFDebtReward(asset, account)
  ...
  static async getCollAndDebtFromAdjustment(contracts, account, ETHChange, DCHFChange) {
    ...
    const fee = DCHFChange.gt(this.toBN('0'))
      ? await contracts.troveManager.getBorrowingFee(DCHFChange)
      : this.toBN('0')
  ...
  static async openTrove_allAccounts_randomETH_ProportionalDCHF(
    minETH,
    maxETH,
    accounts,
    contracts,
  ...
  static async openTrove_allAccounts_randomETH_randomDCHF(
    minETH,
    maxETH,
    accounts,
    contracts,
    ...
    for (const account of accounts) {
      ...
      const proportionalDCHF = web3.utils
        .toBN(randDCHFProportion)
  ...
  static async openTrove(
    contracts,
    { asset, assetSent, maxFeePercentage, extraDCHFAmount, upperHint, lowerHint, ICR, extraParams }
  ) {
    if (!asset) asset = this.ZERO_ADDRESS
    ...
    if (
      !ICR &&
      ((asset == this.ZERO_ADDRESS && !extraParams.value) || (asset != this.ZERO_ADDRESS && !assetSent))
    ) {
  ...
  static async withdrawDCHF(
    contracts,
    { asset, maxFeePercentage, DCHFAmount, ICR, upperHint, lowerHint, extraParams }
  ) {
    if (!asset) asset = this.ZERO_ADDRESS
    if (!maxFeePercentage) maxFeePercentage = this._100pct
    if (!upperHint) upperHint = this.ZERO_ADDRESS
    ...
    assert(
      !(DCHFAmount && ICR) && (DCHFAmount || ICR),
      'Specify either DCHF amount or target ICR, but not both'
    )

    let increasedTotalDebt
    if (ICR) {
      assert(extraParams.from, 'A from account is needed')
      const { debt, coll } = await contracts.troveManagerHelpers.getEntireDebtAndColl(asset, extraParams.from)
      const price = await contracts.priceFeedTestnet.getPrice()
    ...
    await contracts.borrowerOperations.withdrawDCHF(
      asset,
      maxFeePercentage,
      DCHFAmount,
      upperHint,
  ...
  static async withdrawDCHF_allAccounts(accounts, contracts, amount) {
    ...
    for (const account of accounts) {
      ...
      const tx = await contracts.borrowerOperations.withdrawDCHF(this._100pct, amount, upperHint, lowerHint, {
        from: account,
      })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
  ...
  static async withdrawDCHF_allAccounts_randomAmount(min, max, accounts, contracts) {
    ...
    for (const account of accounts) {
      ...
      const { newColl, newDebt } = await this.getCollAndDebtFromWithdrawDCHF(
        contracts,
        account,
        randDCHFAmount
      )
      ...
      const tx = await contracts.borrowerOperations.withdrawDCHF(
        this._100pct,
        randDCHFAmount,
        upperHint,
        lowerHint,
  ...
  static async repayDCHF_allAccounts(accounts, contracts, amount) {
    ...
    for (const account of accounts) {
      ...
      const tx = await contracts.borrowerOperations.repayDCHF(amount, upperHint, lowerHint, {
        from: account,
      })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
  ...
  static async repayDCHF_allAccounts_randomAmount(min, max, accounts, contracts) {
    ...
    for (const account of accounts) {
      ...
      const tx = await contracts.borrowerOperations.repayDCHF(randDCHFAmount, upperHint, lowerHint, {
        from: account,
      })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
  ...
  static async provideToSP_allAccounts_randomAmount(min, max, accounts, stabilityPool) {
    ...
    for (const account of accounts) {
      ...
      const tx = await stabilityPool.provideToSP(randomDCHFAmount, this.ZERO_ADDRESS, {
        from: account,
      })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
  ...
  static async fastForwardTime(seconds, currentWeb3Provider) {
    if (web3.utils.isBigNumber(seconds) || web3.utils.isBN(seconds)) seconds = seconds.toNumber()
  ...
  static getTransactionData(signatureString, params) {
     ...
     */
    return (
      web3.utils.sha3(signatureString).slice(0, 10) + params.reduce((acc, p) => acc + this.formatParam(p), '')
    )
  }
}
