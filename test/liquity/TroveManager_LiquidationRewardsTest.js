...
contract('TroveManager - Redistribution reward calculations', async accounts => {
  ...
  it("redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    await borrowerOperations.withdrawDCHF(ZERO_ADDRESS, th._100pct, await getNetBorrowingAmount(A_totalDebt), alice, alice, { from: alice })
    await borrowerOperations.withdrawDCHF(erc20.address, th._100pct, await getNetBorrowingAmount(A_totalDebt, erc20.address), alice, alice, { from: alice })
    ...
    const bob_DCHFDebt = ((await troveManager.Troves(bob, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]
      .add(await troveManager.getPendingDCHFDebtReward(ZERO_ADDRESS, bob)))
      .toString()
    ...
    const bob_DCHFDebt_Asset = ((await troveManager.Troves(bob, erc20.address))[th.TROVE_DEBT_INDEX]
      .add(await troveManager.getPendingDCHFDebtReward(erc20.address, bob)))
      .toString()
    ...
    assert.isAtMost(th.getDifference(bob_DCHFDebt, A_totalDebt.mul(toBN(2)).add(B_totalDebt).add(C_totalDebt)), 1000)

    assert.isAtMost(th.getDifference(bob_Coll_Asset, expected_B_coll_Asset), 1000)
    assert.isAtMost(th.getDifference(bob_DCHFDebt_Asset, A_totalDebt_Asset.mul(toBN(2)).add(B_totalDebt_Asset).add(C_totalDebt_Asset)), 1000)
  })
  ...
  it("redistribution: A,B,C Open. Liq(C). B tops up coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    */
    ...
    const alice_DCHFDebt = ((await troveManager.Troves(alice, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]
      .add(await troveManager.getPendingDCHFDebtReward(ZERO_ADDRESS, alice)))
      .toString()
    ...
    const alice_DCHFDebt_Asset = ((await troveManager.Troves(alice, erc20.address))[th.TROVE_DEBT_INDEX]
      .add(await troveManager.getPendingDCHFDebtReward(erc20.address, alice)))
      .toString()
  ...
  it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). C tops up. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    */
    ...
    // check DCHF gas compensation
    th.assertIsApproximatelyEqual((await dchfToken.balanceOf(owner)).toString(), toBN(dec(400, 18)).mul(toBN(2)).toString())
  })
  ...
  it("redistribution: A,B,C Open. Liq(C). B withdraws coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    */
    ...
    const entireSystemDebt = (await activePool.getDCHFDebt(ZERO_ADDRESS)).add(await defaultPool.getDCHFDebt(ZERO_ADDRESS))
    const entireSystemDebt_Asset = (await activePool.getDCHFDebt(erc20.address)).add(await defaultPool.getDCHFDebt(erc20.address))
