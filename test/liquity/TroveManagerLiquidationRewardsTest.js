...
contract('TroveManager - Redistribution reward calculations', async (accounts) => {
  ...
  it('redistribution: A, B Open. B Liquidated. C, D Open. D Liquidated. Distributes correct rewards', async () => {
    ...
    const alice_Coll = (await troveManagerHelpers.Troves(alice, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, alice))
      .toString()
    const carol_Coll = (await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, carol))
      .toString()

    const alice_Coll_Asset = (await troveManagerHelpers.Troves(alice, erc20.address))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(erc20.address, alice))
      .toString()
    const carol_Coll_Asset = (await troveManagerHelpers.Troves(carol, erc20.address))[th.TROVE_COLL_INDEX]
    ...
    */
    ...
    assert.isAtMost(
      th.getDifference(
        ...
        A_collAfterL1_Asset.add(
          A_collAfterL1_Asset.mul(th.applyLiquidationFee(D_coll_Asset)).div(
            A_collAfterL1_Asset.add(C_coll_Asset)
          )
        )
      ),
      1000
    ...
    const entireSystemColl = (await activePool.getAssetBalance(ZERO_ADDRESS))
      .add(await defaultPool.getAssetBalance(ZERO_ADDRESS))
      .toString()
    ...
    // check DCHF gas compensation
    assert.equal((await dchfToken.balanceOf(owner)).toString(), toBN(dec(400, 18)).mul(toBN(2)).toString())
  })
  ...
  it('redistribution: A, B, C Open. C Liquidated. D, E, F Open. F Liquidated. Distributes correct rewards', async () => {
    ...
    const { collateral: E_coll_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(400, 16)),
      extraParams: { from: erin },
    })
    const { collateral: F_coll_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(210, 16)),
    ...
    const bob_Coll = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, bob))
      .toString()
    const dennis_Coll = (await troveManagerHelpers.Troves(dennis, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, dennis))
      .toString()
    const erin_Coll = (await troveManagerHelpers.Troves(erin, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, erin))
      .toString()
    ...
    const bob_Coll_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_COLL_INDEX]
      ...
      .toString()
    const dennis_Coll_Asset = (await troveManagerHelpers.Troves(dennis, erc20.address))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(erc20.address, dennis))
      .toString()
    const erin_Coll_Asset = (await troveManagerHelpers.Troves(erin, erc20.address))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(erc20.address, erin))
      .toString()
    ...
    */
    ...
    const totalBeforeL2_Asset = A_collAfterL1_Asset.add(B_collAfterL1_Asset)
      .add(D_coll_Asset)
      .add(E_coll_Asset)
  ...
  it('redistribution: Sequence of alternate opening/liquidation: final surviving trove has ETH from all previously liquidated troves', async () => {
    ...
    const freddy_rawColl = (await troveManagerHelpers.Troves(freddy, ZERO_ADDRESS))[
      th.TROVE_COLL_INDEX
    ].toString()
    const freddy_ETHReward = (
      await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, freddy)
    ).toString()

    const freddy_rawColl_Asset = (await troveManagerHelpers.Troves(freddy, erc20.address))[
      th.TROVE_COLL_INDEX
    ].toString()
    const freddy_ETHReward_Asset = (
      await troveManagerHelpers.getPendingAssetReward(erc20.address, freddy)
    ).toString()
    ...
    */
    ...
    // check DCHF gas compensation
    assert.equal((await dchfToken.balanceOf(owner)).toString(), toBN(dec(1000, 18)).mul(toBN(2)).toString())
  })
  ...
  it('redistribution: A,B,C,D,E open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt', async () => {
    ...
    const totalCollAfterL2_Asset = B_collAfterL1_Asset.add(addedColl1)
      .add(D_collAfterL1_Asset)
      .add(E_collAfterL1_Asset)
    const B_collAfterL2_Asset = B_collAfterL1_Asset.add(addedColl1).add(
      th
        .applyLiquidationFee(C_collAfterL1_Asset)
        .mul(B_collAfterL1_Asset.add(addedColl1))
        .div(totalCollAfterL2_Asset)
    )
    ...
    const B_collAfterL3_Asset = B_collAfterL2_Asset.add(addedColl2).add(
      th
        .applyLiquidationFee(E_collAfterL2_Asset)
        .mul(B_collAfterL2_Asset.add(addedColl2))
        .div(totalCollAfterL3_Asset)
    )
  ...
  it('redistribution: A,B,C,D open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt', async () => {
    ...
    const denominatorColl_1_Asset = (await troveManager.getEntireSystemColl(erc20.address)).sub(
      A_entireColl_0_Asset
    )
    ...
    const B_ETHGain_1 = await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, B)
    const C_ETHGain_1 = await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, C)
    const D_ETHGain_1 = await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, D)
    const E_ETHGain_1 = await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, E)

    const B_ETHGain_1_Asset = await troveManagerHelpers.getPendingAssetReward(erc20.address, B)
    const C_ETHGain_1_Asset = await troveManagerHelpers.getPendingAssetReward(erc20.address, C)
    const D_ETHGain_1_Asset = await troveManagerHelpers.getPendingAssetReward(erc20.address, D)
    const E_ETHGain_1_Asset = await troveManagerHelpers.getPendingAssetReward(erc20.address, E)
    ...
    const denominatorColl_2_Asset = (await troveManager.getEntireSystemColl(erc20.address)).sub(
      C_entireColl_1_Asset
    )
    ...
    const B_ETHGain_2 = await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, B)
    const D_ETHGain_2 = await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, D)
    const E_ETHGain_2 = await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, E)

    const B_ETHGain_2_Asset = await troveManagerHelpers.getPendingAssetReward(erc20.address, B)
    const D_ETHGain_2_Asset = await troveManagerHelpers.getPendingAssetReward(erc20.address, D)
    const E_ETHGain_2_Asset = await troveManagerHelpers.getPendingAssetReward(erc20.address, E)
    ...
    const denominatorColl_3_Asset = (await troveManager.getEntireSystemColl(erc20.address)).sub(
      E_entireColl_2_Asset
    )
    ...
    const B_ETHGain_3 = await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, B)
    const D_ETHGain_3 = await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, D)

    const B_ETHGain_3_Asset = await troveManagerHelpers.getPendingAssetReward(erc20.address, B)
    const D_ETHGain_3_Asset = await troveManagerHelpers.getPendingAssetReward(erc20.address, D)
  ...
  it('redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt', async () => {
    ...
    await borrowerOperations.withdrawDCHF(
      erc20.address,
      th._100pct,
      await getNetBorrowingAmount(A_totalDebt, erc20.address),
      alice,
      alice,
      { from: alice }
    )
    ...
    // Expect Bob now holds all Ether and DCHFDebt in the system: 2 + 0.4975+0.4975*0.995+0.995 Ether and 110*3 DCHF (10 each for gas compensation)
    const bob_Coll = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, bob))
      .toString()

    const bob_DCHFDebt = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]
      .add(await troveManagerHelpers.getPendingDCHFDebtReward(ZERO_ADDRESS, bob))
      .toString()
    ...
    const bob_DCHFDebt_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_DEBT_INDEX]
    ...
    assert.isAtMost(
      th.getDifference(bob_DCHFDebt, A_totalDebt.mul(toBN(2)).add(B_totalDebt).add(C_totalDebt)),
      1000
    )
  ...
  it('redistribution: A,B,C Open. Liq(C). B tops up coll. D Opens. Liq(D). Distributes correct rewards.', async () => {
    ...
    /* Bob rewards:
     L1: 1/2*0.995 ETH, 55 DCHF
     L2: (2.4975/3.995)*0.995 = 0.622 ETH , 110*(2.4975/3.995)= 68.77 DCHFDebt

    coll: 3.1195 ETH
    debt: 233.77 DCHFDebt

     Alice rewards:
    L1 1/2*0.995 ETH, 55 DCHF
    L2 (1.4975/3.995)*0.995 = 0.3730 ETH, 110*(1.4975/3.995) = 41.23 DCHFDebt

    coll: 1.8705 ETH
    debt: 146.23 DCHFDebt

    totalColl: 4.99 ETH
    totalDebt 380 DCHF (includes 50 each for gas compensation)
    */
    const bob_Coll = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, bob))
      .toString()
    ...
    const alice_DCHFDebt = (await troveManagerHelpers.Troves(alice, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]
      .add(await troveManagerHelpers.getPendingDCHFDebtReward(ZERO_ADDRESS, alice))
      .toString()
    ...
    const alice_DCHFDebt_Asset = (await troveManagerHelpers.Troves(alice, erc20.address))[th.TROVE_DEBT_INDEX]
    ...
    const B_collAfterL1 = B_coll.add(B_coll.mul(th.applyLiquidationFee(C_coll)).div(A_coll.add(B_coll))).add(
      addedColl
    )
    ...
    assert.isAtMost(th.getDifference(bob_Coll, expected_B_coll), 1000)
    assert.isAtMost(th.getDifference(bob_DCHFDebt, expected_B_debt), 10000)

    assert.isAtMost(th.getDifference(bob_Coll_Asset, expected_B_coll_Asset), 1000)
    assert.isAtMost(th.getDifference(bob_DCHFDebt_Asset, expected_B_debt_Asset), 10000)
    ...
    assert.isAtMost(th.getDifference(alice_DCHFDebt, expected_A_debt), 10000)

    assert.isAtMost(th.getDifference(alice_Coll_Asset, expected_A_coll_Asset), 1000)
    assert.isAtMost(th.getDifference(alice_DCHFDebt_Asset, expected_A_debt_Asset), 10000)
  ...
  it('redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). C tops up. E Enters, Liq(E). Distributes correct rewards', async () => {
    ...
    const alice_ETHReward_1 = await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, alice)
    const bob_ETHReward_1 = await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, bob)
    const carol_ETHReward_1 = await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, carol)

    const alice_ETHReward_1_Asset = await troveManagerHelpers.getPendingAssetReward(erc20.address, alice)
    const bob_ETHReward_1_Asset = await troveManagerHelpers.getPendingAssetReward(erc20.address, bob)
    const carol_ETHReward_1_Asset = await troveManagerHelpers.getPendingAssetReward(erc20.address, carol)

    //Expect 1000 + 1000*0.995 ETH in system now
    const entireSystemColl_1 = (await activePool.getAssetBalance(ZERO_ADDRESS))
      .add(await defaultPool.getAssetBalance(ZERO_ADDRESS))
      .toString()
    ...
    const entireSystemColl_2 = (await activePool.getAssetBalance(ZERO_ADDRESS)).add(
      await defaultPool.getAssetBalance(ZERO_ADDRESS)
    )
    ...
    th.assertIsApproximatelyEqual(
      entireSystemColl_2_Asset,
      totalColl_Asset.add(th.applyLiquidationFee(D_coll_Asset)).add(C_addedColl)
    )
    ...
    */
    ...
    const totalCollAfterL1_Asset = A_coll_Asset.add(B_coll_Asset)
      ...
      .add(C_addedColl)
    const A_collAfterL1_Asset = A_coll_Asset.add(
      A_coll_Asset.mul(th.applyLiquidationFee(D_coll_Asset)).div(
        A_coll_Asset.add(B_coll_Asset).add(C_coll_Asset)
      )
    )
    const expected_A_coll_Asset = A_collAfterL1_Asset.add(
    ...
    const B_collAfterL1_Asset = B_coll_Asset.add(
      B_coll_Asset.mul(th.applyLiquidationFee(D_coll_Asset)).div(
        A_coll_Asset.add(B_coll_Asset).add(C_coll_Asset)
      )
    )
    const expected_B_coll_Asset = B_collAfterL1_Asset.add(
    ...
    const C_collAfterL1_Asset = C_coll_Asset.add(
      C_coll_Asset.mul(th.applyLiquidationFee(D_coll_Asset)).div(
        A_coll_Asset.add(B_coll_Asset).add(C_coll_Asset)
      )
    ).add(C_addedColl)
    ...
    const entireSystemColl_3 = (await activePool.getAssetBalance(ZERO_ADDRESS))
      .add(await defaultPool.getAssetBalance(ZERO_ADDRESS))
      .toString()
    ...
    // check DCHF gas compensation
    th.assertIsApproximatelyEqual(
      (await dchfToken.balanceOf(owner)).toString(),
      toBN(dec(400, 18)).mul(toBN(2)).toString()
    )
  })
  ...
  it('redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). A, B, C top up. E Enters, Liq(E). Distributes correct rewards', async () => {
    ...
    const entireSystemColl_2 = (await activePool.getAssetBalance(ZERO_ADDRESS))
      .add(await defaultPool.getAssetBalance(ZERO_ADDRESS))
      .toString()
    ...
    th.assertIsApproximatelyEqual(
      entireSystemColl_2_Asset,
      totalColl_Asset.add(th.applyLiquidationFee(D_coll_Asset)).add(addedColl.mul(toBN(3)))
    )
    ...
    */
    ...
    const entireSystemColl_3 = (await activePool.getAssetBalance(ZERO_ADDRESS)).add(
      await defaultPool.getAssetBalance(ZERO_ADDRESS)
    )
  ...
  it('redistribution: A,B,C Open. Liq(C). B withdraws coll. Liq(A). B acquires all coll and debt', async () => {
    ...
    // Expect Bob now holds all Ether and DCHFDebt in the system: 2.5 Ether and 300 DCHF
  ...
  it('redistribution: A,B,C Open. Liq(C). B withdraws coll. D Opens. Liq(D). Distributes correct rewards.', async () => {
    ...
    /* Bob rewards:
     L1: 0.4975 ETH, 55 DCHF
     L2: (0.9975/2.495)*0.995 = 0.3978 ETH , 110*(0.9975/2.495)= 43.98 DCHFDebt

    coll: (1 + 0.4975 - 0.5 + 0.3968) = 1.3953 ETH
    debt: (110 + 55 + 43.98 = 208.98 DCHFDebt 

     Alice rewards:
    L1 0.4975, 55 DCHF
    L2 (1.4975/2.495)*0.995 = 0.5972 ETH, 110*(1.4975/2.495) = 66.022 DCHFDebt

    coll: (1 + 0.4975 + 0.5972) = 2.0947 ETH
    debt: (50 + 55 + 66.022) = 171.022 DCHF Debt

    totalColl: 3.49 ETH
    totalDebt 380 DCHF (Includes 50 in each trove for gas compensation)
    */
    const bob_Coll = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, bob))
      .toString()
    ...
    const entireSystemColl = (await activePool.getAssetBalance(ZERO_ADDRESS)).add(
      await defaultPool.getAssetBalance(ZERO_ADDRESS)
    )
    ...
    const entireSystemDebt = (await activePool.getDCHFDebt(ZERO_ADDRESS)).add(
      await defaultPool.getDCHFDebt(ZERO_ADDRESS)
    )
    const entireSystemDebt_Asset = (await activePool.getDCHFDebt(erc20.address)).add(
  ...
  it('redistribution: Trove with the majority stake withdraws. A,B,C,D open. Liq(D). C withdraws some coll. E Enters, Liq(E). Distributes correct rewards', async () => {
    ...
    //Expect 1995 ETH in system now
    const entireSystemColl_1 = (await activePool.getAssetBalance(ZERO_ADDRESS)).add(
      await defaultPool.getAssetBalance(ZERO_ADDRESS)
    )
    ...
    th.assertIsApproximatelyEqual(
      entireSystemColl_2_Asset,
      totalColl_Asset.add(th.applyLiquidationFee(D_coll_Asset)).sub(C_withdrawnColl)
    )
  ...
  it('redistribution: Trove with the majority stake withdraws. A,B,C,D open. Liq(D). A, B, C withdraw. E Enters, Liq(E). Distributes correct rewards', async () => {
    ...
    const alice_Coll_1 = (await troveManagerHelpers.Troves(alice, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, alice))
      .toString()

    const bob_Coll_1 = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, bob))
      .toString()

    const carol_Coll_1 = (await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, carol))
      .toString()

    const alice_Coll_1_Asset = (await troveManagerHelpers.Troves(alice, erc20.address))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(erc20.address, alice))
      .toString()

    const bob_Coll_1_Asset = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(erc20.address, bob))
      .toString()

    const carol_Coll_1_Asset = (await troveManagerHelpers.Troves(carol, erc20.address))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(erc20.address, carol))
      .toString()
    ...
    assert.isAtMost(
      th.getDifference(
        alice_Coll_1_Asset,
        A_coll_Asset.add(th.applyLiquidationFee(D_coll_Asset).mul(A_coll_Asset).div(totalColl_1_Asset)).sub(
          withdrawnColl
        )
      ),
      1000
    ...
    assert.isAtMost(
      th.getDifference(
        bob_Coll_1_Asset,
        B_coll_Asset.add(th.applyLiquidationFee(D_coll_Asset).mul(B_coll_Asset).div(totalColl_1_Asset)).sub(
          withdrawnColl
        )
      ),
      1000
    ...
    assert.isAtMost(
      th.getDifference(
        carol_Coll_1_Asset,
        C_coll_Asset.add(th.applyLiquidationFee(D_coll_Asset).mul(C_coll_Asset).div(totalColl_1_Asset)).sub(
          withdrawnColl
        )
      ),
      1000
    ...
    th.assertIsApproximatelyEqual(
      entireSystemColl_2_Asset,
      totalColl_Asset.add(th.applyLiquidationFee(D_coll_Asset)).sub(withdrawnColl.mul(toBN(3)))
    )
    ...
    */
    ...
    const alice_Coll_2 = (await troveManagerHelpers.Troves(alice, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, alice))
      .toString()

    const bob_Coll_2 = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, bob))
      .toString()

    const carol_Coll_2 = (await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, carol))
      .toString()

    const alice_Coll_2_Asset = (await troveManagerHelpers.Troves(alice, erc20.address))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(erc20.address, alice))
      .toString()

    const bob_Coll_2_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(erc20.address, bob))
      .toString()

    const carol_Coll_2_Asset = (await troveManagerHelpers.Troves(carol, erc20.address))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(erc20.address, carol))
      .toString()
    ...
    const expected_B_coll_Asset = B_collAfterL1_Asset.add(
      B_collAfterL1_Asset.mul(th.applyLiquidationFee(E_coll_Asset)).div(totalCollAfterL1)
    )
    const C_collAfterL1_Asset = C_coll_Asset.add(
      C_coll_Asset.mul(th.applyLiquidationFee(D_coll_Asset)).div(
        A_coll_Asset.add(B_coll_Asset).add(C_coll_Asset)
  ...
  it('redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Distributes correct rewards', async () => {
    ...
    assert.isAtMost(
      th.getDifference(
        await troveManagerHelpers.getPendingAssetReward(erc20.address, bob),
        B_pendingRewardsAfterL1_Asset
      ),
      1000000
    )
    assert.isAtMost(
      th.getDifference(
        await troveManagerHelpers.getPendingAssetReward(erc20.address, carol),
        C_pendingRewardsAfterL1_Asset
      ),
      1000000
    )
    ...
    const C_pendingRewardsAfterL2_Asset = C_collAfterL1_Asset.mul(
      th.applyLiquidationFee(B_collAfterL1_Asset)
    ).div(C_collAfterL1_Asset.add(D_coll_Asset))
    ...
    assert.isAtMost(
      th.getDifference(
        await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, dennis),
    ...
    assert.isAtMost(
      th.getDifference(
        await troveManagerHelpers.getPendingAssetReward(erc20.address, carol),
        C_pendingRewardsAfterL2_Asset
      ),
      1000000
    )
    assert.isAtMost(
      th.getDifference(
        await troveManagerHelpers.getPendingAssetReward(erc20.address, dennis),
        D_pendingRewardsAfterL2_Asset
      ),
      1000000
    )
    ...
    const totalCollateralSnapshotAfterL2_Asset = C_coll.sub(C_withdrawnColl)
      .add(D_coll_Asset)
    ...
    const { collateral: E_coll_Asset, totalDebt: E_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 16)),
      extraDCHFAmount: dec(110, 18),
      extraParams: { from: erin },
    ...
    const carol_rawColl = (await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[
      th.TROVE_COLL_INDEX
    ].toString()
    ...
    const dennis_rawColl = (await troveManagerHelpers.Troves(dennis, ZERO_ADDRESS))[
      th.TROVE_COLL_INDEX
    ].toString()
    ...
    const erin_rawColl = (await troveManagerHelpers.Troves(erin, ZERO_ADDRESS))[
      th.TROVE_COLL_INDEX
    ].toString()
    const erin_pendingETHReward = (
      await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, erin)
    ).toString()

    const carol_rawColl_Asset = (await troveManagerHelpers.Troves(carol, erc20.address))[
      th.TROVE_COLL_INDEX
    ].toString()
    const carol_pendingETHReward_Asset = (
      await troveManagerHelpers.getPendingAssetReward(erc20.address, carol)
    ).toString()

    const dennis_rawColl_Asset = (await troveManagerHelpers.Troves(dennis, erc20.address))[
      th.TROVE_COLL_INDEX
    ].toString()
    const dennis_pendingETHReward_Asset = (
      await troveManagerHelpers.getPendingAssetReward(erc20.address, dennis)
    ).toString()

    const erin_rawColl_Asset = (await troveManagerHelpers.Troves(erin, erc20.address))[
      th.TROVE_COLL_INDEX
    ].toString()
    const erin_pendingETHReward_Asset = (
      await troveManagerHelpers.getPendingAssetReward(erc20.address, erin)
    ).toString()
    ...
    assert.isAtMost(
      th.getDifference(erin_pendingETHReward_Asset, E_collAfterL3_Asset.sub(E_coll_Asset)),
      1000000
    )
    ...
    const totalStakesSnapshot = (await troveManagerHelpers.totalStakesSnapshot(ZERO_ADDRESS)).toString()
    const totalCollateralSnapshot = (
      await troveManagerHelpers.totalCollateralSnapshot(ZERO_ADDRESS)
    ).toString()

    const totalStakesSnapshotAfterL3_Asset = totalStakesSnapshotAfterL2_Asset.add(
      D_addedColl.add(E_coll_Asset)
        .mul(totalStakesSnapshotAfterL2_Asset)
        .div(totalCollateralSnapshotAfterL2_Asset)
    )
    ...
    const totalStakesSnapshot_Asset = (
      await troveManagerHelpers.totalStakesSnapshot(erc20.address)
    ).toString()
    const totalCollateralSnapshot_Asset = (
      await troveManagerHelpers.totalCollateralSnapshot(erc20.address)
    ).toString()
    ...
    // check DCHF gas compensation
    assert.equal((await dchfToken.balanceOf(owner)).toString(), toBN(dec(600, 18)).mul(toBN(2)).toString())
  })
  ...
  it('redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Varying coll. Distributes correct rewards', async () => {
    ...
    */
    ...
    const carol_Stake = (await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[
    ...
    const dennis_Stake = (await troveManagerHelpers.Troves(dennis, ZERO_ADDRESS))[
      th.TROVE_STAKE_INDEX
    ].toString()
    ...
    const erin_Stake = (await troveManagerHelpers.Troves(erin, ZERO_ADDRESS))[th.TROVE_STAKE_INDEX].toString()
    ...
    const carol_Stake_Asset = (await troveManagerHelpers.Troves(carol, erc20.address))[
      th.TROVE_STAKE_INDEX
    ].toString()
    ...
    const dennis_Stake_Asset = (await troveManagerHelpers.Troves(dennis, erc20.address))[
      th.TROVE_STAKE_INDEX
    ].toString()
    ...
    const erin_Stake_Asset = (await troveManagerHelpers.Troves(erin, erc20.address))[
      th.TROVE_STAKE_INDEX
    ].toString()
