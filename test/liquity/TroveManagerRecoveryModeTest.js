...
contract('TroveManager - in Recovery Mode', async (accounts) => {
  ...
  it('checkRecoveryMode(): Returns true if TCR falls below CCR', async () => {
    ...
    // price drops to 1ETH:150DCHF, reducing TCR below 150%. setPrice() calls checkTCRAndSetRecoveryMode() internally.
  ...
  it('liquidate(), with ICR < 100%: removes stake and updates totalStakes', async () => {
    ...
    const bob_Stake_Before = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_STAKE_INDEX]
    const totalStakes_Before = await troveManagerHelpers.totalStakes(ZERO_ADDRESS)

    const bob_Stake_Before_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[
      th.TROVE_STAKE_INDEX
    ]
    ...
    const bob_Stake_After = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_STAKE_INDEX]
    const totalStakes_After = await troveManagerHelpers.totalStakes(ZERO_ADDRESS)

    const bob_Stake_After_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_STAKE_INDEX]
  ...
  it('liquidate(), with ICR < 100%: updates system snapshots correctly', async () => {
    ...
    // --- TEST ---
    // price drops to 1ETH:100DCHF, reducing TCR below 150%, and all Troves below 100% ICR
    await priceFeed.setPrice('100000000000000000000')
    ...
    const totalStakesSnaphot_before = (await troveManagerHelpers.totalStakesSnapshot(ZERO_ADDRESS)).toString()
    const totalCollateralSnapshot_before = (
      await troveManagerHelpers.totalCollateralSnapshot(ZERO_ADDRESS)
    ).toString()

    const totalStakesSnaphot_before_Asset = (
    ...
    const A_reward_Asset = th
      .applyLiquidationFee(D_coll_Asset)
      .mul(A_coll_Asset)
      .div(A_coll_Asset.add(B_coll_Asset))
    const B_reward_Asset = th
      .applyLiquidationFee(D_coll_Asset)
      .mul(B_coll_Asset)
      .div(A_coll_Asset.add(B_coll_Asset))
    ...
    const totalStakesSnaphot_After = await troveManagerHelpers.totalStakesSnapshot(ZERO_ADDRESS)
    const totalCollateralSnapshot_After = await troveManagerHelpers.totalCollateralSnapshot(ZERO_ADDRESS)

    const totalStakesSnaphot_After_Asset = await troveManagerHelpers.totalStakesSnapshot(erc20.address)
    const totalCollateralSnapshot_After_Asset = await troveManagerHelpers.totalCollateralSnapshot(
      erc20.address
    )
  ...
  it('liquidate(), with ICR < 100%: closes the Trove and removes it from the Trove array', async () => {
    ...
    const bob_TroveStatus_Before = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[
      th.TROVE_STATUS_INDEX
    ]
    const bob_Trove_isInSortedList_Before = await sortedTroves.contains(ZERO_ADDRESS, bob)

    const bob_TroveStatus_Before_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[
      th.TROVE_STATUS_INDEX
    ]
    ...
    const bob_TroveStatus_After = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX]
    const bob_Trove_isInSortedList_After = await sortedTroves.contains(ZERO_ADDRESS, bob)

    const bob_TroveStatus_After_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[
      th.TROVE_STATUS_INDEX
    ]
  ...
  it('liquidate(), with 100 < ICR < 110%: removes stake and updates totalStakes', async () => {
    ...
    // --- TEST ---
    // price drops to 1ETH:100DCHF, reducing TCR to 117%
    await priceFeed.setPrice('100000000000000000000')
  ...
  it('liquidate(), with 100% < ICR < 110%: updates system snapshots correctly', async () => {
    ...
    const totalStakesSnaphot_1 = (await troveManagerHelpers.totalStakesSnapshot(ZERO_ADDRESS)).toString()
    const totalCollateralSnapshot_1 = (
      await troveManagerHelpers.totalCollateralSnapshot(ZERO_ADDRESS)
    ).toString()

    const totalStakesSnaphot_1_Asset = (
      await troveManagerHelpers.totalStakesSnapshot(erc20.address)
    ).toString()
    const totalCollateralSnapshot_1_Asset = (
      await troveManagerHelpers.totalCollateralSnapshot(erc20.address)
    ).toString()
    ...
    const totalStakesSnaphot_2 = (await troveManagerHelpers.totalStakesSnapshot(ZERO_ADDRESS)).toString()
    const totalCollateralSnapshot_2 = (
      await troveManagerHelpers.totalCollateralSnapshot(ZERO_ADDRESS)
    ).toString()
    const totalStakesSnaphot_2_Asset = (
      await troveManagerHelpers.totalStakesSnapshot(erc20.address)
    ).toString()
    const totalCollateralSnapshot_2_Asset = (
      await troveManagerHelpers.totalCollateralSnapshot(erc20.address)
    ).toString()
    ...
    const totalStakesSnaphot_3 = await troveManagerHelpers.totalStakesSnapshot(ZERO_ADDRESS)
    const totalCollateralSnapshot_3 = await troveManagerHelpers.totalCollateralSnapshot(ZERO_ADDRESS)

    const totalStakesSnaphot_3_Asset = await troveManagerHelpers.totalStakesSnapshot(erc20.address)
    const totalCollateralSnapshot_3_Asset = await troveManagerHelpers.totalCollateralSnapshot(erc20.address)
  ...
  it('liquidate(), with 100% < ICR < 110%: offsets as much debt as possible with the Stability Pool, then redistributes the remainder coll and debt', async () => {
    ...
    // check pool DCHF before liquidation
    const stabilityPoolDCHF_Before = (await stabilityPool.getTotalDCHFDeposits()).toString()
    assert.equal(stabilityPoolDCHF_Before, '390000000000000000000')

    const stabilityPoolDCHF_Before_Asset = (await stabilityPoolERC20.getTotalDCHFDeposits()).toString()
    assert.equal(stabilityPoolDCHF_Before_Asset, '390000000000000000000')
    ...
    /* Now, liquidate Bob. Liquidated coll is 21 ether, and liquidated debt is 2000 DCHF.
    
    With 390 DCHF in the StabilityPool, 390 DCHF should be offset with the pool, leaving 0 in the pool.
  
    Stability Pool rewards for alice should be:
    DCHFLoss: 390DCHF
    AssetGain: (390 / 2000) * 21*0.995 = 4.074525 ether

    After offsetting 390 DCHF and 4.074525 ether, the remainders - 1610 DCHF and 16.820475 ether - should be redistributed to all active Troves.
   */
    // Liquidate Bob
    await troveManager.liquidate(ZERO_ADDRESS, bob, { from: owner })
    await troveManager.liquidate(erc20.address, bob, { from: owner })

    const aliceDeposit = await stabilityPool.getCompoundedDCHFDeposit(alice)
    ...
    const aliceDeposit_Asset = await stabilityPoolERC20.getCompoundedDCHFDeposit(alice)
    const aliceETHGain_Asset = await stabilityPoolERC20.getDepositorAssetGain(alice)
    const aliceExpectedETHGain_Asset = spDeposit
      .mul(th.applyLiquidationFee(B_coll_Asset))
      .div(B_totalDebt_Asset)
    ...
    /* Now, check redistribution to active Troves. Remainders of 1610 DCHF and 16.82 ether are distributed.
    ...
    L_DCHFDebt = 1610 / 6 = 268.333 DCHF
    L_ETH = 16.820475 /6 =  2.8034125 ether
    */
    const L_DCHFDebt = (await troveManagerHelpers.L_DCHFDebts(ZERO_ADDRESS)).toString()
    const L_ETH = (await troveManagerHelpers.L_ASSETS(ZERO_ADDRESS)).toString()

    const L_DCHFDebt_Asset = (await troveManagerHelpers.L_DCHFDebts(erc20.address)).toString()
    const L_ETH_Asset = (await troveManagerHelpers.L_ASSETS(erc20.address)).toString()

    assert.isAtMost(
      th.getDifference(L_DCHFDebt, B_totalDebt.sub(spDeposit).mul(mv._1e18BN).div(A_coll.add(D_coll))),
      100
    )
    assert.isAtMost(
      th.getDifference(
  ...
  it('liquidate(), with ICR > 110%, trove has lowest ICR, and StabilityPool is empty: does nothing', async () => {
    ...
    const bob_ICR = (await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price)).toString()
    const alice_ICR = (await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, alice, price)).toString()
    const dennis_ICR = (await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, dennis, price)).toString()

    const bob_ICR_Asset = (await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)).toString()
    const alice_ICR_Asset = (await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)).toString()
    const dennis_ICR_Asset = (
    ...
    await assertRevert(
      troveManager.liquidate(ZERO_ADDRESS, bob, { from: owner }),
      'TroveManager: nothing to liquidate'
    )
    await assertRevert(
      troveManager.liquidate(erc20.address, bob, { from: owner }),
      'TroveManager: nothing to liquidate'
    )
    ...
    const bob_Trove_Asset = await troveManagerHelpers.Troves(bob, erc20.address)
  ...
  // --- liquidate(), applied to trove with ICR > 110% that has the lowest ICR, and Stability Pool DCHF is GREATER THAN liquidated debt ---

  it('liquidate(), with 110% < ICR < TCR, and StabilityPool DCHF > debt to liquidate: offsets the trove entirely with the pool', async () => {
    // --- SETUP ---
    ...
    /* Check accrued Stability Pool rewards after. Total Pool deposits was 1490 DCHF, Alice sole depositor.
    As liquidated debt (250 DCHF) was completely offset

    Alice's expected compounded deposit: (1490 - 250) = 1240DCHF
    Alice's expected ETH gain:  Bob's liquidated capped coll (minus gas comp), 2.75*0.995 ether
  
    */
    const aliceExpectedDeposit = await stabilityPool.getCompoundedDCHFDeposit(alice)
    const aliceExpectedETHGain = await stabilityPool.getDepositorAssetGain(alice)
    const aliceExpectedDeposit_Asset = await stabilityPoolERC20.getCompoundedDCHFDeposit(alice)
    ...
    assert.isAtMost(
      th.getDifference(aliceExpectedDeposit_Asset.toString(), spDeposit.sub(B_totalDebt_Asset)),
      2000
    )
    assert.isAtMost(
      th.getDifference(
        ...
        th.applyLiquidationFee(
          B_totalDebt_Asset.div(toBN(10 ** 10))
            .mul(th.toBN(dec(11, 17)))
            .div(price)
        )
      ),
      3000
    ...
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getCollateral(ZERO_ADDRESS, bob),
      bob_remainingCollateral
    )

    const bob_remainingCollateral_Asset = B_coll_Asset.sub(
    ...
    th.assertIsApproximatelyEqual(
      ...
      bob_balanceBefore_Asset.add(th.toBN(bob_remainingCollateral_Asset).div(toBN(10 ** 10)))
    )
  })

  it('liquidate(), with ICR% = 110 < TCR, and StabilityPool DCHF > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DCHF of debt, and Dennis up to 150, resulting in ICRs of 266%.
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(266, 16)),
      extraDCHFAmount: B_totalDebt_Asset,
      extraParams: { from: alice },
    })
    await openTrove({
      asset: erc20.address,
  ...
  it('liquidate(), with  110% < ICR < TCR, and StabilityPool DCHF > debt to liquidate: removes stake and updates totalStakes', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DCHF of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DCHF of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it('liquidate(), with  110% < ICR < TCR, and StabilityPool DCHF > debt to liquidate: updates system snapshots', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DCHF of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DCHF of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { totalDebt: B_totalDebt } = await openTrove({
  ...
  it('liquidate(), with 110% < ICR < TCR, and StabilityPool DCHF > debt to liquidate: closes the Trove', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DCHF of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DCHF of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it('liquidate(), with 110% < ICR < TCR, and StabilityPool DCHF > debt to liquidate: can liquidate troves out of order', async () => {
    // taking out 1000 DCHF, CR of 200%
    ...
    await openTrove({
      ICR: toBN(dec(200, 16)),
      extraDCHFAmount: totalLiquidatedDebt,
      extraParams: { from: whale },
    })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 16)),
      extraDCHFAmount: totalLiquidatedDebt_Asset,
      extraParams: { from: whale },
    })
    ...
    const ICR_C = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)
    const ICR_D = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, dennis, price)
    ...
    const ICR_C_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)
    const ICR_D_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, dennis, price)
    ...
    assert.equal((await troveManagerHelpers.Troves(dennis, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX], '3')
    assert.equal((await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX], '3')
    assert.equal((await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX], '3')

    assert.equal((await troveManagerHelpers.Troves(dennis, erc20.address))[th.TROVE_STATUS_INDEX], '3')
    assert.equal((await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_STATUS_INDEX], '3')
    assert.equal((await troveManagerHelpers.Troves(carol, erc20.address))[th.TROVE_STATUS_INDEX], '3')
    ...
    const dennis_remainingCollateral_Asset = D_coll_Asset.sub(
      D_totalDebt_Asset.mul(th.toBN(dec(11, 17))).div(price)
    )
    const bob_remainingCollateral_Asset = B_coll_Asset.sub(
      B_totalDebt_Asset.mul(th.toBN(dec(11, 17))).div(price)
    )
    const carol_remainingCollateral_Asset = C_coll_Asset.sub(
      C_totalDebt_Asset.mul(th.toBN(dec(11, 17))).div(price)
    )

    th.assertIsApproximatelyEqual(
      await collSurplusPool.getCollateral(ZERO_ADDRESS, dennis),
    ...
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getCollateral(ZERO_ADDRESS, carol),
    ...
    assert.isTrue(
      dennis_balanceAfter_Asset.eq(
        dennis_balanceBefore_Asset.add(th.toBN(dennis_remainingCollateral_Asset).div(toBN(10 ** 10)))
      )
    )
    ...
    th.assertIsApproximatelyEqual(
      ...
      carol_balanceBefore_Asset.add(th.toBN(carol_remainingCollateral_Asset).div(toBN(10 ** 10)))
    )
  })

  /* --- liquidate() applied to trove with ICR > 110% that has the lowest ICR, and Stability Pool 
  DCHF is LESS THAN the liquidated debt: a non fullfilled liquidation --- */

  it('liquidate(), with ICR > 110%, and StabilityPool DCHF < liquidated debt: Trove remains active', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DCHF of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DCHF of debt, resulting in ICR of 240%. Bob has lowest ICR.
    ...
    /* Since the pool only contains 100 DCHF, and Bob's pre-liquidation debt was 250 DCHF,
  ...
  it('liquidate(), with ICR > 110%, and StabilityPool DCHF < liquidated debt: Trove remains in TroveOwners array', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DCHF of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DCHF of debt, resulting in ICR of 240%. Bob has lowest ICR.
    await openTrove({ ICR: toBN(dec(266, 16)), extraDCHFAmount: dec(1500, 18), extraParams: { from: alice } })
    ...
    const arrayLength = (await troveManagerHelpers.getTroveOwnersCount(ZERO_ADDRESS)).toNumber()
    let addressFound = false
    let addressIdx = 0

    for (let i = 0; i < arrayLength; i++) {
      const address = (await troveManagerHelpers.TroveOwners(ZERO_ADDRESS, i)).toString()
      if (address == bob) {
        addressFound = true
        addressIdx = i
      }
    ...
    const idxOnStruct = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_ARRAY_INDEX].toString()
    assert.equal(addressIdx.toString(), idxOnStruct)

    const arrayLength_Asset = (await troveManagerHelpers.getTroveOwnersCount(erc20.address)).toNumber()
    let addressFound_Asset = false
    let addressIdx_Asset = 0

    for (let i = 0; i < arrayLength_Asset; i++) {
      const address = (await troveManagerHelpers.TroveOwners(erc20.address, i)).toString()
      if (address == bob) {
    ...
    // Check TroveOwners idx on trove struct == idx of address found in TroveOwners array
    const idxOnStruct_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[
      th.TROVE_ARRAY_INDEX
    ].toString()
    assert.equal(addressIdx_Asset.toString(), idxOnStruct_Asset)
  })

  it('liquidate(), with ICR > 110%, and StabilityPool DCHF < liquidated debt: nothing happens', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DCHF of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DCHF of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
    ...
    /*  Since Bob's debt (250 DCHF) is larger than all DCHF in the Stability Pool, Liquidation won’t happen
  ...
  it('liquidate(), with ICR > 110%, and StabilityPool DCHF < liquidated debt: updates system shapshots', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DCHF of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DCHF of debt, resulting in ICR of 240%. Bob has lowest ICR.
    await openTrove({ ICR: toBN(dec(266, 16)), extraDCHFAmount: dec(1500, 18), extraParams: { from: alice } })
    ...
    const totalStakesSnaphot_After = (await troveManagerHelpers.totalStakesSnapshot(ZERO_ADDRESS)).toString()
    const totalCollateralSnapshot_After = (
      await troveManagerHelpers.totalCollateralSnapshot(ZERO_ADDRESS)
    ).toString()
    const totalStakesSnaphot_After_Asset = (
      await troveManagerHelpers.totalStakesSnapshot(erc20.address)
    ).toString()
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool DCHF < liquidated debt: causes correct Pool offset and ETH gain, and doesn't redistribute to active troves", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DCHF of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DCHF of debt, resulting in ICR of 240%. Bob has lowest ICR.
    await openTrove({ ICR: toBN(dec(266, 16)), extraDCHFAmount: dec(1500, 18), extraParams: { from: alice } })
    ...
    const L_DCHFDebt_After = (await troveManagerHelpers.L_DCHFDebts(ZERO_ADDRESS)).toString()
    const L_ETH_After = (await troveManagerHelpers.L_ASSETS(ZERO_ADDRESS)).toString()

    const L_DCHFDebt_After_Asset = (await troveManagerHelpers.L_DCHFDebts(erc20.address)).toString()
    const L_ETH_After_Asset = (await troveManagerHelpers.L_ASSETS(erc20.address)).toString()

    assert.equal(L_DCHFDebt_After, '0')
    assert.equal(L_ETH_After, '0')
    assert.equal(L_DCHFDebt_After_Asset, '0')
    assert.equal(L_ETH_After_Asset, '0')
  })

  it('liquidate(), with ICR > 110%, and StabilityPool DCHF < liquidated debt: ICR of non liquidated trove does not change', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DCHF of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DCHF of debt, resulting in ICR of 240%. Bob has lowest ICR.
    ...
    const carol_ICR_Before = (await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)).toString()

    const bob_ICR_Before_Asset = (
      await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)
    ).toString()
    const carol_ICR_Before_Asset = (
    ...
    const bob_Coll_Before = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
    const bob_Debt_Before = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]

    const bob_Coll_Before_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_COLL_INDEX]
    const bob_Debt_Before_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_DEBT_INDEX]
    ...
    //Check SP DCHF has been completely emptied
    assert.equal((await stabilityPool.getTotalDCHFDeposits()).toString(), dec(100, 18))
    assert.equal((await stabilityPoolERC20.getTotalDCHFDeposits()).toString(), dec(100, 18))
    ...
    const bob_Coll_After = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
    const bob_Debt_After = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]
    const bob_Coll_After_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_COLL_INDEX]
    const bob_Debt_After_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_DEBT_INDEX]
    ...
    await dchfToken.transfer(bob, toBN(dec(100, 18)).mul(toBN(2)), { from: alice })
    ...
    // Alice provides another 50 DCHF to pool
    ...
    const carol_Coll_Before = (await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
    const carol_Debt_Before = (await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]

    const carol_Coll_Before_Asset = (await troveManagerHelpers.Troves(carol, erc20.address))[
      th.TROVE_COLL_INDEX
    ]
    const carol_Debt_Before_Asset = (await troveManagerHelpers.Troves(carol, erc20.address))[
      th.TROVE_DEBT_INDEX
    ]

    // Confirm Carol is last trove in list, and has >110% ICR
    assert.equal(await sortedTroves.getLast(ZERO_ADDRESS), carol)
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)).gt(mv._MCR))

    assert.equal(await sortedTroves.getLast(erc20.address), carol)
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)).gt(mv._MCR))
    ...
    //Check SP DCHF has been completely emptied
    assert.equal((await stabilityPool.getTotalDCHFDeposits()).toString(), dec(150, 18))
    assert.equal((await stabilityPoolERC20.getTotalDCHFDeposits()).toString(), dec(150, 18))

    // Check Carol's collateral and debt remains the same
    const carol_Coll_After = (await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
    const carol_Debt_After = (await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]
    const carol_Coll_After_Asset = (await troveManagerHelpers.Troves(carol, erc20.address))[
      th.TROVE_COLL_INDEX
    ]
    const carol_Debt_After_Asset = (await troveManagerHelpers.Troves(carol, erc20.address))[
      th.TROVE_DEBT_INDEX
    ]
  ...
  it("liquidate(): Doesn't liquidate undercollateralized trove if it is the only trove in the system", async () => {
    // Alice creates a single trove with 0.62 ETH and a debt of 62 DCHF, and provides 10 DCHF to SP
    await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: alice } })
    await openTrove({ asset: erc20.address, ICR: toBN(dec(200, 16)), extraParams: { from: alice } })
    ...
    await assertRevert(
      troveManager.liquidate(ZERO_ADDRESS, alice, { from: owner }),
      'TroveManager: nothing to liquidate'
    )
    await assertRevert(
      troveManager.liquidate(erc20.address, alice, { from: owner }),
      'TroveManager: nothing to liquidate'
    )
  ...
  it('liquidate(): does nothing if trove has >= 110% ICR and the Stability Pool is empty', async () => {
    ...
    const DCHFinSP_Asset = (await stabilityPoolERC20.getTotalDCHFDeposits()).toString()
    assert.equal(DCHFinSP_Asset, '0')
  ...
  it("liquidate(): does nothing if trove ICR >= TCR, and SP covers trove's debt", async () => {
    ...
    const ICR_A = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, A, price)
    const ICR_B = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, B, price)
    const ICR_C = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, C, price)

    const ICR_A_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, A, price)
    const ICR_B_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, B, price)
    ...
    assert.equal(await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, A), 1) // Status 1 -> active

    assert.isTrue(await sortedTroves.contains(erc20.address, A))
    assert.equal(await troveManagerHelpers.getTroveStatus(erc20.address, A), 1)
    ...
    assert.equal(await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, C), 3) // Status liquidated

    assert.isFalse(await sortedTroves.contains(erc20.address, C))
    assert.equal(await troveManagerHelpers.getTroveStatus(erc20.address, C), 3)
  })
  ...
  it('liquidate(): reverts if trove is non-existent', async () => {
    ...
    assert.equal(await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, carol), 0)
    assert.isFalse(await sortedTroves.contains(ZERO_ADDRESS, carol))

    assert.equal(await troveManagerHelpers.getTroveStatus(erc20.address, carol), 0)
    assert.isFalse(await sortedTroves.contains(erc20.address, carol))
  ...
  it('liquidate(): reverts if trove has been closed', async () => {
    ...
    assert.isFalse(await sortedTroves.contains(ZERO_ADDRESS, carol))
    assert.equal(await troveManagerHelpers.getTroveStatus(ZERO_ADDRESS, carol), 3)

    assert.isFalse(await sortedTroves.contains(erc20.address, carol))
    assert.equal(await troveManagerHelpers.getTroveStatus(erc20.address, carol), 3)
  ...
  it('liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt', async () => {
    ...
    /*  Since there is 0 DCHF in the stability Pool, A, with ICR >110%, should stay active.
  ...
  it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(220, 16)),
      extraDCHFAmount: spDeposit_Asset,
      extraParams: { from: bob },
    })
    ...
    //Dennis provides 200 DCHF to SP
    ...
    assert.isAtMost(
      th.getDifference(dennis_Deposit_Before_Asset, spDeposit_Asset.sub(C_totalDebt_Asset)),
      1000
    )
    assert.isAtMost(
      th.getDifference(dennis_ETHGain_Before_Asset, th.applyLiquidationFee(C_coll_Asset.div(toBN(10 ** 10)))),
      1000
    )
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(220, 16)),
      extraDCHFAmount: dec(1000, 18),
      extraParams: { from: whale },
    })

    const { DCHFAmount: A_DCHFAmount } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraDCHFAmount: dec(300, 18),
      extraParams: { from: alice },
    })
  ...
  it('liquidate(), with 110% < ICR < TCR, can claim collateral, re-open, be reedemed and claim again', async () => {
    ...
    const { collateral: B_coll_2_Asset, netDebt: B_netDebt_2_Asset } = await openTrove({
      asset: erc20.address,
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(266, 16)),
      extraDCHFAmount: B_netDebt_2_Asset,
      extraParams: { from: dennis },
    })
    ...
    th.assertIsApproximatelyEqual(
      ...
      bob_balanceBefore_2_Asset.add(th.toBN(bob_surplus_Asset).div(toBN(10 ** 10)))
    )
  })
  ...
  it('liquidate(), with 110% < ICR < TCR, can claim collateral, after another claim from a redemption', async () => {
    ...
    const { collateral: B_coll_Asset, netDebt: B_netDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(222, 16)),
      extraDCHFAmount: dec(90, 18),
      extraParams: { from: bob },
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(266, 16)),
      extraDCHFAmount: B_netDebt_Asset,
      extraParams: { from: dennis },
    })
    ...
    th.assertIsApproximatelyEqual(
      bob_balanceAfter_Asset,
      bob_balanceBefore_Asset.add(bob_surplus_Asset.div(toBN(10 ** 10)))
    )

    // Bob re-opens the trove, price 200, total debt 250 DCHF, ICR = 240% (lowest one)
    const { collateral: B_coll_2, totalDebt: B_totalDebt_2 } = await openTrove({
      ICR: toBN(dec(240, 16)),
      extraParams: { from: bob, value: _3_Ether },
    })
    const { collateral: B_coll_2_Asset, totalDebt: B_totalDebt_2_Asset } = await openTrove({
      asset: erc20.address,
      assetSent: _3_Ether,
      ICR: toBN(dec(240, 16)),
      extraParams: { from: bob },
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(266, 16)),
      extraDCHFAmount: B_totalDebt_2_Asset,
      extraParams: { from: alice },
    })
    ...
    th.assertIsApproximatelyEqual(
      (await collSurplusPool.getCollateral(ZERO_ADDRESS, bob)).toString(),
      bob_remainingCollateral.toString()
    )

    const bob_remainingCollateral_Asset = B_coll_2_Asset.sub(
      B_totalDebt_2_Asset.mul(th.toBN(dec(11, 17))).div(price)
    )
    th.assertIsApproximatelyEqual(
    ...
    th.assertIsApproximatelyEqual(
      bob_balanceAfter_2_Asset,
      bob_balanceBefore_2_Asset.add(th.toBN(bob_remainingCollateral_Asset).div(toBN(10 ** 10)))
    )
  })
  ...
  it('liquidateTroves(): With all ICRs > 110%, Liquidates Troves until system leaves recovery mode', async () => {
    ...
    // Everyone withdraws some DCHF from their Trove, resulting in different ICRs
    ...
    const { totalDebt: E_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(261, 16)),
      extraParams: { from: erin },
    })
    ...
    const { totalDebt: G_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(235, 16)),
      extraParams: { from: greta },
    })
    const { totalDebt: H_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(222, 16)),
      extraDCHFAmount: dec(5000, 18),
      extraParams: { from: harry },
    ...
    // price drops
    // price drops to 1ETH:90DCHF, reducing TCR below 150%
    ...
    */
    ...
    const freddy_ICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, freddy, price)
    const greta_ICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, greta, price)
    const harry_ICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, harry, price)
    const TCR = await th.getTCR(contracts)
    ...
    const freddy_ICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, freddy, price)
    const greta_ICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, greta, price)
    const harry_ICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, harry, price)
    const TCR_Asset = await th.getTCR(contracts, erc20.address)
    ...
    // get all Troves
    const alice_Trove = await troveManagerHelpers.Troves(alice, ZERO_ADDRESS)
    const bob_Trove = await troveManagerHelpers.Troves(bob, ZERO_ADDRESS)
    const carol_Trove = await troveManagerHelpers.Troves(carol, ZERO_ADDRESS)
    const dennis_Trove = await troveManagerHelpers.Troves(dennis, ZERO_ADDRESS)
    const erin_Trove = await troveManagerHelpers.Troves(erin, ZERO_ADDRESS)
    const freddy_Trove = await troveManagerHelpers.Troves(freddy, ZERO_ADDRESS)
    const greta_Trove = await troveManagerHelpers.Troves(greta, ZERO_ADDRESS)
    const harry_Trove = await troveManagerHelpers.Troves(harry, ZERO_ADDRESS)

    const alice_Trove_Asset = await troveManagerHelpers.Troves(alice, erc20.address)
    const bob_Trove_Asset = await troveManagerHelpers.Troves(bob, erc20.address)
    const carol_Trove_Asset = await troveManagerHelpers.Troves(carol, erc20.address)
    const dennis_Trove_Asset = await troveManagerHelpers.Troves(dennis, erc20.address)
    const erin_Trove_Asset = await troveManagerHelpers.Troves(erin, erc20.address)
    const freddy_Trove_Asset = await troveManagerHelpers.Troves(freddy, erc20.address)
    const greta_Trove_Asset = await troveManagerHelpers.Troves(greta, erc20.address)
    const harry_Trove_Asset = await troveManagerHelpers.Troves(harry, erc20.address)
  ...
  it('liquidateTroves(): Liquidates Troves until 1) system has left recovery mode AND 2) it reaches a Trove with ICR >= 110%', async () => {
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(400, 16)),
      extraDCHFAmount: liquidationAmount_Asset,
      extraParams: { from: alice },
    })
    ...
    // price drops to 1ETH:85DCHF, reducing TCR below 150%
   ...
   After the price drop and prior to any liquidations, ICR should be:
    ...
    Freddy      102%
    */
    alice_ICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, alice, price)
    bob_ICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price)
    carol_ICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)
    dennis_ICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, dennis, price)
    erin_ICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, erin, price)
    freddy_ICR = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, freddy, price)

    alice_ICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)
    bob_ICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)
    carol_ICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)
    dennis_ICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, dennis, price)
    erin_ICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, erin, price)
    freddy_ICR_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, freddy, price)
  ...
  it('liquidateTroves(): liquidates only up to the requested number of undercollateralized troves', async () => {
    ...
    assert.equal(await troveManagerHelpers.getTroveOwnersCount(ZERO_ADDRESS), '3')
    assert.equal(await troveManagerHelpers.getTroveOwnersCount(erc20.address), '3')
  ...
  it('liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves', async () => {
    ...
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, freddy, price)).lte(mv._MCR))
    ...
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, freddy, price)).lte(mv._MCR))
    ...
    assert.equal(
      (await troveManagerHelpers.Troves(freddy, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX].toString(),
      '3'
    )
    ...
    assert.equal(
      (await troveManagerHelpers.Troves(freddy, erc20.address))[th.TROVE_STATUS_INDEX].toString(),
      '3'
    )
  })
  ...
  it('liquidateTroves(): a liquidation sequence containing Pool offsets increases the TCR', async () => {
    ...
    await openTrove({
      ICR: toBN(dec(198, 16)),
      extraDCHFAmount: dec(101, 18),
      extraParams: { from: defaulter_1 },
    })
    await openTrove({
      ICR: toBN(dec(184, 16)),
      extraDCHFAmount: dec(217, 18),
      extraParams: { from: defaulter_2 },
    })
    await openTrove({
      ICR: toBN(dec(183, 16)),
      extraDCHFAmount: dec(328, 18),
      extraParams: { from: defaulter_3 },
    })
    await openTrove({
      ICR: toBN(dec(186, 16)),
      extraDCHFAmount: dec(431, 18),
    ...
    assert.isTrue(await th.ICRbetween100and110(defaulter_3, troveManagerHelpers, price))
    assert.isTrue(await th.ICRbetween100and110(defaulter_4, troveManagerHelpers, price))
    ...
    assert.isTrue(await th.ICRbetween100and110(defaulter_3, troveManagerHelpers, price, erc20.address))
    assert.isTrue(await th.ICRbetween100and110(defaulter_4, troveManagerHelpers, price, erc20.address))
    ...
    // Check Stability Pool has 500 DCHF
  ...
  it('liquidateTroves(): A liquidation sequence of pure redistributions decreases the TCR, due to gas compensation, but up to 0.5%', async () => {
    ...
    const { collateral: d1_coll_Asset, totalDebt: d1_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(198, 16)),
      extraDCHFAmount: dec(101, 18),
      extraParams: { from: defaulter_1 },
    })
    const { collateral: d2_coll_Asset, totalDebt: d2_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(184, 16)),
      extraDCHFAmount: dec(217, 18),
      extraParams: { from: defaulter_2 },
    })
    const { collateral: d3_coll_Asset, totalDebt: d3_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(183, 16)),
      extraDCHFAmount: dec(328, 18),
      extraParams: { from: defaulter_3 },
    })
    const { collateral: d4_coll_Asset, totalDebt: d4_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(166, 16)),
      extraDCHFAmount: dec(431, 18),
      extraParams: { from: defaulter_4 },
    ...
    assert.isAtMost(
      th.getDifference(TCR_After_Asset, totalCollAfter_Asset.mul(price).div(totalDebtAfter_Asset)),
      1000
    )
  ...
  it('liquidateTroves(): does nothing if all troves have ICR > 110% and Stability Pool is empty', async () => {
    ...
    // Confirm 0 DCHF in Stability Pool
    assert.equal((await stabilityPool.getTotalDCHFDeposits()).toString(), '0')
    assert.equal((await stabilityPoolERC20.getTotalDCHFDeposits()).toString(), '0')
  ...
  it('liquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves', async () => {
    ...
    const { collateral: F_coll_Asset, totalDebt: F_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(222, 16)),
      extraParams: { from: freddy },
    })
    const { collateral: G_coll_Asset, totalDebt: G_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(222, 16)),
      extraParams: { from: greta },
    })
    ...
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, freddy, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, greta, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, alice, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)).gte(mv._MCR))

    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, freddy, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, greta, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)).gte(mv._MCR))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)).gte(mv._MCR))

    // Confirm DCHF in Stability Pool
    assert.equal((await stabilityPool.getTotalDCHFDeposits()).toString(), spDeposit.toString())
    assert.equal((await stabilityPoolERC20.getTotalDCHFDeposits()).toString(), spDeposit_Asset.toString())
    ...
    const freddy_remainingCollateral_Asset = F_coll_Asset.sub(
      F_totalDebt_Asset.mul(th.toBN(dec(11, 17))).div(price)
    )
    const greta_remainingCollateral_Asset = G_coll_Asset.sub(
      G_totalDebt_Asset.mul(th.toBN(dec(11, 17))).div(price)
    )
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getCollateral(ZERO_ADDRESS, freddy),
      freddy_remainingCollateral
    )
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getCollateral(ZERO_ADDRESS, greta),
    ...
    th.assertIsApproximatelyEqual(
      ...
      freddy_balanceBefore_Asset.add(th.toBN(freddy_remainingCollateral_Asset.div(toBN(10 ** 10))))
    )
    ...
    th.assertIsApproximatelyEqual(
      ...
      greta_balanceBefore_Asset.add(th.toBN(greta_remainingCollateral_Asset).div(toBN(10 ** 10)))
    )
  })
  ...
  it('liquidateTroves():  emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial', async () => {
    ...
    const entireColl_A = (await troveManagerHelpers.Troves(alice, ZERO_ADDRESS))[th.TROVE_COLL_INDEX].add(
      await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, alice)
    )
    const entireDebt_A = (await troveManagerHelpers.Troves(alice, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX].add(
      await troveManagerHelpers.getPendingDCHFDebtReward(ZERO_ADDRESS, alice)
    )

    const entireColl_A_Asset = (await troveManagerHelpers.Troves(alice, erc20.address))[
      th.TROVE_COLL_INDEX
    ].add(await troveManagerHelpers.getPendingAssetReward(erc20.address, alice))
    const entireDebt_A_Asset = (await troveManagerHelpers.Troves(alice, erc20.address))[
      th.TROVE_DEBT_INDEX
    ].add(await troveManagerHelpers.getPendingDCHFDebtReward(erc20.address, alice))
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const { DCHFAmount: DCHFAmountD } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: dennis },
    })
    const { DCHFAmount: DCHFAmountE } = await openTrove({
      ICR: toBN(dec(133, 16)),
      extraParams: { from: erin },
    })
    const { DCHFAmount: DCHFAmountF } = await openTrove({
      ICR: toBN(dec(111, 16)),
      extraParams: { from: freddy },
    })

    const { DCHFAmount: DCHFAmountD_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 16)),
      extraParams: { from: dennis },
    })
    const { DCHFAmount: DCHFAmountE_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(133, 16)),
      extraParams: { from: erin },
    })
    const { DCHFAmount: DCHFAmountF_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(111, 16)),
      extraParams: { from: freddy },
    })
  ...
  it('liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and ETH gain', async () => {
    ...
    const { DCHFAmount: W_DCHFAmount } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraDCHFAmount: dec(4000, 18),
      extraParams: { from: whale },
    })
    const { DCHFAmount: W_DCHFAmount_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(300, 16)),
      extraDCHFAmount: dec(4000, 18),
      extraParams: { from: whale },
    ...
    const { totalDebt: C_totalDebt_Asset, collateral: C_coll_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(209, 16)),
      extraParams: { from: carol },
    })
    ...
    // Check DCHF in Pool
    assert.equal((await stabilityPool.getTotalDCHFDeposits()).toString(), totalDeposit)
    assert.equal((await stabilityPoolERC20.getTotalDCHFDeposits()).toString(), totalDeposit_Asset)
    ...
    Total DCHF in Pool: 680 DCHF

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 100 + 300 + 100 = 500 DCHF
    Total liquidated ETH = 1 + 3 + 1 = 5 ETH

    Whale DCHF Loss: 500 * (400/680) = 294.12 DCHF
    Alice DCHF Loss:  500 *(40/680) = 29.41 DCHF
    Bob DCHF Loss: 500 * (240/680) = 176.47 DCHF

    Whale remaining deposit: (400 - 294.12) = 105.88 DCHF
    Alice remaining deposit: (40 - 29.41) = 10.59 DCHF
    Bob remaining deposit: (240 - 176.47) = 63.53 DCHF

    Whale ETH Gain: 5*0.995 * (400/680) = 2.93 ETH
    Alice ETH Gain: 5*0.995 *(40/680) = 0.293 ETH
    Bob ETH Gain: 5*0.995 * (240/680) = 1.76 ETH

    Total remaining deposits: 180 DCHF
    ...
    const whale_Deposit_After = (await stabilityPool.getCompoundedDCHFDeposit(whale)).toString()
    ...
    const whale_Deposit_After_Asset = (await stabilityPoolERC20.getCompoundedDCHFDeposit(whale)).toString()
    ...
    assert.isAtMost(
      th.getDifference(
        whale_Deposit_After,
        W_DCHFAmount.sub(liquidatedDebt.mul(W_DCHFAmount).div(totalDeposit))
      ),
      100000
    )
    assert.isAtMost(
      th.getDifference(
        alice_Deposit_After,
        A_DCHFAmount.sub(liquidatedDebt.mul(A_DCHFAmount).div(totalDeposit))
      ),
      100000
    )
    assert.isAtMost(
      th.getDifference(
        bob_Deposit_After,
        B_DCHFAmount.sub(liquidatedDebt.mul(B_DCHFAmount).div(totalDeposit))
      ),
      100000
    )
    ...
    assert.isAtMost(
      th.getDifference(
        ...
        W_DCHFAmount_Asset.sub(liquidatedDebt_Asset.mul(W_DCHFAmount_Asset).div(totalDeposit_Asset))
      ),
      100000
    )
    assert.isAtMost(
      th.getDifference(
        ...
        A_DCHFAmount_Asset.sub(liquidatedDebt_Asset.mul(A_DCHFAmount_Asset).div(totalDeposit_Asset))
      ),
      100000
    )
    assert.isAtMost(
      th.getDifference(
        ...
        B_DCHFAmount_Asset.sub(liquidatedDebt_Asset.mul(B_DCHFAmount_Asset).div(totalDeposit_Asset))
      ),
      100000
    )
    ...
    assert.isAtMost(
      th.getDifference(
        whale_ETHGain,
        th.applyLiquidationFee(liquidatedColl).mul(W_DCHFAmount).div(totalDeposit)
      ),
      2000
    )
    assert.isAtMost(
      th.getDifference(
        alice_ETHGain,
        th.applyLiquidationFee(liquidatedColl).mul(A_DCHFAmount).div(totalDeposit)
      ),
      2000
    )
    assert.isAtMost(
      th.getDifference(
        bob_ETHGain,
        th.applyLiquidationFee(liquidatedColl).mul(B_DCHFAmount).div(totalDeposit)
      ),
      2000
    )
    ...
    assert.isAtMost(
      th.getDifference(
        ...
        th.applyLiquidationFee(
          liquidatedColl_Asset
            .div(toBN(10 ** 10))
            .mul(W_DCHFAmount_Asset)
            .div(totalDeposit_Asset)
        )
      ),
      2000
    )
    ...
    assert.isAtMost(th.getDifference(total_DCHFinSP, totalDeposit.sub(liquidatedDebt)), 1000)
    assert.isAtMost(th.getDifference(total_ETHinSP, th.applyLiquidationFee(liquidatedColl)), 1000)

    assert.isAtMost(
      th.getDifference(total_DCHFinSP_Asset, totalDeposit_Asset.sub(liquidatedDebt_Asset)),
      1000
    )
  ...
  it('liquidateTroves(): Liquidating troves at ICR <=100% with SP deposits does not alter their deposit or ETH gain', async () => {
    ...
    // Check DCHF and ETH in Pool  before
    ...
    const DCHFinSP_Before_Asset = (await stabilityPoolERC20.getTotalDCHFDeposits()).toString()
    const ETHinSP_Before_Asset = (await stabilityPoolERC20.getAssetBalance()).toString()

    assert.equal(DCHFinSP_Before, dec(800, 18))
    assert.equal(ETHinSP_Before, '0')
    assert.equal(DCHFinSP_Before_Asset, dec(800, 18))
    assert.equal(ETHinSP_Before_Asset, '0')

    // *** Check A, B, C ICRs < 100
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, alice, price)).lte(mv._ICR100))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price)).lte(mv._ICR100))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)).lte(mv._ICR100))

    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)).lte(mv._ICR100))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)).lte(mv._ICR100))
    assert.isTrue((await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)).lte(mv._ICR100))
    ...
    // Check DCHF and ETH in Pool after
    ...
    const DCHFinSP_After_Asset = (await stabilityPoolERC20.getTotalDCHFDeposits()).toString()
    ...
    assert.equal(DCHFinSP_Before_Asset, DCHFinSP_After_Asset)
  ...
  it('liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains active', async () => {
    ...
    That leaves 50 DCHF in the Pool to absorb exactly half of Carol's debt (100) */
    await troveManager.liquidateTroves(ZERO_ADDRESS, 10)
    await troveManager.liquidateTroves(erc20.address, 10)
  ...
  it('liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains in TroveOwners Array', async () => {
    ...
    // Check TroveOwners idx on trove struct == idx of address found in TroveOwners array
    const idxOnStruct = (await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[
      th.TROVE_ARRAY_INDEX
    ].toString()
    assert.equal(addressIdx.toString(), idxOnStruct)
    ...
    for (let i = 0; i < arrayLength_Asset; i++) {
      const address_Asset = (await troveManagerHelpers.TroveOwners(erc20.address, i)).toString()
    ...
    // Check TroveOwners idx on trove struct == idx of address found in TroveOwners array
    const idxOnStruct_Asset = (await troveManagerHelpers.Troves(carol, erc20.address))[
      th.TROVE_ARRAY_INDEX
    ].toString()
    assert.equal(addressIdx_Asset.toString(), idxOnStruct_Asset)
  })
  ...
  it('liquidateTroves() with a non fullfilled liquidation: still can liquidate further troves after the non-liquidated, emptied pool', async () => {
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 16)),
      extraDCHFAmount: D_totalDebt_Asset,
      extraParams: { from: carol },
    })
    await openTrove({ asset: erc20.address, ICR: toBN(dec(208, 16)), extraParams: { from: erin } })
    ...
    const ICR_C = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)
    const ICR_D = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, dennis, price)
    const ICR_E = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, erin, price)

    const ICR_A_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)
    const ICR_B_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)
    const ICR_C_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)
    const ICR_D_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, dennis, price)
    const ICR_E_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, erin, price)
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
     ...
     That leaves 97 DCHF in the Pool that won’t be enough to absorb Carol,
  ...
  it('liquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct', async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 DCHF in the Pool that won’t be enough to absorb any other trove */
    await troveManager.liquidateTroves(ZERO_ADDRESS, 10)
    await troveManager.liquidateTroves(erc20.address, 10)

    // Expect system debt reduced by 203 DCHF and system coll 2.3 ETH
    const entireSystemCollAfter = await troveManager.getEntireSystemColl(ZERO_ADDRESS)
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt(ZERO_ADDRESS)
    const entireSystemCollAfter_Asset = await troveManager.getEntireSystemColl(erc20.address)
    const entireSystemDebtAfter_Asset = await troveManager.getEntireSystemDebt(erc20.address)
    ...
    th.assertIsApproximatelyEqual(
      changeInEntireSystemDebt_Asset.toString(),
      A_totalDebt_Asset.add(B_totalDebt_Asset)
    )
  })
  ...
  it('liquidateTroves() with a non fullfilled liquidation: emits correct liquidation event values', async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 DCHF in the Pool which won’t be enough for any other liquidation */
    const liquidationTx = await troveManager.liquidateTroves(ZERO_ADDRESS, 10)
    const liquidationTx_Asset = await troveManager.liquidateTroves(erc20.address, 10)

    const [liquidatedDebt, liquidatedColl, collGasComp, DCHFGasComp] =
      th.getEmittedLiquidationValues(liquidationTx)
    const [liquidatedDebt_Asset, liquidatedColl_Asset, collGasComp_Asset, DCHFGasComp_Asset] =
    ...
    assert.equal(DCHFGasComp.toString(), dec(400, 18))
    ...
    assert.equal(DCHFGasComp_Asset.toString(), dec(400, 18))

    // check collateral surplus
    const alice_remainingCollateral = A_coll.sub(A_totalDebt.mul(th.toBN(dec(11, 17))).div(price))
    const bob_remainingCollateral = B_coll.sub(B_totalDebt.mul(th.toBN(dec(11, 17))).div(price))
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getCollateral(ZERO_ADDRESS, alice),
    ...
    const alice_remainingCollateral_Asset = A_coll_Asset.sub(
      A_totalDebt_Asset.mul(th.toBN(dec(11, 17))).div(price)
    )
    const bob_remainingCollateral_Asset = B_coll_Asset.sub(
      B_totalDebt_Asset.mul(th.toBN(dec(11, 17))).div(price)
    )
    ...
    th.assertIsApproximatelyEqual(
      ...
      alice_balanceBefore_Asset.add(th.toBN(alice_remainingCollateral_Asset.div(toBN(10 ** 10))))
    )
  ...
  it('liquidateTroves() with a non fullfilled liquidation: ICR of non liquidated trove does not change', async () => {
    ...
    const ICR_A = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, alice, price)
    const ICR_B = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, bob, price)
    const ICR_C_Before = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)

    const ICR_A_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, alice, price)
    const ICR_B_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, bob, price)
    const ICR_C_Before_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)
    ...
    const ICR_C_After = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, carol, price)
    assert.equal(ICR_C_Before.toString(), ICR_C_After)
    const ICR_C_After_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, carol, price)
  ...
  it('batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode', async () => {
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(426, 16)),
      extraDCHFAmount: A_totalDebt_Asset,
      extraParams: { from: whale },
    })
    ...
    await dchfToken.transfer(alice, A_totalDebt.add(A_totalDebt_Asset), { from: whale })
    // Alice closes trove
  ...
  it('batchLiquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct', async () => {
    ...
    // Expect system debt reduced by 203 DCHF and system coll by 2 ETH
    const entireSystemCollAfter = await troveManager.getEntireSystemColl(ZERO_ADDRESS)
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt(ZERO_ADDRESS)
  ...
  it('batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool DCHF > debt to liquidate: can liquidate troves out of order', async () => {
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: alice },
    })
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool empty: doesn't liquidate any troves", async () => {
    ...
    await openTrove({ asset: erc20.address, ICR: toBN(dec(222, 16)), extraParams: { from: alice } })
    const { totalDebt: bobDebt_Before_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(224, 16)),
      extraParams: { from: bob },
    })
    const { totalDebt: carolDebt_Before_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(226, 16)),
      extraParams: { from: carol },
    })
    const { totalDebt: dennisDebt_Before_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(228, 16)),
      extraParams: { from: dennis },
    })

    const bobColl_Before = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
    const carolColl_Before = (await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
    const dennisColl_Before = (await troveManagerHelpers.Troves(dennis, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]

    const bobColl_Before_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_COLL_INDEX]
    const carolColl_Before_Asset = (await troveManagerHelpers.Troves(carol, erc20.address))[
      th.TROVE_COLL_INDEX
    ]
    const dennisColl_Before_Asset = (await troveManagerHelpers.Troves(dennis, erc20.address))[
      th.TROVE_COLL_INDEX
    ]
    ...
    const trovesToLiquidate = [dennis, bob, carol]
    await assertRevert(
      troveManager.batchLiquidateTroves(ZERO_ADDRESS, trovesToLiquidate),
      'TroveManager: nothing to liquidate'
    )
    await assertRevert(
      troveManager.batchLiquidateTroves(erc20.address, trovesToLiquidate),
      'TroveManager: nothing to liquidate'
    )
    ...
    assert.equal((await troveManagerHelpers.Troves(dennis, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX], '1')
    assert.equal((await troveManagerHelpers.Troves(dennis, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX], '1')
    assert.equal((await troveManagerHelpers.Troves(dennis, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX], '1')

    assert.equal((await troveManagerHelpers.Troves(dennis, erc20.address))[th.TROVE_STATUS_INDEX], '1')
    assert.equal((await troveManagerHelpers.Troves(dennis, erc20.address))[th.TROVE_STATUS_INDEX], '1')
    assert.equal((await troveManagerHelpers.Troves(dennis, erc20.address))[th.TROVE_STATUS_INDEX], '1')

    // Confirm D, B, C coll & debt have not changed
    const dennisDebt_After = (await troveManagerHelpers.Troves(dennis, ZERO_ADDRESS))[
      th.TROVE_DEBT_INDEX
    ].add(await troveManagerHelpers.getPendingDCHFDebtReward(ZERO_ADDRESS, dennis))
    const bobDebt_After = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX].add(
      await troveManagerHelpers.getPendingDCHFDebtReward(ZERO_ADDRESS, bob)
    )
    const carolDebt_After = (await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX].add(
      await troveManagerHelpers.getPendingDCHFDebtReward(ZERO_ADDRESS, carol)
    )

    const dennisColl_After = (await troveManagerHelpers.Troves(dennis, ZERO_ADDRESS))[
      th.TROVE_COLL_INDEX
    ].add(await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, dennis))
    const bobColl_After = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX].add(
      await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, bob)
    )
    const carolColl_After = (await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[th.TROVE_COLL_INDEX].add(
      await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, carol)
    )

    const dennisDebt_After_Asset = (await troveManagerHelpers.Troves(dennis, erc20.address))[
      th.TROVE_DEBT_INDEX
    ].add(await troveManagerHelpers.getPendingDCHFDebtReward(erc20.address, dennis))
    const bobDebt_After_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[
      th.TROVE_DEBT_INDEX
    ].add(await troveManagerHelpers.getPendingDCHFDebtReward(erc20.address, bob))
    const carolDebt_After_Asset = (await troveManagerHelpers.Troves(carol, erc20.address))[
      th.TROVE_DEBT_INDEX
    ].add(await troveManagerHelpers.getPendingDCHFDebtReward(erc20.address, carol))

    const dennisColl_After_Asset = (await troveManagerHelpers.Troves(dennis, erc20.address))[
      th.TROVE_COLL_INDEX
    ].add(await troveManagerHelpers.getPendingAssetReward(erc20.address, dennis))
    const bobColl_After_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[
      th.TROVE_COLL_INDEX
    ].add(await troveManagerHelpers.getPendingAssetReward(erc20.address, bob))
    const carolColl_After_Asset = (await troveManagerHelpers.Troves(carol, erc20.address))[
      th.TROVE_COLL_INDEX
    ].add(await troveManagerHelpers.getPendingAssetReward(erc20.address, carol))
    ...
    th.assertIsApproximatelyEqual(
      th.toBN(dennisDebt_Before_Asset).toString(),
      dennisDebt_After_Asset.toString()
    )
    th.assertIsApproximatelyEqual(th.toBN(bobDebt_Before_Asset).toString(), bobDebt_After_Asset.toString())
    th.assertIsApproximatelyEqual(
      th.toBN(carolDebt_Before_Asset).toString(),
      carolDebt_After_Asset.toString()
    )
  })
  ...
  it('batchLiquidateTroves(): skips liquidation of troves with ICR > TCR, regardless of Stability Pool size', async () => {
    ...
    const { totalDebt: I_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(290, 16)),
      extraParams: { from: I },
    })
    ...
    const G_collBefore = (await troveManagerHelpers.Troves(G, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
    const G_debtBefore = (await troveManagerHelpers.Troves(G, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]
    const H_collBefore = (await troveManagerHelpers.Troves(H, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
    const H_debtBefore = (await troveManagerHelpers.Troves(H, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]
    const I_collBefore = (await troveManagerHelpers.Troves(I, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
    const I_debtBefore = (await troveManagerHelpers.Troves(I, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]

    const G_collBefore_Asset = (await troveManagerHelpers.Troves(G, erc20.address))[th.TROVE_COLL_INDEX]
    const G_debtBefore_Asset = (await troveManagerHelpers.Troves(G, erc20.address))[th.TROVE_DEBT_INDEX]
    const H_collBefore_Asset = (await troveManagerHelpers.Troves(H, erc20.address))[th.TROVE_COLL_INDEX]
    const H_debtBefore_Asset = (await troveManagerHelpers.Troves(H, erc20.address))[th.TROVE_DEBT_INDEX]
    const I_collBefore_Asset = (await troveManagerHelpers.Troves(I, erc20.address))[th.TROVE_COLL_INDEX]
    const I_debtBefore_Asset = (await troveManagerHelpers.Troves(I, erc20.address))[th.TROVE_DEBT_INDEX]

    const ICR_A = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, A, price)
    const ICR_B = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, B, price)
    const ICR_C = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, C, price)
    const ICR_D = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, D, price)
    const ICR_E = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, E, price)
    const ICR_F = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, F, price)
    const ICR_G = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, G, price)
    const ICR_H = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, H, price)
    const ICR_I = await troveManagerHelpers.getCurrentICR(ZERO_ADDRESS, I, price)

    const ICR_A_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, A, price)
    const ICR_B_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, B, price)
    const ICR_C_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, C, price)
    const ICR_D_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, D, price)
    const ICR_E_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, E, price)
    const ICR_F_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, F, price)
    const ICR_G_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, G, price)
    const ICR_H_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, H, price)
    const ICR_I_Asset = await troveManagerHelpers.getCurrentICR(erc20.address, I, price)
    ...
    await assertRevert(
      troveManager.batchLiquidateTroves(ZERO_ADDRESS, [G, H, I]),
      'TroveManager: nothing to liquidate'
    )
    await assertRevert(
      troveManager.batchLiquidateTroves(erc20.address, [G, H, I]),
      'TroveManager: nothing to liquidate'
    )
    ...
    assert.equal(G_collBefore.eq(await troveManagerHelpers.Troves(G, ZERO_ADDRESS))[th.TROVE_COLL_INDEX])
    assert.equal(G_debtBefore.eq(await troveManagerHelpers.Troves(G, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX])
    assert.equal(H_collBefore.eq(await troveManagerHelpers.Troves(H, ZERO_ADDRESS))[th.TROVE_COLL_INDEX])
    assert.equal(H_debtBefore.eq(await troveManagerHelpers.Troves(H, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX])
    assert.equal(I_collBefore.eq(await troveManagerHelpers.Troves(I, ZERO_ADDRESS))[th.TROVE_COLL_INDEX])
    assert.equal(I_debtBefore.eq(await troveManagerHelpers.Troves(I, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX])

    assert.equal(
      G_collBefore_Asset.eq(await troveManagerHelpers.Troves(G, erc20.address))[th.TROVE_COLL_INDEX]
    )
    assert.equal(
      G_debtBefore_Asset.eq(await troveManagerHelpers.Troves(G, erc20.address))[th.TROVE_DEBT_INDEX]
    )
    assert.equal(
      H_collBefore_Asset.eq(await troveManagerHelpers.Troves(H, erc20.address))[th.TROVE_COLL_INDEX]
    )
    assert.equal(
      H_debtBefore_Asset.eq(await troveManagerHelpers.Troves(H, erc20.address))[th.TROVE_DEBT_INDEX]
    )
    assert.equal(
      I_collBefore_Asset.eq(await troveManagerHelpers.Troves(I, erc20.address))[th.TROVE_COLL_INDEX]
    )
    assert.equal(
      I_debtBefore_Asset.eq(await troveManagerHelpers.Troves(I, erc20.address))[th.TROVE_DEBT_INDEX]
    )
    ...
    // Whale withdraws entire deposit, and re-deposits 132 DCHF
    ...
    // Expected Stability Pool to fully absorb B (92 DCHF + 10 virtual debt),
    // but not E as there are not enough funds in Stability Pool

    const stabilityBefore = await stabilityPool.getTotalDCHFDeposits()
    const dEbtBefore = (await troveManagerHelpers.Troves(E, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]

    const stabilityBefore_Asset = await stabilityPoolERC20.getTotalDCHFDeposits()
    const dEbtBefore_Asset = (await troveManagerHelpers.Troves(E, erc20.address))[th.TROVE_DEBT_INDEX]
    ...
    const dEbtAfter = (await troveManagerHelpers.Troves(E, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]
    const stabilityAfter = await stabilityPool.getTotalDCHFDeposits()

    const dEbtAfter_Asset = (await troveManagerHelpers.Troves(E, erc20.address))[th.TROVE_DEBT_INDEX]
    const stabilityAfter_Asset = await stabilityPoolERC20.getTotalDCHFDeposits()
