  ...
  const testCorpus = ({ withProxy = false }) => {
    ...
    if (!withProxy) {
      ...
      it('approve(): reverts when owner param is address(0)', async () => {
        const txPromise = DCHFTokenTester.callInternalApprove(ZERO_ADDRESS, alice, dec(1000, 18), {
          from: bob,
        })
        await assertAssert(txPromise)
      })
    ...
    if (!withProxy) {
      ...
      it('permits and emits an Approval event (replay protected)', async () => {
        ...
        await assertRevert(
          DCHFTokenTester.permit(approve.owner, approve.spender, approve.value, deadline, v, r, s),
          'DCHF: invalid signature'
        )

        // Check that the zero address fails
        await assertAssert(
          DCHFTokenTester.permit(
