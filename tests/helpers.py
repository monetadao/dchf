...
def logGlobalState(contracts):
    ...
    activePoolDebt = contracts.activePool.getDCHFDebt()
    defaultPoolColl = contracts.defaultPool.getETH()
    defaultPoolDebt = contracts.defaultPool.getDCHFDebt()
    ...
    SP_USDV = contracts.stabilityPool.getTotalDCHFDeposits().to("ether")
    SP_ETH = contracts.stabilityPool.getETH().to("ether")
    print('SP DCHF         ', SP_USDV)
    print('SP ETH          ', SP_ETH)
    ...
    return [ETH_price, num_troves, total_coll, total_debt, TCR, recovery_mode, last_ICR, SP_USDV, SP_ETH]
