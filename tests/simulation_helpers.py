...
DCHF_GAS_COMPENSATION = 200.0
MIN_NET_DEBT = 1800.0
MAX_FEE = Wei(1e18)

"""# Ether price (exogenous)

Ether is the collateral for DCHF. The ether price $P_t^e$ follows
...
"""# MON price
In the first month, the price of MON follows

> $P_t^q = P_{t-1}^q (1+\zeta_t^q)(1+\sigma_t^q)$. 

Note that $\zeta_t^q \sim N(0,$ sd_MON) represents MON price shock and $\sigma_t^q$ the drift. Here, $\sigma_t^q =$ drift_MON, so that the expected MON price increases from price_MON_initial to the following at the end of the first month:
> $E(P_{720}^q) = $price_MON_initial$ \cdot (1+$ drift_MON$)^{720}$

The MON price from the second month on is endogenously determined.
"""

#MON price & airdrop
price_MON_initial = 0.4
...
"""**MON Endogenous Price**

The staked MON pool earning consists of the issuance fee revenue and redemption fee revenue
> $R_t^q = R_t^i + R_t^r.$
...
> $$r_t=\frac{MON Market Cap}{Annualized Earning}=\frac{MC_t}{E_t}$$

> $MC_t=P_t^q \cdot$ MON_total_supply

Therefore, the MON price dynamics is determined
> $$P_t^q=discount \cdot \frac{r^{PE}}{MON\_total\_supply}E_t$$

Interpretation: The denominator implies that with more MON tokens issued, MON price decreases. However, the depreciation effect can be counteracted by the growth of the earning.
...
$R_{t-1}^s$ is the return in the stability pool, which consists of liquidation gain and airdrop MON gain.
...
> (collateral in ether, debt in DCHF, collateral ratio target, rational inattention, collateral ratio)
...
$P_{t}^{l}$ is the price of DCHF.

$N_t^o$ is rounded to an integer.

---

The amount of DCHF tokens generated by a new trove is
...
- $R_t^a=P_{t}^q\hat{Q}_t^q$ is the airdrop gain, $\hat{Q}_t^q=1000$ denotes the amount of MON token airdropped to the stability pool providers
- $D_{t}^{s}$ is the total amount of DCHF deposited in the Stability Pool (see below)
...
def pending_liquidations(contracts, price_ether_current):
    ...
    stability_pool_balance = contracts.stabilityPool.getTotalDCHFDeposits()
    trove = last_trove
...
# F = 0.5 ** (1/8760)
# F = 0.99992087674
def quantity_MON_airdrop(index):
    F = 0.99992087674
    if index <= 0:
        return 0
    return 32e6 * (F ** (index-1) - F ** index)

def liquidate_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_USDV, price_MON_current, data, index):
    if len(active_accounts) == 0:
        return [0, 0]

    stability_pool_previous = contracts.stabilityPool.getTotalDCHFDeposits() / 1e18
    ...
    while pending_liquidations(contracts, price_ether_current):
        try:
            ...
            for i in range(NUM_LIQUIDATIONS):
                ...
                ICR = contracts.troveManager.getCurrentICR(trove, Wei(price_ether_current * 1e18))
                print(f"ICR: {ICR}")
    stability_pool_current = contracts.stabilityPool.getTotalDCHFDeposits() / 1e18
    ...
    liquidation_gain = ether_liquidated * price_ether_current - debt_liquidated * price_USDV
    airdrop_gain = price_MON_current * quantity_MON_airdrop(index)
...
def close_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_USDV, index):
    if len(active_accounts) == 0:
        return [0]
    ...
    if index <= 240:
        ...
        number_closetroves = max(0, n_steady * (1+shock_closetroves)) + beta*(1-price_USDV)*n_troves
    ...
    for i in range(0, len(drops)):
        ...
        coll = amounts['coll']
        debt = amounts['debt']
        pending = get_USDV_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt)
        if pending == 0:
...
def transfer_from_to(contracts, from_account, to_account, amount):
    balance = contracts.dchfToken.balanceOf(from_account)
    transfer_amount = min(balance, amount)
    if transfer_amount == 0:
        return amount
    if from_account == to_account:
        return amount
    contracts.dchfToken.transfer(to_account, transfer_amount, { 'from': from_account })
    pending = amount - transfer_amount

    return pending

def get_USDV_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt):
    USDVBalance = contracts.dchfToken.balanceOf(account)
    if debt > USDVBalance:
        ...
        # try with active accounts, which are more likely to hold DCHF
        ...
        if pending > 0:
            print(f"\n ***Error: not enough DCHF to repay! {debt / 1e18} DCHF for {account}")
    ...
    for i, working_trove in enumerate(active_accounts):
        ...
        if p >= ratio:
            ...
            if check < -1:
                ...
                pending = get_USDV_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, repay_amount)
                if pending == 0:
                    contracts.borrowerOperations.repayDCHF(repay_amount, hints[0], hints[1], { 'from': account })
                ...
                if isNewTCRAboveCCR(contracts, 0, False, withdraw_amount_wei, True, floatToWei(price_ether_current)):
                    contracts.borrowerOperations.withdrawDCHF(MAX_FEE, withdraw_amount_wei, hints[0], hints[1], { 'from': account })
                    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
                    issuance_USDV_adjust = issuance_USDV_adjust + rate_issuance * withdraw_amount
...
def open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention, price_ether_current):
    ...
    debtChange = floatToWei(supply_trove) + DCHF_GAS_COMPENSATION
    DCHF = get_USDV_amount_from_net_debt(contracts, floatToWei(supply_trove))
    if isNewTCRAboveCCR(contracts, coll, True, debtChange, True, floatToWei(price_ether_current)):
        contracts.borrowerOperations.openTrove(MAX_FEE, DCHF, hints[0], hints[1],
...
def open_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_USDV, index):
    ...
    if index <= 0:
        ...
        number_opentroves = max(0, n_steady * (1+shock_opentroves)) + \
                        alpha * (price_USDV - rate_issuance - 1) * n_troves
    ...
    for i in range(0, number_opentroves):
        ...
        issuance_USDV_open = issuance_USDV_open + rate_issuance * supply_trove
...
def stability_update(accounts, contracts, active_accounts, return_stability, index):
    supply = contracts.dchfToken.totalSupply() / 1e18
    stability_pool_previous = contracts.stabilityPool.getTotalDCHFDeposits() / 1e18
    ...
    if stability_pool > stability_pool_previous:
        ...
        while remaining > 0 and i < len(active_accounts):
          ...
          balance = contracts.dchfToken.balanceOf(account) / 1e18
        ...
        current_deposit = contracts.stabilityPool.getCompoundedDCHFDeposit(accounts[0])
...
---
With the supply and demand of DCHF tokens defined above, the price of DCHF at the current period is given by the following equilibrium condition:
> $$S_t = D_t^s + D_t^l = D_t^s + D_{t-1}^l (1+\zeta_t^l)(1+\sigma_t^l) (\frac{P_t^l}{P_{t-1}^l})^\delta$$

where $S$ is the total supply of DCHF.
...
def calculate_price(price_USDV, liquidity_pool, liquidity_pool_next):
  ...
  price_USDV_current= price_USDV * (liquidity_pool / liquidity_pool_next) ** (1/delta)

  return price_USDV_current

""" **Stabilizers**

There are two stabilizers to attenuate DCHF price deviation from its target range.
...
The amount of $Q_t^d(c)$ is expected to bring the DCHF price back to $1.1+f_t^i$. This means that
...
If $P_t^l < 1-f_t^r$, a fraction $\chi_t$ of DCHF in the liquidity pool is used for redemption
> $$D_t^r = \chi_t D_t^l,$$
...
Note that unlike stabilizer 1, stabilizer 2 has impact of DCHF price in
...
def redeem_trove(accounts, contracts, i, price_ether_current):
    USDV_balance = contracts.dchfToken.balanceOf(accounts[i])
    [firstRedemptionHint, partialRedemptionHintNICR, truncatedDCHFamount] = contracts.hintHelpers.getRedemptionHints(USDV_balance, price_ether_current, 70)
    if truncatedDCHFamount == Wei(0):
        return None
    ...
    try:
        ...
        print(f"DCHF Token:    {contracts.dchfToken.address}")
        print(f"i: {i}")
        print(f"account: {accounts[i]}")
        print(f"DCHF bal: {USDV_balance / 1e18}")
        print(f"truncated: {truncatedDCHFamount / 1e18}")
        ...
        print(f"amount: {truncatedDCHFamount}")
...
def price_stabilizer(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_USDV, index):

    stability_pool = contracts.stabilityPool.getTotalDCHFDeposits() / 1e18
    redemption_pool = 0
    redemption_fee = 0
    ...
    #Calculating Price
    price_USDV_current = calculate_price(price_USDV, liquidity_pool, liquidity_pool_next)
    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
    ...
    if price_USDV_current > 1.1 + rate_issuance:
        supply_wanted = stability_pool + \
                        liquidity_pool_next * \
                        ((1.1+rate_issuance) / price_USDV)**delta
        ...
        issuance_USDV_stabilizer = rate_issuance * supply_trove
    ...
    if price_USDV_current < 1 - rate_redemption:
        ...
        supply_target = stability_pool + \
                        liquidity_pool_next * \
                        ((1-rate_redemption) / price_USDV)**delta
    ...
    return [price_USDV_current, redemption_pool, redemption_fee, issuance_USDV_stabilizer]

"""# MON Market"""

def MON_market(index, data):
    #quantity_MON = (MON_total_supply/3)*(1-0.5**(index/period))
    ...
    return [price_MON_current, annualized_earning]
