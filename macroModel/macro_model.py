...
def liquidate_troves(troves, index, data):
  ...
  if index <= day:
   return_stability = initial_return*(1+shock_return)
    ...
    return_stability = min(0.5, 365*(data.loc[index-day:index, 'liquidation_gain'].sum()+data.loc[index-day:index, 'airdrop_gain'].sum())/(price_USDV_previous*stability_pool_previous))
  else:
    return_stability = (365/30)*(data.loc[index-month:index, 'liquidation_gain'].sum()+data.loc[index-month:index, 'airdrop_gain'].sum())/(price_USDV_previous*stability_pool_previous)
  
  return[troves, return_stability, debt_liquidated, ether_liquidated, liquidation_gain, airdrop_gain, n_liquidate]

"""Close Troves"""

def close_troves(troves, index2, price_USDV_previous):
  ...
  if index2 <= 240:
    ...
    number_closetroves = max(0, n_steady * (1+shock_closetroves)) + beta*(1-price_USDV_previous)*n_troves
...
def adjust_troves(troves, index):
  ...
  #A part of the troves are adjusted by adjusting debt
    if p >= ratio:
      ...
      if check>2:
        ...
        issuance_USDV_adjust = issuance_USDV_adjust + rate_issuance * (supply_new - working_trove['Supply'])
  ...
  return[troves, issuance_USDV_adjust]

"""Open Troves"""

def open_troves(troves, index1, price_USDV_previous):
  ...
  if index1<=0:
    ...
    number_opentroves = max(0, n_steady * (1+shock_opentroves)) + alpha*(price_USDV_previous-rate_issuance-1)*n_troves
...
#Calculating Price
  ...
  price_USDV_current= price_USDV_previous*((supply-stability_pool)/(liquidity_pool_previous*(drift_liquidity+shock_liquidity)))**(1/delta)
...
#Stabilizer
  ...
  if price_USDV_current > 1.1 + rate_issuance:
    ...
    supply_wanted=stability_pool+liquidity_pool_previous*(drift_liquidity+shock_liquidity)*((1.1+rate_issuance)/price_USDV_previous)**delta
  ...
  if price_USDV_current < 1 - rate_redemption:
    ...
    #supply_current = sum(troves['Supply'])
    supply_target=stability_pool+liquidity_pool_previous*(drift_liquidity+shock_liquidity)*((1-rate_redemption)/price_USDV_previous)**delta
    ...
    if supply_diff < redemption_ratio * liquidity_pool:
      ...
      price_USDV_current= price_USDV_previous * (liquidity_pool/(liquidity_pool_previous*(drift_liquidity+shock_liquidity)))**(1/delta)
...
#close troves
  result_close = close_troves(troves, index, price_USDV_previous)
...
#open troves
  result_open = open_troves(troves, index, price_USDV_previous)
  troves = result_open[0]
...
troves2= pd.DataFrame({"Ether_Price":[], "Ether_Quantity":[], "CR_initial":[], 
              "Supply":[], "Rational_inattention":[], "CR_current":[]})
result_open = open_troves(troves2, 0, data2['Price_USDV'][0])
troves2 = result_open[0]
issuance_USDV_open = result_open[2]
data2.loc[0,'issuance_fee'] = issuance_USDV_open * initials["Price_USDV"][0]
data2.loc[0,'supply_USDV'] = troves2["Supply"].sum()
...
#close troves
  result_close = close_troves(troves2, index, price_USDV_previous)
...
#open troves
  result_open = open_troves(troves2, index, price_USDV_previous)
  troves2 = result_open[0]
  n_open = result_open[1]  
  issuance_USDV_open = result_open[2]
